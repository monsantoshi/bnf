            ...
            for key in object_list:
                if (key.get('positionSide') =='LONG' and key.get('positionAmt') > 0.0):
                    ...
                    try:
                        ...
                        if  ((priceIn > 3000)):
                            print("""""""""""""""""""""""")
                            p = str(round(float(priceIn) +((0.02* float(priceIn))/20),2))
                            pf = str(round(float(priceIn) +((0.02* float(priceIn))/20),2))

                            ploss =  str(round(float(priceIn) +((-0.10*float(priceIn))/20),2))
                            plostf =  str(round(float(priceIn) +((-0.20*float(priceIn))/20),2))
                            plostl =  str(round(float(priceIn) +((-0.20*float(priceIn))/20),2))
                            if (mPrice >= Decimal(ploss) and float(a) <=30 ):
                                ...
                                time.sleep(1)
                            elif (mPrice >= Decimal(ploss) and float(a) > 30 ):
                        ...
                        elif  ((priceIn > 120) and (priceIn <=3000)):
                            print("""""""""""""""""""""""")
                            p = str(round(float(priceIn) +((0.02* float(priceIn))/20),3))
                            pf = str(round(float(priceIn) +((0.02* float(priceIn))/20),3))

                            ploss =  str(round(float(priceIn) +((-0.10*float(priceIn))/20),3))
                            plostf =  str(round(float(priceIn) +((-0.20*float(priceIn))/20),3))
                            plostl =  str(round(float(priceIn) +((-0.20*float(priceIn))/20),3))
                            if (mPrice >= Decimal(ploss) and float(a) <=30 ):
                                take_profit_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.LIMIT, price=pf, quantity=a,timeInForce='GTC')
                                time.sleep(1)
                            elif (mPrice >= Decimal(ploss) and float(a) > 30 ):
                            ...
                            p = str(round(float(priceIn) +((0.02* float(priceIn))/20),5))
                            pf = str(round(float(priceIn) +((0.02* float(priceIn))/20),5))
                            ploss =  str(round(float(priceIn) +((-0.10*float(priceIn))/20),5))
                            plostf =  str(round(float(priceIn) +((-0.20*float(priceIn))/20),5))
                            plostl =  str(round(float(priceIn) +((-0.20*float(priceIn))/20),5))
                            if (mPrice >= Decimal(ploss) and float(a) <=30 ):
                                take_profit_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.LIMIT, price=pf, quantity=a,timeInForce='GTC')
                                time.sleep(1)
                            elif (mPrice >= Decimal(ploss) and float(a) > 30 ):
                            ...
                            p = str(round(float(priceIn) +((0.02* float(priceIn))/20),6))
                            pf = str(round(float(priceIn) +((0.02* float(priceIn))/20),6))
                            ploss =  str(round(float(priceIn) +((-0.10*float(priceIn))/20),6))
                            plostf =  str(round(float(priceIn) +((-0.20*float(priceIn))/20),6))
                            plostl =  str(round(float(priceIn) +((-0.20*float(priceIn))/20),6))
                            if (mPrice >= Decimal(ploss) and float(a) <=30 ):
                                take_profit_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.LIMIT, price=pf, quantity=a,timeInForce='GTC')
                                time.sleep(1)
                            elif (mPrice >= Decimal(ploss) and float(a) > 30 ):
