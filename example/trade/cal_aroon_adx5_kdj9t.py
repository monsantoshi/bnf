...
class calculate_signal:
    ...
    def supertrend(df, period=14, atr_multiplier=2):
        ...
        df = pd.concat([df,adx,aroon,kdj],axis=1)
        for current in range(1, len(df.index)):
            previous = current - 1


            if df['close'][current] > df['upperband'][previous]:
                df['in_uptrendSU'][current] = True
                su = 1
            elif df['close'][current] < df['lowerband'][previous]:
                df['in_uptrendSU'][current] = False
                su =0
            else:
                df['in_uptrendSU'][current] = df['in_uptrendSU'][previous]

                if df['in_uptrendSU'][current] and df['lowerband'][current] < df['lowerband'][previous]:
                    df['lowerband'][current] = df['lowerband'][previous]

                if not df['in_uptrendSU'][current] and df['upperband'][current] > df['upperband'][previous]:
                    df['upperband'][current] = df['upperband'][previous] 
            
            
            if df['AROONOSC_14'][current] >= 30 and df['AROONOSC_14'][current] <= 90: 
                if df['AROONU_14'][current] > df['AROOND_14'][current]: 
                    df['in_uptrend'][current]= True   
                    s =1   
                # elif df['AROONU_5'][current] < df['AROOND_5'][current]:
            ...
            elif df['AROONOSC_14'][current] > -90 and df['AROONOSC_14'][current] < 0:
                #if df['AROONOSC_5'][current] >= 50 : 
                # if df['AROONU_5'][current] > df['AROOND_5'][current]: 
                #     df['in_uptrend'][current]= True   
                #     s =1   
                if df['AROONU_14'][current] < df['AROOND_14'][current]:
                    df['in_uptrend'][current]= False
            ...
            if ((su==1) and (s == 1) and (k==1) and (a==1)):
                #if df['in_uptrend'][current]==True and df['in_uptrendADX'][current]==True: 
                df['in_uptrendAll'][current]=8
            elif ((su==0) and (s == 0) and (k==0) and (a==0)):
                #if df['in_uptrend'][current]==True and df['in_uptrendADX'][current]==True: 
                df['in_uptrendAll'][current]=22
            else:
                df['in_uptrendAll'][current]= df['in_uptrendAll'][previous]  
