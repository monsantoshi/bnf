...
def take_order_sell_coin_adausdt(self,markett,coin):
    ...
    for key in object_list:
        if (key.get('positionSide') =='SHORT' and key.get('symbol') == markett and key.get('positionAmt') <= 0.0):
            ...
            print("ass" ,ass)
            plostsf =  str(round(float(priceIns) +((3.00*float(priceIns))/20),2))
            ...
            if ((float(ass) >=  0.01 )  and float(ass) <0.02 and float(mPrices) > float(plostsf) ):
...
def take_order_buy_coin_adausdt(self,markett,coin):
    ...
    #         coin_data = collection_coin.find_one({"market":markett})
            ...
            if ((float(a) < 0.01) ):
                ...
                PrintBasic.print_obj(take_profit_coin)
            if ((float(a) >= 0.01 )  and float(a) < 0.02 and float(mPrice) < float(plostf)):
...
def check_buy_sell_signals(df):
    ...
    # if (df['in_uptrendAll'][last_row_index] != 8): # or df['in_uptrendAll'][last_row_index] != 8 or df['in_uptrendAll'][last_row_index] != 1) :
    ...
    #         close_order_buy_coin_usdt(self,'ETHUSDT')  
    #         in_position = False
    #         message = f"CLOSE POSITION LONG ETHUSDT"
    ...
    # if (df['in_uptrendAll'][last_row_index] != 22): #or df['in_uptrendAll'][last_row_index] != 6 or df['in_uptrendAll'][last_row_index] != 4) :   
    #     if (df['in_uptrendAll'][previous_row_index] ==22):
    #         #order = exchange.create_market_buy_order('ETH/USD', 0.05)
    #     #order = exchange.create_order(symbol='adausdt', side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=1)
    #         close_order_sell_coin_usdt(self,'ETHUSDT')  
    #         #in_position = False
    #         message = f"CLOSE POSITION SHORT ETHUSDT"
    ...
    if df['in_uptrendAll'][last_row_index] == 8:
        if not in_position:
            ...
            #order = exchange.create_order(symbol='adausdt', side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=1)
                ...
                order = take_order_buy_coin_adausdt(self,markett,0.01)
                print(order)
                in_position = True
    ...
    if df['in_uptrendAll'][last_row_index] == 22:
        if in_position:
            ...
            if (df['in_uptrendAll'][previous_row_index] != 22):
                ...
                order = take_order_sell_coin_adausdt(self,markett,0.01)
                print(order)
                in_position = False
...
def run_bot():
    ...
    # bars = exchange.fetch_ohlcv('ETHUSDT', timeframe='5m', limit=140)
    ...
    bars = request_client.get_candlestick_data(symbol="ETHUSDT", interval=CandlestickInterval.MIN3,startTime=None, endTime=None, limit=100)
