                ...
                if (key.get('positionSide') =='SHORT' and key.get('positionAmt') < 0.0):
                    ...
                    try:
                        if (priceIns > 120):
                            ...
                            ps = str(round(float(priceIns) -((0.030* float(priceIns))/20),3))
                            psf = str(round(float(priceIns) -((0.030* float(priceIns))/20),3))
                            psloss = str(round(float(priceIns) +((0.10*float(priceIns))/20),3))
                            pslostf = str(round(float(priceIns) +((0.25*float(priceIns))/20),3))
                            pslostl = str(round(float(priceIns) +((0.30*float(priceIns))/20),3))
                            ...
                            ps = str(round(float(priceIns) -((0.030* float(priceIns))/20),5))

                            psf = str(round(float(priceIns) -((0.030* float(priceIns))/20),5))
                            psloss = str(round(float(priceIns) +((0.10*float(priceIns))/20),5))
                            pslostf = str(round(float(priceIns) +((0.25*float(priceIns))/20),5))
                            pslostl = str(round(float(priceIns) +((0.30*float(priceIns))/20),5))
                            if (mPrices <= Decimal(psloss)  and float(ass) <= 30):
                                ake_profit_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=psf, quantity=ass,timeInForce='GTC')
                                time.sleep(1)
                            elif (mPrices <= Decimal(psloss)  and float(ass) > 30):
                            ...
                            ps = str(round(float(priceIns) -((0.030* float(priceIns))/20),6))
                            psf = str(round(float(priceIns) -((0.030* float(priceIns))/20),6))

                            psloss = str(round(float(priceIns) +((0.10*float(priceIns))/20),6))
                            pslostf = str(round(float(priceIns) +((0.25*float(priceIns))/20),6))
                            pslostl = str(round(float(priceIns) +((0.30*float(priceIns))/20),6))
                            if (mPrices <= Decimal(psloss)  and float(ass) <= 30):
                                ake_profit_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=psf, quantity=ass,timeInForce='GTC')
                                time.sleep(1)
                            elif (mPrices <= Decimal(psloss)  and float(ass) > 30):
