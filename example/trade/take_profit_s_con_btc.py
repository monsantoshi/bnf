    ...
    try:
        while True:
            ...
            for keys in object_list_open:
                if ((keys.get('positionSide') =='SHORT') and (keys.get('symbol') == "BTCUSD_PERP") and (keys.get('type')=="LIMIT")  and (keys.get('status')=="NEW") and (keys.get('side') == "BUY")):
...
###########################
            ...
            for key in object_list:
                ...
                if (key.get('positionSide') =='SHORT' and key.get('symbol') =='BTCUSD_PERP' and key.get('positionAmt') < 0.0):
                    syms = key.get('symbol')
                    ...
                    try:
                        ...
                        ps = str(round(float(priceIns) -((0.50* float(priceIns))/20),1))

                        psf = str(round(float(priceIns) -((0.02* float(priceIns))/20),1))
                        psff = str(round(float(priceIns) -((0.025* float(priceIns))/20),1))
                        psfff = str(round(float(priceIns) -((0.03* float(priceIns))/20),1))
                        psffff = str(round(float(priceIns) -((0.15* float(priceIns))/20),1))
                        psloss = str(round(float(priceIns) +((0.10*float(priceIns))/20),1))
                        pslostf = str(round(float(priceIns) +((0.15*float(priceIns))/20),1))
                        pslostl = str(round(float(priceIns) +((0.20*float(priceIns))/20),1))
                        ...
                        cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT,stopPrice=pslostf,  ordertype=OrderType.STOP_MARKET, closePosition='True')
                            ...
                            #cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=mPrices, quantity=ass//2,timeInForce='GTC')
                        #    cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET, quantity= str(round(float(ass),1)))
                        # elif (mPrices >= Decimal(pslostf)):
                        #     request_client = RequestClient(api_key=g_api_key, secret_key=g_secret_key)
                        ...
                        #             #print(market,'',markPrice,'',entryPriceLong,'',positionAmtLong)
                        #             al = str(round(float(positionAmtLong),1))
                        #             if (mPrices >= Decimal(pslostf) and float(ass) > float(al)*2):
                        #                 re_unit = ((float(ass))-float(al))
                        #                 if (re_unit > 0):
                        #                 #cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=mPrices, quantity=ass//2,timeInForce='GTC')
                        #                     cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET, quantity= str(round(float(re_unit),1))) 
                        if (mPrices <= Decimal(psloss)  and float(ass) >= 6):
                            ...
                            time.sleep(5)
                        elif (mPrices <= Decimal(psloss)  and float(ass) >=1 and  float(ass) < 6):
                            if float(conts) == 0:
                                ake_profit_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=psfff, quantity=ass,timeInForce='GTC')
                                #time.sleep(5)
                                conts= 0
                            if float(ass) > float(conts):
                                rest = float(ass) - float(conts)
                                ake_profit_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=psfff, quantity=str(rest),timeInForce='GTC')
                                #time.sleep(5)
                                conts = 0
                        elif (mPrices <= Decimal(psfff) and mPrices > Decimal(psffff)  and float(a) > 0 and float(ass) <1):
                            # ps = []
                            add_int_short_price =  round(float(priceIns)+ ((float(mPrices) - float(priceIns))/(float(priceIns)*100)*sec_p),1)
                            #print("in if")
                            #time.sleep(1)
                            add_short_price = 0
                            ...
                            #while True:
                                ...
                                for keyi in object_list_in:
                                    if (keyi.get('positionSide') =='SHORT' and keyi.get('symbol') =='BTCUSD_PERP' and keyi.get('positionAmt') < 0.0):
                                        ...
                                        add_short_p = round(add_short_price,1)
                                        ...
                                        if (float(ass) != float(short_amount)):
                                            stop_threads = True
                                        psin = str(round(float(priceIns) -((sec_p* float(priceIns))/20),1))
                                        ...
                                        if (float(dym_cur_short_price) < psl[-1]):
                                            ...
                                            psl = []
                                            stop_threads = True
                        elif (mPrices <= Decimal(psffff) and mPrices > Decimal(ps) and float(ass) <=2):
                            # ps = []
                            add_int_short_price =  round(float(priceIns)+ ((float(mPrices) - float(priceIns))/(float(priceIns)*100)*first_p),1)
                            #print("in if")
                            #time.sleep(1)
                            add_short_price = 0
                            psl.append(add_int_short_price)
                            ...
                            #while True:
                                ...
                                for keyi in object_list_in:
                                    if (keyi.get('positionSide') =='SHORT' and keyi.get('symbol') =='BTCUSD_PERP' and keyi.get('positionAmt') < 0.0):
                                        ...
                                        if (float(ass) != float(short_amount)):
                                            stop_threads = True
                                        psin = str(round(float(priceIns) -((first_p* float(priceIns))/20),1))
                                        # if (mPrices <= Decimal(psin) ):
                                        #     stop_threads = True
                                        if (float(dym_cur_short_price) < psl[-1]):
                                            if (add_short_p < psl[-1]):
