...
def check_buy_sell_signals(df):
    ...
    #         close_order_buy_coin_usdt(self,'NEOUSDT')  
    #         in_position = False
    #         message = f"CLOSE POSITION LONG NEOUSDT"
    ...
    # if (df['in_uptrendAll'][last_row_index] !=22):# 8 or df['in_uptrendAll'][last_row_index] == 1) :   
    ...
    #         #order = exchange.create_market_buy_order('ETH/USD', 0.05)
    #     #order = exchange.create_order(symbol='adausdt', side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=1)
    #         close_order_sell_coin_usdt(self,'NEOUSDT')  
    #         in_position = False
    #         message = f"CLOSE POSITION SHORT NEOUSDT"
...
def run_bot():
    ...
    # bars = exchange.fetch_ohlcv('NEOUSDT', timeframe='5m', limit=140)
    ...
    bars = request_client.get_candlestick_data(symbol="NEOUSDT", interval=CandlestickInterval.MIN3,startTime=None, endTime=None, limit=100)
