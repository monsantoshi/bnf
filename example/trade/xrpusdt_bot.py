    ...
    for key in object_list:
        if (key.get('positionSide') =='SHORT' and key.get('symbol') == markett and key.get('positionAmt') <= 0.0):
            ...
            if ((float(ass) <  20)):
                ...
                PrintBasic.print_obj(take_profit_coin)
            if ((float(ass) >=  20)  and float(ass) < 40 and float(mPrices) > float(plostsf) ):
...
def check_buy_sell_signals(df):
    ...
    # if (df['in_uptrendAll'][last_row_index] != 8):# 6 or df['in_uptrendAll'][last_row_index] == 4) :
    ...
    #         #order = exchange.create_market_buy_order('8/USD', 0.05)
    ...
    #         close_order_buy_coin_usdt(self,'XRPUSDT')  
    #         in_position = False
    #         message = f"CLOSE POSITION LONG XRPUSDT"
    ...
    #     #order = exchange.create_order(symbol='adausdt', side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=1)
    #         close_order_sell_coin_usdt(self,'XRPUSDT')  
    #         in_position = False
    #         message = f"CLOSE POSITION SHORT XRPUSDT"
...
def run_bot():
    ...
    # bars = exchange.fetch_ohlcv('XRPUSDT', timeframe='5m', limit=140)
    ...
    bars = request_client.get_candlestick_data(symbol="XRPUSDT", interval=CandlestickInterval.MIN3,startTime=None, endTime=None, limit=100)
