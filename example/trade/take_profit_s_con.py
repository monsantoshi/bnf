    ...
    try:
        while True:
            ...
            for keys in object_list_open:
                if ((keys.get('positionSide') =='SHORT') and (keys.get('side') == "BUY")):
                ...
                if (key.get('positionSide') =='SHORT' and key.get('positionAmt') < 0.0):
                    ...
                    try:
                        if  ((priceIns > 3000)):
                            ...
                            ps = str(round(float(priceIns) -((0.025* float(priceIns))/20),2))
                            psf = str(round(float(priceIns) -((0.025* float(priceIns))/20),2))
                            psloss = str(round(float(priceIns) +((0.10*float(priceIns))/20),2))
                            pslostf = str(round(float(priceIns) +((0.20*float(priceIns))/20),2))
                            pslostl = str(round(float(priceIns) +((0.20*float(priceIns))/20),2))
                            if (mPrices <= Decimal(psloss)  and float(ass) <= 30):
                                ...
                                time.sleep(1)
                            elif (mPrices <= Decimal(psloss)  and float(ass) > 30):
                                ...
                                for key in object_list:
                                    ...
                                    if (key.get('positionSide') =='LONG' and key.get('positionAmt') >=0.0 and key.get('symbol') == syms):
                                        ...
                                        #print(market,'',markPrice,'',entryPriceLong,'',positionAmtLong)
                                        al = str(round(float(positionAmtLong),3))
                                        if (mPrices >= Decimal(pslostf) and float(ass) > float(al)*2):
                                            re_unit = ((float(ass)//2)-float(al))
                                            if (re_unit > 0):
                                            #cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=mPrices, quantity=ass//2,timeInForce='GTC')
                                                cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET, quantity= str(round(float(re_unit),2))) 
   
                        elif  ((priceIns > 120) and (priceIns <=3000)):
                            ps = str(round(float(priceIns) -((0.025* float(priceIns))/20),3))
                            psf = str(round(float(priceIns) -((0.025* float(priceIns))/20),3))

                            psloss = str(round(float(priceIns) +((0.10*float(priceIns))/20),3))
                            pslostf = str(round(float(priceIns) +((0.20*float(priceIns))/20),3))
                            pslostl = str(round(float(priceIns) +((0.20*float(priceIns))/20),3))
                            if (mPrices <= Decimal(psloss)  and float(ass) <= 30):
                                ake_profit_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=psf, quantity=ass,timeInForce='GTC')
                                time.sleep(1)
                            elif (mPrices <= Decimal(psloss)  and float(ass) > 30):
                        ...
                        elif ((priceIns > 2) and (priceIns <=120)):
                            ps = str(round(float(priceIns) -((0.025* float(priceIns))/20),3))
                                    ...
                                    for keyi in object_list_in:
                                        if (keyi.get('positionSide') =='SHORT' and keyi.get('symbol') == 'BNBUSD_PERP' and keyi.get('positionAmt') < 0.0):
                                            ...
                                            if (float(dym_cur_short_price) < ps[-1]):
                                                ...
                                                print("last_short_TP_price",price)
                                                #cancel_order_all()
                                                cut_loss_long = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET, quantity=str(round(float(a),2)))
                                                PrintBasic.print_obj(cut_loss_long)
                                                time.sleep(1)
...
# elif (mPrices >= Decimal(pslostf)):
                            ...
                            psf = str(round(float(priceIns) -((0.025* float(priceIns))/20),5))

                            psloss = str(round(float(priceIns) +((0.10*float(priceIns))/20),5))
                            pslostf = str(round(float(priceIns) +((0.20*float(priceIns))/20),5))
                            pslostl = str(round(float(priceIns) +((0.20*float(priceIns))/20),5))
                            if (mPrices <= Decimal(psloss)  and float(ass) <= 30):
                                ake_profit_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=psf, quantity=ass,timeInForce='GTC')
                                time.sleep(1)
                            elif (mPrices <= Decimal(psloss)  and float(ass) > 30):
                        ...
                        #                         cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET, quantity= str(round(float(re_unit),2))) 
                            ...
                            ps = str(round(float(priceIns) -((0.025* float(priceIns))/20),6))
                            psf = str(round(float(priceIns) -((0.025* float(priceIns))/20),6))

                            psloss = str(round(float(priceIns) +((0.10*float(priceIns))/20),6))
                            pslostf = str(round(float(priceIns) +((0.20*float(priceIns))/20),6))
                            pslostl = str(round(float(priceIns) +((0.20*float(priceIns))/20),6))
                            if (mPrices <= Decimal(psloss)  and float(ass) <= 30):
                                ake_profit_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=psf, quantity=ass,timeInForce='GTC')
                                time.sleep(1)
                            elif (mPrices <= Decimal(psloss)  and float(ass) > 30):
