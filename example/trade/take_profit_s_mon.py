                ...
                if (key.get('positionSide') =='SHORT' and key.get('positionAmt') < 0.0):
                    ...
                    try:
                        if (priceIns > 120):
                            ...
                            ps = str(round(float(priceIns) -((0.065* float(priceIns))/20),2))
                            psloss = str(round(float(priceIns) +((0.20*float(priceIns))/20),2))
                            pslost = str(round(float(priceIns) +((1.50*float(priceIns))/20),2))
                            ...
                            if (mPrices >= Decimal(pslost) and float(ass)>= 0.1):
                            ...
                            elif (mPrices >= Decimal(pslost) and float(ass) >= 0.006 and float(ass) <= 0.008):
                                #     print("mPrice", mPrices)
                            #     print("psloss", psloss)
                                #cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=mPrices, quantity=ass//2,timeInForce='GTC')
                                cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET, quantity= str(round(float(ass)/4,2)))
                            elif (mPrices >= Decimal(pslost) and float(ass) >= 0.004 and float(ass) <= 0.006):
                                #     print("mPrice", mPrices)
                            #     print("psloss", psloss)
                                #cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=mPrices, quantity=ass//2,timeInForce='GTC')
                                cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET, quantity= str(round(float(ass)/3,2)))
                            elif (mPrices >= Decimal(pslost) and float(ass) > 0.002 and float(ass) <= 0.004):
                                #     print("mPrice", mPrices)
                            #     print("psloss", psloss)
                                #cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=mPrices, quantity=ass//2,timeInForce='GTC')
                                cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET, quantity= str(round(float(ass),2)))

                        elif ((priceIns > 2) and (priceIns <=120)):
                            ps = str(round(float(priceIns) -((0.065* float(priceIns))/20),3))
                            psloss = str(round(float(priceIns) +((0.20*float(priceIns))/20),3))
                            pslost = str(round(float(priceIns) +((1.50*float(priceIns))/20),3))
                            print("mPrice", mPrices)
                            print("ps",ps)
                            print("psloss", psloss)
                            print("pslost", pslost)
                            ...
                            if (mPrices <= Decimal(psloss)):
                                 ake_profit_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=ps, quantity=ass,timeInForce='GTC')
                                 time.sleep(1)
                            if (mPrices >= Decimal(pslost) and float(ass) >= 5):
                            #     print("mPrice", mPrices)
                            #     print("psloss", psloss)
                                #cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=mPrices, quantity=ass//2,timeInForce='GTC')
                                cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET, quantity= str(round(float(ass),2)))
                            # elif (mPrices >= Decimal(pslost) and float(ass) > 60 and float(ass) <= 80):
                            #     #     print("mPrice", mPrices)
                            # #     print("psloss", psloss)
                            #     #cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=mPrices, quantity=ass//2,timeInForce='GTC')
                            #     cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET, quantity= str(round(float(ass)//4,2)))
                            # elif (mPrices >= Decimal(pslost) and float(ass) > 40 and float(ass) <= 60):
                            #     #     print("mPrice", mPrices)
                            # #     print("psloss", psloss)
                            #     #cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=mPrices, quantity=ass//2,timeInForce='GTC')
                            #     cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET, quantity= str(round(float(ass)//3,2)))
                            # elif (mPrices >= Decimal(pslost) and float(ass) >= 20 and float(ass) <= 40):
                            #     #     print("mPrice", mPrices)
                            # #     print("psloss", psloss)
                            #     #cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=mPrices, quantity=ass//2,timeInForce='GTC')
                            #     cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET, quantity= str(round(float(ass),2)))

                            #ake_profit_short = request_client.post_order(symbol=syms, side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=ps, quantity=ass,timeInForce='GTC')
                        elif ((priceIns > 0.2) and (priceIns <=2)):
                            ps = str(round(float(priceIns) -((0.065* float(priceIns))/20),4))
                            psloss = str(round(float(priceIns) +((0.20*float(priceIns))/20),4))
                            pslost = str(round(float(priceIns) +((1.50*float(priceIns))/20),4))
                            if (mPrices <= Decimal(psloss)):
                                ake_profit_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=ps, quantity=ass,timeInForce='GTC')
                                time.sleep(1)
                            if (mPrices >= Decimal(pslost) and float(ass) >= 5):
