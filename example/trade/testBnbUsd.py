...
def run_bot():
    ...
    bars = exchange.fetch_ohlcv("BNBUSD_PERP", timeframe='5m', limit=50)
...
print(f"Fetching new bars for {datetime.now().isoformat()}")
bars = exchange.fetch_ohlcv("BNBUSD_PERP", timeframe='5m', limit=250)
#bars = sub_client.subscribe_candlestick_event("bnbusdt", CandlestickInterval.MIN30, callback, error)
df = pd.DataFrame(bars[:-1], columns=['datetime', 'open', 'high', 'low', 'close', 'volume'])
...
# indi_add = df.ta.tma()
#print(df)
#supertrend_add = df.ta.supertrend()
