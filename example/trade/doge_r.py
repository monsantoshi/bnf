...
def take_order_sell(self):
    ...
    count_s = doge_data["count_ss"]
    
    limit_s = doge_data["limit_ss"]
    count_l = doge_data["count_ll"]
    
    limit_l = doge_data["limit_ll"]
    reentryunit = str(round(float(doge_data["reentry_units"]),4))

    if ((short_amount_in == 0) and (count_l > limit_l) and (count_s > 3)):
...
def take_order_buy(self):
    ...
    if ((long_amount_in == 0) and (count_s > limit_s) and (count_l > 3)):
...
def inc_count_s(count):
    ...
    collection_doge.update_one({"market": 'DOGEUSDT'},{"$set":{"count_ss":count_ss} })
...
def cls_count_s(count):
    ...
    collection_doge = db["cut_loss"]
    doge_data = collection_doge.find_one({"market":"DOGEUSDT"})
    count_ll = doge_data["count_ll"]
    if (count_ll > 5):
        collection_doge.update_one({"market": 'DOGEUSDT'},{"$set":{"count_ss":count} })
def cls_count_ss(count):
    #cio_id = 'a8966001117907911480'
    #state = int(order_type)
    client = MongoClient(port=27017)
    db = client.binance
...
def inc_count_l(count):
    ...
    collection_doge.update_one({"market": 'DOGEUSDT'},{"$set":{"count_ll":count_ll} })
    #if (count_l > limit_l+1):
        #count_ss = count_s + count
        #collection_doge.update_one({"market": 'DOGEUSDT'},{"$set":{"count_ll":0} })
...
def cls_count_l(count):
    ...
    collection_doge = db["cut_loss"]
    doge_data = collection_doge.find_one({"market":"DOGEUSDT"})
    count_ss = doge_data["count_ss"]
    if (count_ss > 5):
        collection_doge.update_one({"market": 'DOGEUSDT'},{"$set":{"count_ll":count} })
    return
def cls_count_ll(count):
    #cio_id = 'a8966001117907911480'
    #state = int(order_type)
    client = MongoClient(port=27017)
    db = client.binance
...
##    step_seven_unit = doge_data["step_seven_unit"]
    ...
    reentry_unit = doge_data["reentry_units"]
    ...
    try:
        while True:
            ...
            if (long_amount_in > 0):
                ...
                plossfirst = str(round(float(object_list[long_idx]["entryPrice"])+((-1.00*(object_list[long_idx]["entryPrice"])/20)),4))
                ...
                if ((float(int_cur_long_price) <= float(plossfirst))):
