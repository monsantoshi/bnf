...
def take_order_sell(self):
    ...
    count_s = sxp_data["count_ss"]
    
    limit_s = sxp_data["limit_ss"]
    count_l = sxp_data["count_ll"]
    
    limit_l = sxp_data["limit_ll"]
    reentryunit = str(round(float(sxp_data["reentry_units"]),4))
...
def inc_count_s(count):
    ...
    collection_sxp.update_one({"market": 'SXPUSDT'},{"$set":{"count_ss":count_ss} })
...
def cls_count_s(count):
    ...
    collection_sxp = db["cut_loss"]
    sxp_data = collection_sxp.find_one({"market":"SXPUSDT"})
    count_ll = sxp_data["count_ll"]
    if (count_ll > 5):
        collection_sxp.update_one({"market": 'SXPUSDT'},{"$set":{"count_ss":count} })
...
def inc_count_l(count):
    ...
    collection_sxp.update_one({"market": 'SXPUSDT'},{"$set":{"count_ll":count_ll} })
    #if (count_l > limit_l+1):
        #count_ss = count_s + count
        #collection_sxp.update_one({"market": 'SXPUSDT'},{"$set":{"count_ll":0} })
...
def cls_count_l(count):
    ...
    collection_sxp = db["cut_loss"]
    sxp_data = collection_sxp.find_one({"market":"SXPUSDT"})
    count_ss = sxp_data["count_ss"]
    if (count_ss > 5):
        collection_sxp.update_one({"market": 'SXPUSDT'},{"$set":{"count_ll":count} })
    return
...
##    step_seven_unit = sxp_data["step_seven_unit"]
    ...
    reentry_unit = sxp_data["reentry_units"]
