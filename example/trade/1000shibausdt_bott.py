...
def check_buy_sell_signals(df):
    ...
    if not df['in_uptrend'][previous_row_index] and df['in_uptrend'][last_row_index]:
        ...
        if not in_position:
            ...
            close_order_sell_coin_usdt(self,'1000SHIBUSDT')
            order = take_order_buy_coin_adausdt(self,'1000SHIBUSDT',1000)
            print(order)
            in_position = True
            message = f"LONG 1000SHIBA"
    ...
    if df['in_uptrend'][previous_row_index] and not df['in_uptrend'][last_row_index]:
        if in_position:
            ...
            close_order_buy_coin_usdt(self,'1000SHIBUSDT')
            order = take_order_sell_coin_adausdt(self,'1000SHIBUSDT',1000)
            print(order)
            in_position = False
            message = f"SHORT 1000SHIBA"
...
def run_bot():
    ...
    bars = request_client.get_candlestick_data(symbol="1000SHIBUSDT", interval=CandlestickInterval.MIN3,startTime=None, endTime=None, limit=140)
