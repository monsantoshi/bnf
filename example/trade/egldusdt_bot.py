...
def close_order_sell_coin_usdt(self,markett):
    ...
    request_client = Client(api_key=config.BINANCE_API_KEY, api_secret=config.BINANCE_SECRET_KEY)

    result = request_client.futures_position_information()
    ...
    for key in object_list:
        if (key.get('positionSide') =='SHORT' and key.get('symbol') == markett and key.get('positionAmt') <= 0.0):
            ...
            if ((float(ass) <  0.1)):
                ...
                PrintBasic.print_obj(take_profit_coin)
            if ((float(ass) >= 0.1) and float(ass) < 2 and float(mPrices) > float(plostsf) ):
    ...
    for key in object_list:
        if (key.get('positionSide') =='LONG' and key.get('symbol') ==markett and key.get('positionAmt') >= 0.0):
            ...
            if ((float(a) < 0.1) ):
                ...
                PrintBasic.print_obj(take_profit_coin)
            if ((float(a) >= 0.1) and float(a) < 2 and float(mPrice)< float(plostf) ):
...
def check_buy_sell_signals(df):
    ...
    #         close_order_buy_coin_usdt(self,'EGLDUSDT')  
    #         in_position = False
    #         message = f"CLOSE POSITION LONG EGLDUSDT"
    ...
    #         #order = exchange.create_market_buy_order('ETH/USD', 0.05)
    #     #order = exchange.create_order(symbol='adausdt', side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=1)
    #         close_order_sell_coin_usdt(self,'EGLDUSDT')  
    #         in_position = False
    #         message = f"CLOSE POSITION SHORT EGLDUSDT"
...
def run_bot():
    ...
    # bars = exchange.fetch_ohlcv('EGLDUSDT', timeframe='5m', limit=140)
    ...
    bars = request_client.get_candlestick_data(symbol="EGLDUSDT", interval=CandlestickInterval.MIN3,startTime=None, endTime=None, limit=100)
