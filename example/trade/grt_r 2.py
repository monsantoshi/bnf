...
def take_order_sell(self):
    ...
    count_s = grt_data["count_ss"]
    
    limit_s = grt_data["limit_ss"]
    count_l = grt_data["count_ll"]
    
    limit_l = grt_data["limit_ll"]
    reentryunit = str(round(float(grt_data["reentry_units"]),4))
...
def inc_count_s(count):
    ...
    collection_grt.update_one({"market": 'GRTUSDT'},{"$set":{"count_ss":count_ss} })
...
def cls_count_s(count):
    ...
    collection_grt = db["cut_loss"]
    grt_data = collection_grt.find_one({"market":"GRTUSDT"})
    count_ll = grt_data["count_ll"]
    if (count_ll > 5):
        collection_grt.update_one({"market": 'GRTUSDT'},{"$set":{"count_ss":count} })
...
def inc_count_l(count):
    ...
    collection_grt.update_one({"market": 'GRTUSDT'},{"$set":{"count_ll":count_ll} })
    #if (count_l > limit_l+1):
        #count_ss = count_s + count
        #collection_grt.update_one({"market": 'GRTUSDT'},{"$set":{"count_ll":0} })
...
def cls_count_l(count):
    ...
    collection_grt = db["cut_loss"]
    grt_data = collection_grt.find_one({"market":"GRTUSDT"})
    count_ss = grt_data["count_ss"]
    if (count_ss > 5):
        collection_grt.update_one({"market": 'GRTUSDT'},{"$set":{"count_ll":count} })
    return
...
##    step_seven_unit = grt_data["step_seven_unit"]
    ...
    reentry_unit = grt_data["reentry_units"]
