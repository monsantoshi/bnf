...
class TryToTakeOrder:
    ...
    def take_order_point(self):
        ...
        result = request_client.get_order_book(symbol = "GRTUSDT", limit = 20)
        init_bq = []
        init_bp = []
        init_aq = []
        init_ap = []
        ...
        for i in range(20):
            result_xtz = request_client.get_order_book(symbol = "GRTUSDT", limit = 20)
            #time.sleep(1)
        ...
        if (price_bids[0]< price_bids[19]):
            ...
            l = 0
        #old mgrtod
        kline2 = request_client.get_candlestick_data(symbol="GRTUSDT", interval=CandlestickInterval.MIN3,startTime=None, endTime=None, limit=20)
        ...
        #grt_TrendDown = Bearish_Engulfing
        print("Down",Down)
        Up =  (float(haopen) < float(haclose)) and (l == 1)
        #grt_TrendUp =  Bullish_Engulfing
        print("Up",Up)
        result_p = request_client.get_position()
        #grt_account_trade = request_client.get_account_trades(symbol="GRTUSDT",limit= 1)
        #time.sleep(1)
        order_list = []
...
class CountCandle:
    ...
    def take_order_point(self):
        ...
        for i in range(20):
            result_grt = request_client.get_order_book(symbol = "GRTUSDT", limit = 20)
            #time.sleep(1)
            for resb in result_grt.bids:
                #print(resb.qty)
                bid_q.append(round(float(resb.qty),4))
                bid_p.append(round(float(resb.price),4))

               
            for resa in result_grt.asks:
                #print(resb.qty)
                ask_q.append(round(float(resa.qty),4))
                ask_p.append(round(float(resa.price),4))
        ...
        if (price_bids[0]< price_bids[19]):
            ...
            s = 1
        #old mgrtod
        kline2 = request_client.get_candlestick_data(symbol="GRTUSDT", interval=CandlestickInterval.MIN35,startTime=None, endTime=None, limit=10)
        #print("======= Kline/Candlestick Data =======")
        #PrintMix.print_data(result[0].volume)
        #print("======================================")
        open_list = []
        ...
        result_m = request_client.get_mark_price("GRTUSDT")
...
class CheckProfitLong:
    ...
    def check_profit_long_grt(self):
            #price_exit = 0
        global stop_threads
        if stop_threads:
            return
            ...
            if (((int_long_rate*100) >= min_profit_rate_long) and (long_amount >=  step_three_unit)):
                take_profit_grt = request_client.post_order(symbol="GRTUSDT", side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=long_amount)
                PrintBasic.print_obj(take_profit_grt)
                time.sleep(1)
            ...
            if (((int_long_rate*100) >= stop_profit_rate_long) and ((long_amount > step_two_unit) and (long_amount < step_three_unit))):
                ...
                while True:
                    ...
                    #grt_account_trade_in = request_client.get_account_trades(symbol="GRTUSDT",limit= 1)
                    #time.sleep(1)
                    object_list_in = []
                    ...
                    if (float(dym_cur_long_price) > pl[-1]):
                        ...
                        #cancel_order_all()
                        take_profit_grt = request_client_in.post_order(symbol="GRTUSDT", side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=long_amount)
                        PrintBasic.print_obj(take_profit_grt)
                        time.sleep(1)
...
class CheckProfitShort:
    ...
    def check_profit_short_grt(self):
            #price_exit = 0
        global stop_threads
        if stop_threads:
            return
            ...
            if (((int_short_rate*100) >= min_profit_rate_short) and (short_amount >= step_three_unit)):
                take_profit_grt = request_client.post_order(symbol="GRTUSDT", side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=short_amount)
                PrintBasic.print_obj(take_profit_grt)
                time.sleep(1)
            ...
            if (((int_short_rate*100) >= stop_profit_rate_short) and ((short_amount > step_two_unit) and  (short_amount < step_three_unit))):
                ...
                while True:
                    ...
                    if (float(dym_cur_short_price) < ps[-1]):
                        ...
                        #cancel_order_all()
                        take_profit_grt = request_client_in.post_order(symbol="GRTUSDT", side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=short_amount)
                        PrintBasic.print_obj(take_profit_grt)
                        time.sleep(1)
        ...
        if (in_short_out):
            ...
            if ((int_short_rate_out *100) >= stop_profit_rate_short):
                ...
                if ((short_amount_in == 0) and (short_amount_in < short_amount_out)):
                    take_profit_grt = request_client_in.post_order(symbol="GRTUSDT", side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=sale_unit)
                    PrintBasic.print_obj(take_profit_grt)
                    time.sleep(1)
...
##                    l = 0
##                #old mgrtod
##                kline2 = request_client.get_candlestick_data(symbol="GRTUSDT", interval=interval,startTime=None, endTime=None, limit=20)
##                #print("======= Kline/Candlestick Data =======")
##                #PrintMix.print_data(result[0].volume)
##                #print("======================================")
##                open_list = []
...
##                    take_profit_grt = request_client_in.post_order(symbol="GRTUSDT", side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=buy_unit*2)
##                    PrintBasic.print_obj(take_profit_grt)
##                    time.sleep(1)
##
##                if (in_long_in):
##                    if ((Up) and (long_amount_in < step_three_unit) and (long_amount_in < short_amount_out) and (short_amount_out < step_three_unit)):
##                        take_profit_grt = request_client_in.post_order(symbol="GRTUSDT", side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=buy_unit)
##                        PrintBasic.print_obj(take_profit_grt)
##                        time.sleep(1)
##                        return
##                    if ((Up) and (long_amount_in < step_three_unit) and (long_amount_in < short_amount_out) and (short_amount_out >= step_three_unit)):
...
##                    take_profit_grt = request_client_in.post_order(symbol="GRTUSDT", side=OrderSide.SELL, positionSide=PositionSide.SHORT,ordertype=OrderType.MARKET,  quantity=sale_unit*2)
##                    PrintBasic.print_obj(take_profit_grt)
##                    time.sleep(1)
##
##                if (in_short_in):
...
class CheckProfitLongMore:
    ...
    def check_profit_long_grt(self):
        ...
        if (((int_long_rate*100) >= stop_profit_rate_long_more) and ((long_amount >=  step_three_unit) and (long_amount < step_four_unit))):
            take_profit_grt = request_client.post_order(symbol="GRTUSDT", side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=buy_unit//2)
            PrintBasic.print_obj(take_profit_grt)
            time.sleep(1)
            stop_threads = True
            #subprocess.call([r'C:\Users\mon\Desktop\Batch_Re\grt_m2End.bat'])
...
class CheckProfitShortMore:
    ...
    def check_profit_short_grt(self):
        ...
        if (((int_short_rate*100) >= stop_profit_rate_short_more) and ((short_amount >= step_three_unit) and (short_amount < step_four_unit))):
            take_profit_grt = request_client.post_order(symbol="GRTUSDT", side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=sale_unit//2)
            PrintBasic.print_obj(take_profit_grt)
            time.sleep(1)
            stop_threads = True
            #subprocess.call([r'C:\Users\mon\Desktop\Batch_Re\grt_m2End.bat'])
        ...
        #print("long_price:  ",self.long_price +"   "+"int_long_price: ",self.int_cur_long_price +"   "+
         #     "int_long_rate: ",round((self.int_long_rate*100),4))
            ...
            if ((float(int_cur_long_price_in) <= float(plossfirst)) and (short_amount_in == 0)):
                ...
                take_profit_grt = request_client_in.post_order(symbol="GRTUSDT", side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=str(round(float(reentry_unit),4)))
                PrintBasic.print_obj(take_profit_grt)
                time.sleep(1)
        ...
        #print("long_price:  ",self.long_price +"   "+"int_long_price: ",self.int_cur_long_price +"   "+
         #     "int_long_rate: ",round((self.int_long_rate*100),4))
            ...
            if (((int_short_rate_in *100) <= -50) and (long_amount_in == 0)):
                ...
                take_profit_grt = request_client_in.post_order(symbol="GRTUSDT", side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=str(round(float(reentry_unit),4)))
...
def take_order_sell(self):
    ...
    db = client.binance
    collection_grt = db["cut_loss"]
 
    grt_data = collection_grt.find_one({"market": 'GRTUSDT'})
    sale_unit = grt_data["sale"]

    step_two_unit = grt_data["step_two_unit"]


    
    count_s = grt_data["count_s"]
    
    limit_s = grt_data["limit_s"]
    reentryunit = str(round(float(grt_data["reentry_unit"]),4))

    if ((short_amount_in == 0) and (short_amount_in <= step_two_unit) and (count_s > limit_s)):
        inc_inc_s(1)
        if ((inc_s % 7)==0):
            take_profit_grt = request_client.post_order(symbol="GRTUSDT", side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=str(round(float(reentryunit)//4,4)))
            PrintBasic.print_obj(take_profit_grt)
            time.sleep(1)
        else:
            take_profit_grt = request_client.post_order(symbol="GRTUSDT", side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=reentryunit)
            PrintBasic.print_obj(take_profit_grt)
            time.sleep(1)
        #inc_count_s(1)
        cls_count_s(0)
        ...
        inc_inc_s(1)
        take_profit_grt = request_client.post_order(symbol="GRTUSDT", side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=sale_unit)
        PrintBasic.print_obj(take_profit_grt)
        time.sleep(1)
        #inc_count_s(1)
        cls_count_s(0)
    ...
    #     take_profit_grt = request_client.post_order(symbol="GRTUSDT", side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=long_amount_in)
    #     PrintBasic.print_obj(take_profit_grt)
    #     time.sleep(1)
...
def take_order_buy(self):
    ...
    count_l = grt_data["count_l"]
    
    limit_l = grt_data["limit_l"]

    if ((long_amount_in == 0) and (long_amount_in <= step_two_unit) and (count_l > limit_l)):
        inc_inc_l(1)
        if ((inc_l % 7) == 0):
            take_profit_grt = request_client.post_order(symbol="GRTUSDT", side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=str(round(float(reentryunit)//4,4)))
            PrintBasic.print_obj(take_profit_grt)
            time.sleep(1)
        else:
                    
            take_profit_grt = request_client.post_order(symbol="GRTUSDT", side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=reentryunit)
            PrintBasic.print_obj(take_profit_grt)
            time.sleep(1)
        #inc_count_s(1)
        cls_count_l(0)
    ...
    #     cut_loss_grt = request_client.post_order(symbol="GRTUSDT", side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=short_amount_in)
    #     time.sleep(1)
    elif ((long_amount_in > 0) and (long_amount_in <= step_two_unit) and (count_l > limit_l)):
        inc_inc_l(1)
        take_profit_grt = request_client.post_order(symbol="GRTUSDT", side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=buy_unit)
        PrintBasic.print_obj(take_profit_grt)
        time.sleep(1)
        #inc_count_s(1)
        cls_count_l(0)
...
def inc_inc_l(count):
    ...
    #if (count_s <= limit_s):
    inc_ll = inc_l + count
    collection_grt.update_one({"market": 'GRTUSDT'},{"$set":{"inc_l":inc_ll} })
    #f (count_s > limit_s+1):
     #   #count_ss = count_s + count
     #   collection_grt.update_one({"market": 'GRTUSDT'},{"$set":{"count_s":0} })
...
def inc_inc_s(count):
    ...
    #if (count_s <= limit_s):
    inc_ss = inc_s + count
    collection_grt.update_one({"market": 'GRTUSDT'},{"$set":{"inc_s":inc_ss} })
    #f (count_s > limit_s+1):
     #   #count_ss = count_s + count
     #   collection_grt.update_one({"market": 'GRTUSDT'},{"$set":{"count_s":0} })
...
def inc_count_s(count):
    ...
    if (count_s <= limit_s):
        count_ss = count_s + count
        collection_grt.update_one({"market": 'GRTUSDT'},{"$set":{"count_s":count_ss} })
...
def cls_count_s(count):
    ...
    db = client.binance
    collection_grt = db["cut_loss"]
    collection_grt.update_one({"market": 'GRTUSDT'},{"$set":{"count_s":count} })
...
def inc_count_l(count):
    ...
    if (count_l <= limit_l):
        ...
        collection_grt.update_one({"market": 'GRTUSDT'},{"$set":{"count_l":count_ll} })
    if (count_l > limit_l+1):
        #count_ss = count_s + count
        collection_grt.update_one({"market": 'GRTUSDT'},{"$set":{"count_l":0} })
...
def cls_count_l(count):
    ...
    db = client.binance
    collection_grt = db["cut_loss"]
    collection_grt.update_one({"market": 'GRTUSDT'},{"$set":{"count_l":count} })
    return
...
##            take_profit_grt = request_client.post_order(symbol="GRTUSDT",newClientOrderId="long10000" , side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=reentry)
##            PrintBasic.print_obj(take_profit_grt)
##            time.sleep(5)
##            stop_threads = True
##            #subprocess.call([r'C:\Users\mon\Desktop\Batch_Re\grt_m2End.bat'])
...
##            take_profit_grt = request_client.post_order(symbol="GRTUSDT", side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=reentrys)
##            PrintBasic.print_obj(take_profit_grt)
##            time.sleep(1)
##            #stop_threads = True
##            #subprocess.call([r'C:\Users\mon\Desktop\Batch_Re\grt_m2End.bat'])
...
class TakeProfitDoge:
    ...
    def clean_open_order_grt(self):
        ...
        for key in object_list_open:
            if ((key.get('positionSide') =='LONG') and (key.get('side') == "SELL") and (key.get('symbol') == "GRTUSDT")):
        ...
        for keys in object_list_open:
            if ((keys.get('positionSide') =='SHORT') and (keys.get('side') == "BUY") and (keys.get('symbol') == "GRTUSDT")):
    ...
    def take_profit_long_grt(self):
        ...
        for key in object_list:
            if (key.get('positionSide') =='LONG' and (key.get('symbol') == "GRTUSDT") and key.get('positionAmt') >= 0.0):
        ...
        for keys in object_list:
            if (keys.get('positionSide') =='SHORT' and (keys.get('symbol') == "GRTUSDT") and keys.get('positionAmt') <= 0.0):
...
if __name__ == '__main__':
    ...
    index_data = []
    index_data = collection_test.find({"symbol": 'GRTUSDT'})
    ...
    stop_profit_rate_short = grt_data["mgain_profit_percent_s"]
    stop_profit_rate_long = grt_data["mgain_profit_percent"]
    stop_profit_rate_short_more = grt_data["mgain_profit_percent_s"]
    stop_profit_rate_long_more = grt_data["mgain_profit_percent"]
    min_profit_rate_short = grt_data["fgain_profit_percent_s"]
    min_profit_rate_long = grt_data["fgain_profit_percent"]
    step_two_profit_short = grt_data["lgain_profit_percent_s"]
    step_two_profit_long = grt_data["lgain_profit_percent"]
    stop_loss_rate_long = grt_data["loss_percent"]
    stop_loss_rate_short = grt_data["loss_percent_s"]
    buy_unit = grt_data["buy"]
    sale_unit = grt_data["sale"]
    step_one_unit = grt_data["step_one_unit"]
    step_two_unit = grt_data["step_two_unit"]
    step_three_unit = grt_data["step_three_unit"]
    step_four_unit = grt_data["step_four_unit"]
##    step_five_unit = grt_data["step_five_unit"]
##    step_six_unit = grt_data["step_six_unit"]
##    step_seven_unit = grt_data["step_seven_unit"]
    guided = grt_data["guided"]
    macd = grt_data["macd"]
    count_l = grt_data["count_l"]
    count_s = grt_data["count_s"]
    limit_l = grt_data["limit_l"]
    limit_s = grt_data["limit_s"]
    inc_l = grt_data["inc_l"]
    inc_s = grt_data["inc_s"]
    #interval = grt_data["interval"]
    reentry_unit = grt_data["reentry_unit"]
...
##                    #thread8 = Bin8(8, "Check Loss Long grt")
