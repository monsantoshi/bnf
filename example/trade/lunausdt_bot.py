...
def supertrend(df, period=7, atr_multiplier=3.5):
    ...
    adx = df.ta.adx(length=7)
    df = pd.concat([df,supertest['SUPERTd_7_3.5'],adx],axis=1)

    for current in range(1, len(df.index)):
        # if df['ADX_7'][current] >= 14: 
        #     if df['DMP_7'][current] > df['DMN_7'][current]: 
        #         df['in_uptrendADX'][current]=True        
        #     if df['DMP_7'][current] < df['DMN_7'][current]:
        #         df['in_uptrendADX'][current]=False

        if df['ADX_7'][current] >= 19: 
            if df['DMP_7'][current] > df['DMN_7'][current]: 
                df['in_uptrend'][current]=True        
            elif df['DMP_7'][current] < df['DMN_7'][current]:
...
def make_profit_long():
    ...
    for keys in object_list_open:
        #cont = 0
        if ((keys.get('positionSide') =='LONG') and (keys.get('symbol') == "LUNAUSDT")  and (keys.get('type')=="LIMIT") and (keys.get('status')=="NEW") and (keys.get('side') == "SELL")):
    ...
    for key in object_list:
        ...
        f.close()
        if (key.get('positionSide') =='LONG' and key.get('symbol') =='LUNAUSDT' and key.get('positionAmt') > 0.0):
            sym = key.get('symbol')
            mPrice = key.get('markPrice')
            priceIn = key.get('entryPrice')
            positionAmtLong = key.get('positionAmt')
            ...
            try:
                ...
                pf = str(round(float(priceIn) +((0.05* float(priceIn))/20),3))
                pff = str(round(float(priceIn) +((pff* float(priceIn))/20),3))
                puffs = str(round(float(priceIn) +((puffs* float(priceIn))/20),3))
...
def make_profit_short():
    ...
    for keys in object_list_open:
        if ((keys.get('positionSide') =='SHORT') and (keys.get('symbol') == "LUNAUSDT") and (keys.get('type')=="LIMIT") and (keys.get('status')=="NEW") and (keys.get('side') == "BUY")):
            markets = keys.get('symbol')
            orderIds = keys.get('orderId')
            conts = keys.get('origQty')
            print("market",markets)
...
###########################
    ...
    for key in object_list:
        ...
        f.close()
        if (key.get('positionSide') =='SHORT' and key.get('symbol') =='LUNAUSDT' and key.get('positionAmt') < 0.0):
            syms = key.get('symbol')
            mPrices = key.get('markPrice')
            priceIns = key.get('entryPrice')
            positionAmtShort = key.get('positionAmt')
            ...
            try:
                ...
                psf = str(round(float(priceIns) -((0.05* float(priceIns))/20),3))
...
def check_buy_sell_signals(df):
    ...
    if not df['in_uptrend'][previous_row_index] and df['in_uptrend'][last_row_index]:
        ...
        if not in_position:
            ...
            close_order_sell_coin_usdt(self,'LUNAUSDT')
            order = take_order_buy_coin_adausdt(self,'LUNAUSDT',1)
            print(order)
            in_position = True
            message = f"LONG LUNA"
    ...
    if df['in_uptrend'][previous_row_index] and not df['in_uptrend'][last_row_index]:
        if in_position:
            ...
            close_order_buy_coin_usdt(self,'LUNAUSDT')
            order = take_order_sell_coin_adausdt(self,'LUNAUSDT',1)
            print(order)
            in_position = False
            message = f"SHORT LUNA"
...
def run_bot():
    ...
    bars = exchange.fetch_ohlcv('LUNAUSDT', timeframe='5m', limit=50)
