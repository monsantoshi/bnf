...
def check_buy_sell_signals(df):
    ...
    #         in_position = False
    #         message = f"CLOSE POSITION LONG ADAUSDT"
    ...
    #         in_position = False
    #         message = f"CLOSE POSITION SHORT ADAUSDT"
    ...
    if df['in_uptrendAll'][last_row_index] == 8:
        ...
        if not in_position:
            ...
            #order = exchange.create_order(symbol='adausdt', side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=1)
                ...
                close_order_sell_coin_usdt(self,'ADAUSDT')
                order = take_order_buy_coin_adausdt(self,'ADAUSDT',20)
                print(order)
                in_position = True
    ...
    if df['in_uptrendAll'][last_row_index] == 22:
        if in_position:
            ...
            if (df['in_uptrendAll'][previous_row_index] != 22):
                ...
                close_order_buy_coin_usdt(self,'ADAUSDT')
                order = take_order_sell_coin_adausdt(self,'ADAUSDT',20)
                print(order)
                in_position = False
...
def run_bot():
    ...
    # bars = exchange.fetch_ohlcv('ADAUSDT', timeframe='5m', limit=140)
    ...
    bars = request_client.get_candlestick_data(symbol="ADAUSDT", interval=CandlestickInterval.MIN3,startTime=None, endTime=None, limit=100)
