...
def make_profit_long():
    ...
    for keys in object_list_open:
        #cont = 0
        if ((keys.get('positionSide') =='LONG') and (keys.get('symbol') == "KSMUSDT")  and (keys.get('type')=="LIMIT") and (keys.get('status')=="NEW") and (keys.get('side') == "SELL")):
    ...
    for key in object_list:
        ...
        f.close()
        if (key.get('positionSide') =='LONG' and key.get('symbol') =='KSMUSDT' and key.get('positionAmt') > 0.0):
            sym = key.get('symbol')
            mPrice = key.get('markPrice')
            priceIn = key.get('entryPrice')
            positionAmtLong = key.get('positionAmt')
            a = str(round(float(positionAmtLong),1))
            try:
...
def make_profit_short():
    ...
    for keys in object_list_open:
        if ((keys.get('positionSide') =='SHORT') and (keys.get('symbol') == "KSMUSDT") and (keys.get('type')=="LIMIT") and (keys.get('status')=="NEW") and (keys.get('side') == "BUY")):
            markets = keys.get('symbol')
            orderIds = keys.get('orderId')
            conts = keys.get('origQty')
            print("market",markets)
...
###########################
    ...
    for key in object_list:
        ...
        f.close()
        if (key.get('positionSide') =='SHORT' and key.get('symbol') =='KSMUSDT' and key.get('positionAmt') < 0.0):
            syms = key.get('symbol')
            mPrices = key.get('markPrice')
            priceIns = key.get('entryPrice')
            positionAmtShort = key.get('positionAmt')
...
def check_buy_sell_signals(df):
    ...
    if not df['in_uptrend'][previous_row_index] and df['in_uptrend'][last_row_index]:
        ...
        if not in_position:
            ...
            close_order_sell_coin_usdt(self,'KSMUSDT')
            order = take_order_buy_coin_adausdt(self,'KSMUSDT',0.1)
            print(order)
            in_position = True
            message = f"LONG KSM"
    ...
    if df['in_uptrend'][previous_row_index] and not df['in_uptrend'][last_row_index]:
        if in_position:
            ...
            close_order_buy_coin_usdt(self,'KSMUSDT')
            order = take_order_sell_coin_adausdt(self,'KSMUSDT',0.1)
            print(order)
            in_position = False
            message = f"SHORT KSM"
...
def run_bot():
    ...
    bars = exchange.fetch_ohlcv('KSMUSDT', timeframe='5m', limit=50)
