...
if __name__ == '__main__':
    g_api_key = 'uaEqPbv7c1PjQVn0F0P9cvCbZOMhUo1bs3HNj1zEmSxIMTexaBZhFSIjYehBTMOr'
    g_secret_key = 'Evum9nWi7PMPGbZAfYuY085As86Ek1RPnpHz1WuGf9optYiaITMgQlrdawJwGmoF'
            ...
            for key in object_list:
                if (key.get('positionSide') =='LONG' and key.get('positionAmt') > 0.0):
                    ...
                    try:
                        ...
                        if (priceIn > 120):
                            print("""""""""""""""""""""""")
                            p = str(round(float(priceIn) +((0.030* float(priceIn))/20),3))
                            pf = str(round(float(priceIn) +((0.030* float(priceIn))/20),3))
                            ploss =  str(round(float(priceIn) +((-0.10*float(priceIn))/20),3))
                            plostf =  str(round(float(priceIn) +((-0.25*float(priceIn))/20),3))
                            plostl =  str(round(float(priceIn) +((-0.30*float(priceIn))/20),3))
                            ...
                            p = str(round(float(priceIn) +((0.030* float(priceIn))/20),5))
                            pf = str(round(float(priceIn) +((0.030* float(priceIn))/20),5))
                            ploss =  str(round(float(priceIn) +((-0.10*float(priceIn))/20),5))
                            plostf =  str(round(float(priceIn) +((-0.25*float(priceIn))/20),5))
                            plostl =  str(round(float(priceIn) +((-0.30*float(priceIn))/20),5))
                            if (mPrice >= Decimal(ploss) and float(a) <=30 ):
                                take_profit_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.LIMIT, price=pf, quantity=a,timeInForce='GTC')
                                time.sleep(1)
                            elif (mPrice >= Decimal(ploss) and float(a) > 30 ):
                            ...
                            p = str(round(float(priceIn) +((0.030* float(priceIn))/20),6))
                            pf = str(round(float(priceIn) +((0.030* float(priceIn))/20),6))
                            ploss =  str(round(float(priceIn) +((-0.10*float(priceIn))/20),6))
                            plostf =  str(round(float(priceIn) +((-0.25*float(priceIn))/20),6))
                            plostl =  str(round(float(priceIn) +((-0.30*float(priceIn))/20),6))
                            if (mPrice >= Decimal(ploss) and float(a) <=30 ):
                                take_profit_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.LIMIT, price=pf, quantity=a,timeInForce='GTC')
                                time.sleep(1)
                            elif (mPrice >= Decimal(ploss) and float(a) > 30 ):
