    ...
    try:
        while True:
            ...
            for keys in object_list_open:
                if ((keys.get('positionSide') =='SHORT') and (keys.get('symbol') == "YFIUSDT") and (keys.get('type')=="LIMIT") and (keys.get('status')=="NEW") and (keys.get('side') == "BUY")):
...
###########################
            ...
            for key in object_list:
                ...
                f.close()
                if (key.get('positionSide') =='SHORT' and key.get('symbol') =='YFIUSDT' and key.get('positionAmt') < 0.0):
                    syms = key.get('symbol')
                    ...
                    try:
                        ...
                        ps = str(round(float(priceIns) -((ps* float(priceIns))/20),0))

                        psf = str(round(float(priceIns) -((psf* float(priceIns))/20),0))
                        psff = str(round(float(priceIns) -((psff* float(priceIns))/20),0))
                        psfff = str(round(float(priceIns) -((psfff* float(priceIns))/20),0))
                        psffff = str(round(float(priceIns) -((psffff* float(priceIns))/20),0))
                        psloss = str(round(float(priceIns) +((pslost*float(priceIns))/20),0))
                        pslostf = str(round(float(priceIns) +((pslostf*float(priceIns))/20),0))
                        pslostl = str(round(float(priceIns) +((pslostl*float(priceIns))/20),0))
                        try:
                        ...
                        #if (mPrices >= Decimal(pslostl)):
                            #cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=mPrices, quantity=ass//2,timeInForce='GTC')
                            #cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET, quantity= str(round(float(ass),0)))
                        ...
                        if (mPrices <= Decimal(psff) and mPrices > Decimal(psfff) ):
                            # ps = []
                            add_int_short_price =  round(float(priceIns)+ ((float(mPrices) - float(priceIns))/(float(priceIns)*100)*first_p),0)
                            #print("in if")
                            #time.sleep(1)
                            add_short_price = 0
                            ...
                            #while True:
                                ...
                                for keyi in object_list_in:
                                    if (keyi.get('positionSide') =='SHORT' and keyi.get('symbol') =='YFIUSDT' and keyi.get('positionAmt') < 0.0):
                                        ...
                                        add_short_p = round(add_short_price,0)
                                        ...
                                        if (float(ass) != float(short_amount) or mPrices < Decimal(psfff) ):
                                            stop_threads = True
                                        psin = str(round(float(priceIns) -((first_p* float(priceIns))/20),0))
                            ...
                            # ps = []
                            add_int_short_price =  round(float(priceIns)+ ((float(mPrices) - float(priceIns))/(float(priceIns)*100)*sec_p),0)
                            #print("in if")
                            #time.sleep(1)
                            add_short_price = 0
                            psl.append(add_int_short_price)
                                ...
                                for keyi in object_list_in:
                                    if (keyi.get('positionSide') =='SHORT' and keyi.get('symbol') =='YFIUSDT' and keyi.get('positionAmt') < 0.0):
                                        ...
                                        if (float(ass) != float(short_amount) or mPrices > Decimal(psffff) ):
                                            stop_threads = True
                                        psin = str(round(float(priceIns) -((sec_p* float(priceIns))/20),0))
                                        # if (mPrices <= Decimal(psin) ):
                                        #     stop_threads = True
                                        if (float(dym_cur_short_price) < psl[-1]):
                                            if (add_short_p < psl[-1]):
                            ...
                            # ps = []
                            add_int_short_price =  round(float(priceIns)+ ((float(mPrices) - float(priceIns))/(float(priceIns)*100)*thr_p),0)
                            #print("in if")
                            #time.sleep(1)
                            add_short_price = 0
                            psl.append(add_int_short_price)
                                ...
                                for keyi in object_list_in:
                                    if (keyi.get('positionSide') =='SHORT' and keyi.get('symbol') =='YFIUSDT' and keyi.get('positionAmt') < 0.0):
                                        ...
                                        if (float(ass) != float(short_amount)):
                                            stop_threads = True
                                        psin = str(round(float(priceIns) -((thr_p* float(priceIns))/20),0))
                                        # if (mPrices <= Decimal(psin) ):
                                        #     stop_threads = True
                                        if (float(dym_cur_short_price) < psl[-1]):
                                            if (add_short_p < psl[-1]):
