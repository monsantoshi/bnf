...
class TryToTakeOrder:
    ...
    def take_order_point(self):
        ...
        #profit_TrendDown = Bearish_Engulfing
        print("Down",Down)
        Up =  (float(haopen) < float(haclose)) and (l == 1)
        #profit_TrendUp =  Bullish_Engulfing
        print("Up",Up)
        result_p = request_client.get_position()
        #profit_account_trade = request_client.get_account_trades(symbol="ATOMUSDT",limit= 1)
        #time.sleep(1)
        order_list = []
...
class CheckProfitLong:
    ...
    def check_profit_long_profit(self):
            #price_exit = 0
        global stop_threads
        if stop_threads:
            return
            ...
            if (((int_long_rate*100) >= min_profit_rate_long) and (long_amount >=  step_three_unit)):
                take_profit_profit = request_client.post_order(symbol="ATOMUSDT", side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=long_amount)
                PrintBasic.print_obj(take_profit_profit)
                time.sleep(1)
            ...
            if (((int_long_rate*100) >= stop_profit_rate_long) and ((long_amount > step_two_unit) and (long_amount < step_three_unit))):
                ...
                while True:
                    ...
                    #profit_account_trade_in = request_client.get_account_trades(symbol="ATOMUSDT",limit= 1)
                    #time.sleep(1)
                    object_list_in = []
                    ...
                    if (float(dym_cur_long_price) > pl[-1]):
                        ...
                        #cancel_order_all()
                        take_profit_profit = request_client_in.post_order(symbol="ATOMUSDT", side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=long_amount)
                        PrintBasic.print_obj(take_profit_profit)
                        time.sleep(1)
...
class CheckProfitShort:
    ...
    def check_profit_short_profit(self):
            #price_exit = 0
        global stop_threads
        if stop_threads:
            return
            ...
            if (((int_short_rate*100) >= min_profit_rate_short) and (short_amount >= step_three_unit)):
                take_profit_profit = request_client.post_order(symbol="ATOMUSDT", side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=short_amount)
                PrintBasic.print_obj(take_profit_profit)
                time.sleep(1)
            ...
            if (((int_short_rate*100) >= stop_profit_rate_short) and ((short_amount > step_two_unit) and  (short_amount < step_three_unit))):
                ...
                while True:
                    ...
                    if (float(dym_cur_short_price) < ps[-1]):
                        ...
                        #cancel_order_all()
                        take_profit_profit = request_client_in.post_order(symbol="ATOMUSDT", side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=short_amount)
                        PrintBasic.print_obj(take_profit_profit)
                        time.sleep(1)
        ...
        if (in_short_out):
            ...
            if ((int_short_rate_out *100) >= stop_profit_rate_short):
                ...
                if ((short_amount_in == 0) and (short_amount_in < short_amount_out)):
                    take_profit_profit = request_client_in.post_order(symbol="ATOMUSDT", side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=sale_unit)
                    PrintBasic.print_obj(take_profit_profit)
                    time.sleep(1)
...
##                    take_profit_profit = request_client_in.post_order(symbol="ATOMUSDT", side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=buy_unit*2)
##                    PrintBasic.print_obj(take_profit_profit)
##                    time.sleep(1)
##
##                if (in_long_in):
##                    if ((Up) and (long_amount_in < step_three_unit) and (long_amount_in < short_amount_out) and (short_amount_out < step_three_unit)):
##                        take_profit_profit = request_client_in.post_order(symbol="ATOMUSDT", side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=buy_unit)
##                        PrintBasic.print_obj(take_profit_profit)
##                        time.sleep(1)
##                        return
##                    if ((Up) and (long_amount_in < step_three_unit) and (long_amount_in < short_amount_out) and (short_amount_out >= step_three_unit)):
...
##                    take_profit_profit = request_client_in.post_order(symbol="ATOMUSDT", side=OrderSide.SELL, positionSide=PositionSide.SHORT,ordertype=OrderType.MARKET,  quantity=sale_unit*2)
##                    PrintBasic.print_obj(take_profit_profit)
##                    time.sleep(1)
##
##                if (in_short_in):
...
class CheckLossLong:
    ...
    def check_loss_long_profit(self):
            #price_exit = 0
...
class CheckLossShort:
    ...
    def check_loss_short_profit(self):
...
class CheckProfitLongMore:
    ...
    def check_profit_long_profit(self):
        ...
        if (((int_long_rate*100) >= stop_profit_rate_long_more) and ((long_amount >=  step_three_unit) and (long_amount < step_four_unit))):
            take_profit_profit = request_client.post_order(symbol="ATOMUSDT", side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=buy_unit/2)
            PrintBasic.print_obj(take_profit_profit)
            time.sleep(1)
            stop_threads = True
            #subprocess.call([r'C:\Users\mon\Desktop\Batch_Re\profit_m2End.bat'])
        ...
        if (((int_long_rate*100) >= stop_profit_rate_long_more) and ((long_amount > step_one_unit) and (long_amount <= step_two_unit))):
            take_profit_profit = request_client.post_order(symbol="ATOMUSDT", side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=buy_unit/4)
            PrintBasic.print_obj(take_profit_profit)
            time.sleep(1)
            stop_threads = True
            #subprocess.call([r'C:\Users\mon\Desktop\Batch_Re\profit_m2End.bat'])
...
class CheckProfitShortMore:
    ...
    def check_profit_short_profit(self):
        ...
        if (((int_short_rate*100) >= stop_profit_rate_short_more) and ((short_amount >= step_three_unit) and (short_amount < step_four_unit))):
            take_profit_profit = request_client.post_order(symbol="ATOMUSDT", side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=sale_unit/2)
            PrintBasic.print_obj(take_profit_profit)
            time.sleep(1)
            stop_threads = True
            #subprocess.call([r'C:\Users\mon\Desktop\Batch_Re\profit_m2End.bat'])
        ...
        if (((int_short_rate*100) >= stop_profit_rate_short_more) and ((short_amount > step_one_unit) and  (short_amount <= step_two_unit))):
            take_profit_profit = request_client.post_order(symbol="ATOMUSDT", side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=sale_unit/4)
            PrintBasic.print_obj(take_profit_profit)
            time.sleep(1)
            stop_threads = True
            #subprocess.call([r'C:\Users\mon\Desktop\Batch_Re\profit_m2End.bat'])
        ...
        if (in_long_in):
            ...
            if ((int_long_rate_in *100) <= -1):
                ...
                if ((short_amount_in == 0) and (short_amount_in < long_amount_in)):
                    ...
                    take_profit_profit = request_client_in.post_order(symbol="ATOMUSDT", side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=reentry_unit)
                    PrintBasic.print_obj(take_profit_profit)
                    time.sleep(1)
        ...
        if (in_short_in):
            ...
            if ((int_short_rate_in *100) <= -1):
                ...
                if ((long_amount_in == 0) and (long_amount_in < short_amount_in)):
                    ...
                    take_profit_profit = request_client_in.post_order(symbol="ATOMUSDT", side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=reentry_unit)
        ...
        try:
            ...
            if email_message.is_multipart():
                ...
                # print('To:\t\t', email_message['To'])
                    ...
                    #rs1 = s1.find('profit3MSH')
                    #rl1 = s1.find('profit3MLO')
...
def take_order_sell(self):
    ...
    db = client.binance
    collection_profit = db["profit"]
    collection_core = db["base"]

    core_data = collection_core.find_one({"market": 'ATOM'})
    #print(core_data)
    profit_active_plan = core_data["ap"]
    #print(profit_active_plan)
    if (profit_active_plan == 'A'):
        profit_data = collection_profit.find_one({"market": 'ATOM_A'})
    if (profit_active_plan == 'B'):
        profit_data = collection_profit.find_one({"market": 'ATOM_B'})
    if (profit_active_plan == 'C'):
        profit_data = collection_profit.find_one({"market": 'ATOM_C'})     

    sale_unit = profit_data["sale"]

    step_two_unit = profit_data["step_two_unit"]


    
    count_s = profit_data["count_s"]
    
    limit_s = profit_data["limit_s"]


    if ((short_amount_in < step_two_unit) and (count_s > limit_s)):
        take_profit_profit = request_client.post_order(symbol="ATOMUSDT", side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=sale_unit)
        PrintBasic.print_obj(take_profit_profit)
        time.sleep(1)
...
def take_order_buy(self):
    ...
    buy_unit = profit_data["buy"]

    step_two_unit = profit_data["step_two_unit"]


    count_l = profit_data["count_l"]
    
    limit_l = profit_data["limit_l"]


    if ((long_amount_in < step_two_unit) and (count_l > limit_l)):
        take_profit_profit = request_client.post_order(symbol="ATOMUSDT", side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=buy_unit)
        time.sleep(1)
...
def inc_count_s(count):
    ...
    collection_profit.update_one({"market": 'ATOM_A'},{"$set":{"count_s":count_ss} })
    collection_profit.update_one({"market": 'ATOM_B'},{"$set":{"count_s":count_ss} })
    collection_profit.update_one({"market": 'ATOM_C'},{"$set":{"count_s":count_ss} })
    return
...
def cls_count_s(count):
    ...
    db = client.binance
    collection_profit = db["profit"]
    collection_profit.update_one({"market": 'ATOM_A'},{"$set":{"count_s":count} })
    collection_profit.update_one({"market": 'ATOM_B'},{"$set":{"count_s":count} })
    collection_profit.update_one({"market": 'ATOM_C'},{"$set":{"count_s":count} })
    return
...
def inc_count_l(count):
    ...
    collection_profit.update_one({"market": 'ATOM_A'},{"$set":{"count_l":count_ll} })
    collection_profit.update_one({"market": 'ATOM_B'},{"$set":{"count_l":count_ll} })
    collection_profit.update_one({"market": 'ATOM_C'},{"$set":{"count_l":count_ll} })
    return
...
def cls_count_l(count):
    ...
    db = client.binance
    collection_profit = db["profit"]
    collection_profit.update_one({"market": 'ATOM_A'},{"$set":{"count_l":count} })
    collection_profit.update_one({"market": 'ATOM_B'},{"$set":{"count_l":count} })
    collection_profit.update_one({"market": 'ATOM_C'},{"$set":{"count_l":count} })
    return
...
def insert_test(x):
    ...
    client = MongoClient(port=27017)
    db = client.binance
    collection_profit = db["test_mon_s"]
    #count_ss = count_s + count
    collection_profit.insert_one(x)
    ...
    try:
        while True:
            ...
            for keys in object_list_open:
                #cont = 0
                if ((keys.get('positionSide') =='LONG') and (keys.get('symbol') == "ATOMUSDT")  and (keys.get('type')=="LIMIT") and (keys.get('status')=="NEW") and (keys.get('side') == "SELL")):
...
###########################
            ...
            for key in object_list:
                ...
                f.close()
                if (key.get('positionSide') =='LONG' and key.get('symbol') =='ATOMUSDT' and key.get('positionAmt') > 0.0):
                    sym = key.get('symbol')
                    ...
                    try:
                        ...
                        p = str(round(float(priceIn) +((p* float(priceIn))/20),3))
                        pf = str(round(float(priceIn) +((pf* float(priceIn))/20),3))
                        pff = str(round(float(priceIn) +((pff* float(priceIn))/20),3))
                        pfff = str(round(float(priceIn) +((pfff* float(priceIn))/20),3))
                        pffff = str(round(float(priceIn) +((pffff* float(priceIn))/20),3))
                        ploss =  str(round(float(priceIn) +((plost*float(priceIn))/20),3))
                        plostf =  str(round(float(priceIn) +((plostf*float(priceIn))/20),3))
                        plostl =  str(round(float(priceIn) +((plostl*float(priceIn))/20),3))
                        try:
                        ...
                        if (mPrice <= Decimal(plostl)):
                            try:
                            #cut_loss_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.LIMIT, price=mPrice, quantity=a//2,timeInForce='GTC')
                                cut_loss_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET, quantity=a)
                            except IndexError as error:
                                print("Exiting: with error in try" + "\n")
                            except Exception as exception:
                                print("Exiting: Exception error in try" +  "\n")  

                        #cut_loss_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG,stopPrice=plostf, ordertype=OrderType.STOP_MARKET,closePosition='True')                        
                        #if (mPrice < Decimal(plostl) and float(a) > 0):
                        #if (float(a) > 0):
                            #cut_loss_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.LIMIT, price=mPrice, quantity=a//2,timeInForce='GTC')
                            #cut_loss_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET, quantity=a)
                            #cut_loss_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG,stopPrice=plostf, ordertype=OrderType.STOP_MARKET,closePosition='True')
                        if (mPrice >= Decimal(ploss) and float(a) > 0 ):
                            try:
                                if float(conts) == 0:
                                ...
                                if float(a) > float(conts):
                                    ...
                                    take_profit_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.LIMIT, price=pf, quantity=str(round(rest,0)),timeInForce='GTC')
                                    #time.sleep(1) 
                                    conts = 0
                            except IndexError as error:
                                print("Exiting: with error in try" + "\n")
                        ...
                        #             #time.sleep(1)
                        #             result_for_cancel_stop_market = request_client.cancel_order(symbol=markets, orderId=orderIds)      
                        #     result_s = request_client.get_position()
                        ...
                        #     for key in object_list:
                        #         if (key.get('positionSide') =='SHORT' and key.get('positionAmt') <= 0.0  and key.get('symbol') == sym):
                        #             syms = key.get('symbol')
                        #             mPrices = key.get('markPrice')
                        #             priceIns = key.get('entryPrice')
                        #             positionAmtShort = key.get('positionAmt')
                        #             #print(market,'',markPrice,'',entryPriceLong,'',positionAmtLong)
                        #             ass = str(float(positionAmtShort)*-1)
                        #             if (mPrice < Decimal(plostf) and float(a) > float(ass)):
                        #                 re_unit = ((float(a))-float(ass))
                        #                 if (re_unit > 0):
                        ...
                        #                     cut_loss_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET, quantity= str(float(re_unit)))
                        if (mPrice >= Decimal(pff) and  mPrice < Decimal(pfff) ):
                            #pl = []
                            add_int_long_price =   round(float(priceIn)+ ((float(mPrice) - float(priceIn))/(float(priceIn)*100)*first_p),3)
                            print("in if")
                            ...
                            print("pl [-1]",pl[-1])
                            stop_threads = False
                            while ((stop_threads == False) and  mPrice < Decimal(pfff)):
                                request_client = RequestClient(api_key=g_api_key, secret_key=g_secret_key)
                                ...
                                for res_in in result_in:
                                    ...
                                    #d['unRealizedProfit'] = res.unRealizedProfit
                                    object_list_in.append(d)
                                for key in object_list_in:
                                    if (key.get('positionSide') =='LONG' and key.get('symbol') =='ATOMUSDT' and key.get('positionAmt') > 0.0):
                                        sym = key.get('symbol')
                                        mPrice = key.get('markPrice')
                                        priceIn = key.get('entryPrice')
                                        positionAmtLong = key.get('positionAmt')
                                        dym_cur_long_price = float(mPrice)
                                        long_amount = float(positionAmtLong)
                                        #print("dc_price",dym_cur_long_price)
                                        add_long_price =  float(priceIn)+ float(priceIn)*((float(dym_cur_long_price) - float(priceIn))/(float(priceIn))*first_p)
                                        add_long_p = round(add_long_price,3)
                                        #pl.append(add_long_p)
                                        print("add_long_p" ,add_long_p)
                                        #time.sleep(1)
                                        print("pl [-1] in loop",pl)
                                        #time.sleep(1)
                                        last_p = pl[-1]
                                        if (float(a) != float(long_amount) or  mPrice > Decimal(pfff) ):
                                            stop_threads = True
                                        pin = str(round(float(priceIn) +((first_p* float(priceIn))/20),3))
                                        # if (mPrice >= Decimal(pin)):
                                        #     stop_threads = True
                                        if (float(dym_cur_long_price) > pl[-1]):
                                            if (add_long_p > pl[-1]):
                                            ...
                                            pricef = round(pl[-1],3)
                                            print("last_long_TP_price",pricef)
                                            #cancel_order_all()
                                            take_profit_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG,  ordertype=OrderType.MARKET, quantity=a)
                                            #take_profit_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.LIMIT, price=pricef, quantity=a,timeInForce='GTC')
                                            PrintBasic.print_obj(take_profit_long)
                                            #time.sleep(1)

                                            pl = []       
                                            stop_threads = True    
                        elif (mPrice >= Decimal(pfff) and  mPrice < Decimal(pffff) ):
                            #pl = []
                            add_int_long_price =   round(float(priceIn)+ ((float(mPrice) - float(priceIn))/(float(priceIn)*100)*sec_p),3)
                            print("in if")
                            ...
                            print("pl [-1]",pl[-1])
                            stop_threads = False
                            while ((stop_threads == False) and  mPrice < Decimal(pffff)):
                                request_client = RequestClient(api_key=g_api_key, secret_key=g_secret_key)
                                result_in = request_client.get_position()
                                #sxp_account_trade_in = request_client.get_account_trades(symbol="SXPUSDT",limit= 1)
                                time.sleep(1)
                                ...
                                for key in object_list_in:
                                    if (key.get('positionSide') =='LONG' and key.get('symbol') =='ATOMUSDT' and key.get('positionAmt') > 0.0):
                                        ...
                                        long_amount = float(positionAmtLong)
                                        #print("dc_price",dym_cur_long_price)
                                        add_long_price =  float(priceIn)+ float(priceIn)*((float(dym_cur_long_price) - float(priceIn))/(float(priceIn))*sec_p)
                                        add_long_p = round(add_long_price,3)
                                        #pl.append(add_long_p)
                                        print("add_long_p" ,add_long_p)
                                        #time.sleep(1)
                                        ...
                                        #time.sleep(1)
                                        last_p = pl[-1]
                                        if (float(a) != float(long_amount)):
                                            stop_threads = True
                                        pin = str(round(float(priceIn) +((sec_p* float(priceIn))/20),3))
                                        # if (mPrice >= Decimal(pin)):
                                        #     stop_threads = True
                                        if (float(dym_cur_long_price) > pl[-1]):
                                            if (add_long_p > pl[-1]):
                        ...
                        elif (mPrice >= Decimal(pffff)  and mPrice < Decimal(p) ):
                            #pl = []
                            add_int_long_price =   round(float(priceIn)+ ((float(mPrice) - float(priceIn))/(float(priceIn)*100)*thr_p),3)
                            print("in if")
                            ...
                            print("pl [-1]",pl[-1])
                            stop_threads = False
                            while ((stop_threads == False) and  mPrice < Decimal(p)):
                                request_client = RequestClient(api_key=g_api_key, secret_key=g_secret_key)
                                result_in = request_client.get_position()
                                #sxp_account_trade_in = request_client.get_account_trades(symbol="SXPUSDT",limit= 1)
                                time.sleep(1)
                                ...
                                for key in object_list_in:
                                    if (key.get('positionSide') =='LONG' and key.get('symbol') =='ATOMUSDT' and key.get('positionAmt') > 0.0):
                                        ...
                                        long_amount = float(positionAmtLong)
                                        #print("dc_price",dym_cur_long_price)
                                        add_long_price =  float(priceIn)+ float(priceIn)*((float(dym_cur_long_price) - float(priceIn))/(float(priceIn))*thr_p)
                                        add_long_p = round(add_long_price,3)
                                        #pl.append(add_long_p)
                                        print("add_long_p" ,add_long_p)
                                        #time.sleep(1)
                                        ...
                                        if (float(a) != float(long_amount)):
                                            stop_threads = True
                                        pin = str(round(float(priceIn) +((thr_p* float(priceIn))/20),3))
                                        # if (mPrice >= Decimal(pin)):
                                        #     stop_threads = True
                                        if (float(dym_cur_long_price) > pl[-1]):
                                            if (add_long_p > pl[-1]):
