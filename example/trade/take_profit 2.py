            ...
            for key in object_list:
                if (key.get('positionSide') =='LONG' and key.get('positionAmt') > 0.0):
                    ...
                    try:
                        ...
                        if (priceIn > 10000):
                            p = str(round(float(priceIn) +((0.06* float(priceIn))/20),1))
                            ploss =  str(round(float(priceIn) +((-0.20*float(priceIn))/20),1))
                            plostf =  str(round(float(priceIn) +((-0.45*float(priceIn))/20),1))
                            plostl =  str(round(float(priceIn) +((-1.00*float(priceIn))/20),1))
                            ...
                            #             re_unit = float(a) - ((float(a)-float(ass))/2)
                        ...
                        elif ((priceIn > 100) and (priceIn <=10000)):
                            p = str(round(float(priceIn) +((0.06* float(priceIn))/20),2))
                            ploss =  str(round(float(priceIn) +((-0.20*float(priceIn))/20),2))
                            plostf =  str(round(float(priceIn) +((-0.45*float(priceIn))/20),2))
                            plostl =  str(round(float(priceIn) +((-1.00*float(priceIn))/20),2))
                            if (mPrice >= Decimal(ploss)):
                                take_profit_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.LIMIT, price=p, quantity=a,timeInForce='GTC')
                                time.sleep(1) 
                            if (mPrice < Decimal(plostl)):
                        ...
                        elif ((priceIn > 2) and (priceIn <=100)):
                            p = str(round(float(priceIn) +((0.06* float(priceIn))/20),3))
                            ploss =  str(round(float(priceIn) +((-0.20*float(priceIn))/20),3))
                            plostf =  str(round(float(priceIn) +((-0.45*float(priceIn))/20),3))
                            plostl =  str(round(float(priceIn) +((-1.00*float(priceIn))/20),3))
                            if (mPrice >= Decimal(ploss)):
                                take_profit_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.LIMIT, price=p, quantity=a,timeInForce='GTC')
                                time.sleep(1) 
                            if (mPrice < Decimal(plostl)):
                            ...
                            p = str(round(float(priceIn) +((0.06* float(priceIn))/20),4))
                            ploss =  str(round(float(priceIn) +((-0.20*float(priceIn))/20),4))
                            plostf =  str(round(float(priceIn) +((-0.45*float(priceIn))/20),4))
                            plostl =  str(round(float(priceIn) +((-1.00*float(priceIn))/20),4))
                            if (mPrice >= Decimal(ploss)):
                                take_profit_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.LIMIT, price=p, quantity=a,timeInForce='GTC')
                                time.sleep(1) 
                            if (mPrice < Decimal(plostl)):
                            ...
                            p = str(round(float(priceIn) +((0.06* float(priceIn))/20),6))
                            ploss =  str(round(float(priceIn) +((-0.20*float(priceIn))/20),6))
                            plostf =  str(round(float(priceIn) +((-0.45*float(priceIn))/20),6))
                            plostl =  str(round(float(priceIn) +((-1.00*float(priceIn))/20),6))
                            if (mPrice >= Decimal(ploss)):
                                take_profit_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.LIMIT, price=p, quantity=a,timeInForce='GTC')
                                time.sleep(1) 
                            if (mPrice < Decimal(plostl)):
