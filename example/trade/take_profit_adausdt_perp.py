    ...
    try:
        while True:
            ...
            for keys in object_list_open:
                #cont = 0
                if ((keys.get('positionSide') =='LONG') and (keys.get('symbol') == "ADAUSDT")  and (keys.get('type')=="LIMIT") and (keys.get('status')=="NEW") and (keys.get('side') == "SELL")):
...
###########################
            ...
            for key in object_list:
                ...
                if (key.get('positionSide') =='LONG' and key.get('symbol') =='ADAUSDT' and key.get('positionAmt') > 0.0):
                    sym = key.get('symbol')
                    ...
                    try:
                        ...
                        pf = str(round(float(priceIn) +((pf* float(priceIn))/20),4))
                        pff = str(round(float(priceIn) +((pff* float(priceIn))/20),4))
                        pfff = str(round(float(priceIn) +((pfff* float(priceIn))/20),4))
                        ...
                        if (mPrice >= Decimal(pff) and  mPrice < Decimal(pfff) ):
                            #pl = []
                            add_int_long_price =   round(float(priceIn)+ ((float(mPrice) - float(priceIn))/(float(priceIn)*100)*first_p),4)
                            print("in if")
                                ...
                                for key in object_list_in:
                                    if (key.get('positionSide') =='LONG' and key.get('symbol') =='ADAUSDT' and key.get('positionAmt') > 0.0):
                                        ...
                                        if (float(a) != float(long_amount) or  mPrice > Decimal(pfff) ):
                                            stop_threads = True
                                        pin = str(round(float(priceIn) +((first_p* float(priceIn))/20),4))
                            ...
                            #pl = []
                            add_int_long_price =   round(float(priceIn)+ ((float(mPrice) - float(priceIn))/(float(priceIn)*100)*sec_p),4)
                            print("in if")
                                ...
                                for key in object_list_in:
                                    if (key.get('positionSide') =='LONG' and key.get('symbol') =='ADAUSDT' and key.get('positionAmt') > 0.0):
                                        ...
                                        if (float(a) != float(long_amount) or mPrice > Decimal(pffff)):
                                            stop_threads = True
                                        pin = str(round(float(priceIn) +((sec_p* float(priceIn))/20),4))
                                        # if (mPrice >= Decimal(pin)):
                                        #     stop_threads = True
                                        if (float(dym_cur_long_price) > pl[-1]):
                                            if (add_long_p > pl[-1]):
                            ...
                            #pl = []
                            add_int_long_price =   round(float(priceIn)+ ((float(mPrice) - float(priceIn))/(float(priceIn)*100)*thr_p),4)
                            print("in if")
                                ...
                                for key in object_list_in:
                                    if (key.get('positionSide') =='LONG' and key.get('symbol') =='ADAUSDT' and key.get('positionAmt') > 0.0):
                                        ...
                                        if (float(a) != float(long_amount)):
                                            stop_threads = True
                                        pin = str(round(float(priceIn) +((thr_p* float(priceIn))/20),4))
                                        # if (mPrice >= Decimal(pin)):
                                        #     stop_threads = True
                                        if (float(dym_cur_long_price) > pl[-1]):
                                            if (add_long_p > pl[-1]):
