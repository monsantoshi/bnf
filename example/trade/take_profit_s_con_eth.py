    ...
    try:
        while True:
            ...
            for keys in object_list_open:
                if ((keys.get('positionSide') =='SHORT') and (keys.get('symbol') == "ETHUSD_PERP")  and (keys.get('type')=="LIMIT") and (keys.get('status')=="NEW") and (keys.get('side') == "BUY")):
...
###########################
            ...
            for key in object_list:
                ...
                if (key.get('positionSide') =='SHORT' and key.get('symbol') =='ETHUSD_PERP' and key.get('positionAmt') < 0.0):
                    syms = key.get('symbol')
                   ...
                   # pres = (d.as_tuple().exponent)*-1
                    ...
                    try:
                        ...
                        ps = str(round(float(priceIns) -((3.00* float(priceIns))/20),2))

                        psf = str(round(float(priceIns) -((0.10* float(priceIns))/20),2))
                        psff = str(round(float(priceIns) -((0.15* float(priceIns))/20),2))
                        psfff = str(round(float(priceIns) -((0.20* float(priceIns))/20),2))
                        psffff = str(round(float(priceIns) -((1.00* float(priceIns))/20),2))
                        psloss = str(round(float(priceIns) +((0.20*float(priceIns))/20),2))
                        pslostf = str(round(float(priceIns) +((0.20*float(priceIns))/20),2))
                        pslostl = str(round(float(priceIns) +((0.50*float(priceIns))/20),2))
                        ...
                        #             #print(market,'',markPrice,'',entryPriceLong,'',positionAmtLong)
                        #             al = str(round(float(positionAmtLong),2))
                        ...
                        if (mPrices <= Decimal(psloss)  and float(ass) >= 90):
                            ...
                            time.sleep(5)
                        elif (mPrices <= Decimal(psloss)  and float(ass) >= 2 and  float(ass) < 90):
                            if float(conts) == 0:
                            ...
                            if float(ass) > float(conts):
                                ...
                                #time.sleep(5)
                                conts = 0
                        elif (mPrices <= Decimal(psfff) and mPrices > Decimal(psffff) and float(ass) < 2):
                            # ps = []
                            add_int_short_price =  round(float(priceIns)+ ((float(mPrices) - float(priceIns))/(float(priceIns)*100)*0.50),2)
                            #print("in if")
                            #time.sleep(1)
                            add_short_price = 0
                            ...
                            #while True:
                                ...
                                for keyi in object_list_in:
                                    if (keyi.get('positionSide') =='SHORT' and keyi.get('symbol') =='ETHUSD_PERP' and keyi.get('positionAmt') < 0.0):
                                        ...
                                        #print("dc_price",dym_cur_long_price)
                                        add_short_price =  float(priceIns)+ float(priceIns)*((float(dym_cur_short_price) - float(priceIns))/(float(priceIns))*0.50)
                                        ...
                                        if (float(ass) != float(short_amount)):
                                            stop_threads = True
                                        psin = str(round(float(priceIns) -((0.50* float(priceIns))/20),2))
                                        ...
                                        if (float(dym_cur_short_price) < psl[-1]):
                                            ...
                                            psl = []
                                            stop_threads = True
                        elif (mPrices <= Decimal(psffff) and mPrices > Decimal(ps) and float(ass) <=20):
                            # ps = []
