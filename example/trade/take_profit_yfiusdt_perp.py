    ...
    try:
        while True:
            ...
            for keys in object_list_open:
                #cont = 0
                if ((keys.get('positionSide') =='LONG') and (keys.get('symbol') == "YFIUSDT")  and (keys.get('type')=="LIMIT") and (keys.get('status')=="NEW") and (keys.get('side') == "SELL")):
...
###########################
            ...
            for key in object_list:
                ...
                f.close()
                if (key.get('positionSide') =='LONG' and key.get('symbol') =='YFIUSDT' and key.get('positionAmt') > 0.0):
                    sym = key.get('symbol')
                    ...
                    try:
                        ...
                        p = str(round(float(priceIn) +((p* float(priceIn))/20),0))
                        pf = str(round(float(priceIn) +((pf* float(priceIn))/20),0))
                        pff = str(round(float(priceIn) +((pff* float(priceIn))/20),0))
                        pfff = str(round(float(priceIn) +((pfff* float(priceIn))/20),0))
                        pffff = str(round(float(priceIn) +((pffff* float(priceIn))/20),0))
                        ploss =  str(round(float(priceIn) +((plost*float(priceIn))/20),0))
                        plostf =  str(round(float(priceIn) +((plostf*float(priceIn))/20),0))
                        plostl =  str(round(float(priceIn) +((plostl*float(priceIn))/20),0))
                        try:
                        ...
                        if (mPrice >= Decimal(pff) and  mPrice < Decimal(pfff) ):
                            #pl = []
                            add_int_long_price =   round(float(priceIn)+ ((float(mPrice) - float(priceIn))/(float(priceIn)*100)*first_p),0)
                            print("in if")
                            ...
                            while ((stop_threads == False) and  mPrice < Decimal(pfff)):
                                ...
                                for key in object_list_in:
                                    if (key.get('positionSide') =='LONG' and key.get('symbol') =='YFIUSDT' and key.get('positionAmt') > 0.0):
                                        ...
                                        add_long_p = round(add_long_price,0)
                                        #pl.append(add_long_p)
                                        print("add_long_p" ,add_long_p)
                                        #time.sleep(1)
                                        print("pl [-1] in loop",pl)
                                        ...
                                        if (float(a) != float(long_amount) or  mPrice > Decimal(pfff) ):
                                            stop_threads = True
                                        pin = str(round(float(priceIn) +((first_p* float(priceIn))/20),0))
                            ...
                            #pl = []
                            add_int_long_price =   round(float(priceIn)+ ((float(mPrice) - float(priceIn))/(float(priceIn)*100)*sec_p),0)
                            print("in if")
                                ...
                                for key in object_list_in:
                                    if (key.get('positionSide') =='LONG' and key.get('symbol') =='YFIUSDT' and key.get('positionAmt') > 0.0):
                                        ...
                                        if (float(a) != float(long_amount)):
                                            stop_threads = True
                                        pin = str(round(float(priceIn) +((sec_p* float(priceIn))/20),0))
                                        # if (mPrice >= Decimal(pin)):
                                        #     stop_threads = True
                                        if (float(dym_cur_long_price) > pl[-1]):
                                            if (add_long_p > pl[-1]):
                            ...
                            #pl = []
                            add_int_long_price =   round(float(priceIn)+ ((float(mPrice) - float(priceIn))/(float(priceIn)*100)*thr_p),0)
                            print("in if")
                            ...
                            while ((stop_threads == False) and  mPrice < Decimal(p)):
                                ...
                                for key in object_list_in:
                                    if (key.get('positionSide') =='LONG' and key.get('symbol') =='YFIUSDT' and key.get('positionAmt') > 0.0):
                                        ...
                                        if (float(a) != float(long_amount)):
                                            stop_threads = True
                                        pin = str(round(float(priceIn) +((thr_p* float(priceIn))/20),0))
                                        # if (mPrice >= Decimal(pin)):
                                        #     stop_threads = True
                                        if (float(dym_cur_long_price) > pl[-1]):
                                            if (add_long_p > pl[-1]):
