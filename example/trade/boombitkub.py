...
def boom_position():
    ...
    #print(data)
    data = data['result']
    market_list = ['BTC','XRP','ETH','BNB','AAVE','ADA','ALPHA','AXS','BAL','BAND','CRV','CVC','IOST','LINK','MANA','NEAR','OMG','UNI','WAN','XLM','ZIL']
    try:
        for ma in market_list:
            unit_avl = data[ma]['available']+data[ma]['reserved']
            if (unit_avl) > 0:
                print(ma,' ',data[ma])
                symbollic = 'THB_'+ma
                data_a = {
                ...
                signature = sign(data_a)
                data_a['sig'] = signature

                print('Payload with signature: ' + json_encode(data_a))
                response = requests.post(API_HOST + '/api/market/my-open-orders', headers=header, data=json_encode(data_a))
                ...
                for d in data_b: 
                    ...
                    signature = sign(data_cancel)
                    data_cancel['sig'] = signature
                    print('Payload with signature: ' + json_encode(data_cancel))
                    response = requests.post(API_HOST + '/api/market/cancel-order', headers=header, data=json_encode(data_cancel))
                    time.sleep(2)
                order_data = {
                    'sym': symbollic,
                    'amt': unit_avl,
                    'rat':0,# amount you want to sell
                    'typ': 'market',
                    'ts': ts,
                }
                order_signature = sign(order_data)
...
def run_bot():
    ...
    # bars = exchange.fetch_ohlcv('WAN/USDT', timeframe='5m', limit=140)
    # print("THB_WAN")
