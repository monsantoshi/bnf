...
class TryToTakeOrder:
    ...
    def take_order_point(self):
        ...
        for resb in result.bids:
            init_bq.append(round(float(resb.qty),6))
            init_bp.append(round(float(resb.price),6))
        for resa in result.asks:
            init_aq.append(round(float(resa.qty),6))
            init_ap.append(round(float(resa.price),6))
        ...
        for i in range(20):
            ...
            for resb in result_xtz.bids:
                #print(resb.qty)
                bid_q.append(round(float(resb.qty),6))
                bid_p.append(round(float(resb.price),6))

               
            for resa in result_xtz.asks:
                #print(resb.qty)
                ask_q.append(round(float(resa.qty),6))
                ask_p.append(round(float(resa.price),6))
    ...
    def take_order_point_long(self):
        ...
        for res in result.bids:
            init_bq.append(round(float(res.qty),6))
            init_bp.append(round(float(res.price),6))
        for res in result.asks:
            init_aq.append(round(float(res.qty),6))
            init_ap.append(round(float(res.price),6))
        index_init_bq = init_bq.index(max(init_bq))
        init_price_bid = init_bp[index_init_bq]
        #print("init_price_bid" ,init_price_bid)
        index_init_aq = init_aq.index(max(init_aq))
    ...
    def check_profit_long_atom(self):
        ...
        if stop_threads:
            ...
            #print("long_price:  ",self.long_price +"   "+"int_long_price: ",self.int_cur_long_price +"   "+
             #     "int_long_rate: ",round((self.int_long_rate*100),6))
            print("int_long_rate : ",round((int_long_rate*100),6))
            ...
            if (((int_long_rate*100) >= stop_profit_rate_long) and ((long_amount > step_two_unit) and (long_amount < step_three_unit))):
                add_int_long_price =  round(float(long_price)+ ((float(int_cur_long_price) - float(long_price))/(float(long_price)*100)*protect_profit),6)
                #print("in if")
                #time.sleep(1)
                add_long_price = 0
                ...
                while True:
                    ...
                    add_long_p = round(add_long_price,6)
...
class CheckProfitShort:
    ...
    def check_profit_short_atom(self):
        ...
        if stop_threads:
            ...
            #print("long_price:  ",self.long_price +"   "+"int_long_price: ",self.int_cur_long_price +"   "+
             #     "int_long_rate: ",round((self.int_long_rate*100),6))
            print("int_short_rate : ",round((int_short_rate*100),6))
            ...
            if (((int_short_rate*100) >= stop_profit_rate_short) and ((short_amount > step_two_unit) and  (short_amount < step_three_unit))):
                add_int_short_price =  round(float(short_price)+ ((float(int_cur_short_price) - float(short_price))/(float(short_price)*100)*protect_profit_s),6)
                #print("in if")
                #time.sleep(1)
                add_short_price = 0
                ...
                while True:
                    ...
                    add_short_p = round(add_short_price,6)
                    #pl.append(add_long_p)
...
class CheckProfitLongFromOut:
    ...
    def check_profit_long_atom(self):
        ...
        if (in_short_out):
            ...
            #print("long_price:  ",self.long_price +"   "+"int_long_price: ",self.int_cur_long_price +"   "+
             #     "int_long_rate: ",round((self.int_long_rate*100),6))
            print("int_short_rate_out : ",round((int_short_rate_out*100),6))
...
class CheckProfitShortFromOut:
    ...
    def check_profit_short_atom(self):
        ...
        if (in_long_out):
            ...
            #print("long_price:  ",self.long_price +"   "+"int_long_price: ",self.int_cur_long_price +"   "+
             #     "int_long_rate: ",round((self.int_long_rate*100),6))
            print("int_long_rate_out : ",round((int_long_rate_out*100),6))
...
class CheckLossLongFromIn:
    ...
    def check_profit_long_atom(self):
        ...
        if (in_long_in):
            ...
            #print("long_price:  ",self.long_price +"   "+"int_long_price: ",self.int_cur_long_price +"   "+
             #     "int_long_rate: ",round((self.int_long_rate*100),6))
            print("int_long_rate_in : ",round((int_long_rate_in*100),6))
...
class CheckLossShortFromIn:
    ...
    def check_profit_short_atom(self):
        ...
        if (in_short_in):
            ...
            #print("long_price:  ",self.long_price +"   "+"int_long_price: ",self.int_cur_long_price +"   "+
             #     "int_long_rate: ",round((self.int_long_rate*100),6))
            print("int_short_rate_in : ",round((int_short_rate_in*100),6))
...
if __name__ == '__main__':
    ...
    try:
        while True:
            ...
            #     if ((key.get('positionSide') =='LONG') and key.get('symbol') =='DOGEUSD_PERP' and (key.get('side') == "SELL")):
            ...
            for keys in object_list_open:
                #cont = 0
                if ((keys.get('positionSide') =='LONG') and (keys.get('symbol') == "DOGEUSD_PERP")  and (keys.get('type')=="LIMIT") and (keys.get('status')=="NEW") and (keys.get('side') == "SELL")):
            ...
            for keys in object_list_open:
                #cont = 0
                if ((keys.get('positionSide') =='LONG') and (keys.get('symbol') == "DOGEUSD_PERP")  and (keys.get('type')=="STOP_MARKET") and (keys.get('status')=="NEW") and (keys.get('side') == "SELL")):
                    markets = keys.get('symbol')
                    orderIds = keys.get('orderId')
                    ...
                    #time.sleep(1)
                    result_for_cancel_stop_market = request_client.cancel_order(symbol="DOGEUSD_PERP", orderId=orderIds)    
            ...
            for key in object_list:
                ...
                f.close()
                if (key.get('positionSide') =='LONG' and key.get('symbol') =='DOGEUSD_PERP' and key.get('positionAmt') > 0.0):
                    sym = key.get('symbol')
                    ...
                    try:
                        pl=[]
                        #if ((sym != 'BTCUSDT') and (sym != 'ETHUSDT')):
                        p = str(round(float(priceIn) +((p* float(priceIn))/25),6))
                        pf = str(round(float(priceIn) +((pf* float(priceIn))/25),6))
                        pff = str(round(float(priceIn) +((pff* float(priceIn))/25),6))
                        pfff = str(round(float(priceIn) +((pfff* float(priceIn))/25),6))
                        pffff = str(round(float(priceIn) +((pffff* float(priceIn))/25),6))
                        ploss =  str(round(float(priceIn) +((plost*float(priceIn))/25),6))
                        plostf =  str(round(float(priceIn) +((plostf*float(priceIn))/25),6))
                        plostl =  str(round(float(priceIn) +((plostl*float(priceIn))/25),6))
                        try:
                        ...
                        if (mPrice >= Decimal(ploss) and float(a) > 0 ):
                            try:
                                ...
                                if float(a) > float(conts):
                                    ...
                                    take_profit_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.LIMIT, price=pf, quantity=str(rest),timeInForce='GTC')
                                    #time.sleep(1) 
                                    conts = 0
                            except IndexError as error:
                                print("Exiting: with error in try" + "\n")
                        ...
                        #     for keys in object_list_open:
                        #         #cont = 0
                        #         if ((keys.get('positionSide') =='SHORT') and (keys.get('symbol') == "DOGEUSD_PERP")  and (keys.get('type')=="STOP_MARKET") and (keys.get('status')=="NEW") and (keys.get('side') == "BUY")):
                        #             markets = keys.get('symbol')
                        #             orderIds = keys.get('orderId')
                        ...
                        if (mPrice >= Decimal(pff) and  mPrice < Decimal(pfff) ):
                            #pl = []
                            add_int_long_price =   round(float(priceIn)+ ((float(mPrice) - float(priceIn))/(float(priceIn)*100)*first_p),6)
                            print("in if")
                                ...
                                for key in object_list_in:
                                    if (key.get('positionSide') =='LONG' and key.get('symbol') =='DOGEUSD_PERP' and key.get('positionAmt') > 0.0):
                                        ...
                                        if (float(a) != float(long_amount) or  mPrice > Decimal(pfff) ):
                                            stop_threads = True
                                        pin = str(round(float(priceIn) +((first_p* float(priceIn))/25),6))
                                        ...
                                        if (float(dym_cur_long_price) > pl[-1]):
                                            ...
                                            pl = []       
                                            stop_threads = True    
                        if (mPrice >= Decimal(pfff) and  mPrice < Decimal(pffff) ):
                            #pl = []
                            add_int_long_price =   round(float(priceIn)+ ((float(mPrice) - float(priceIn))/(float(priceIn)*100)*sec_p),6)
                            print("in if")
                            ...
                            while ((stop_threads == False) and  mPrice < Decimal(pffff)):
                                ...
                                for key in object_list_in:
                                    if (key.get('positionSide') =='LONG' and key.get('symbol') =='DOGEUSD_PERP' and key.get('positionAmt') > 0.0):
                                        ...
                                        #time.sleep(1)
                                        last_p = pl[-1]
                                        if (float(a) != float(long_amount) or mPrice > Decimal(pffff)):
                                            stop_threads = True
                                        pin = str(round(float(priceIn) +((sec_p* float(priceIn))/25),6))
                                        # if (mPrice >= Decimal(pin)):
                                        #     stop_threads = True
                                        if (float(dym_cur_long_price) > pl[-1]):
                                            if (add_long_p > pl[-1]):
                        ...
                        if (mPrice >= Decimal(pffff)  and mPrice < Decimal(p)  ):
                            #pl = []
                            add_int_long_price =   round(float(priceIn)+ ((float(mPrice) - float(priceIn))/(float(priceIn)*100)*thr_p),6)
                            print("in if")
                                ...
                                for key in object_list_in:
                                    if (key.get('positionSide') =='LONG' and key.get('symbol') =='DOGEUSD_PERP' and key.get('positionAmt') > 0.0):
                                        ...
                                        if (float(a) != float(long_amount)):
                                            stop_threads = True
                                        pin = str(round(float(priceIn) +((thr_p* float(priceIn))/25),6))
                                        # if (mPrice >= Decimal(pin)):
                                        #     stop_threads = True
                                        if (float(dym_cur_long_price) > pl[-1]):
                                            if (add_long_p > pl[-1]):
