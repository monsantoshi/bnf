...
def supertrend(df, period=7, atr_multiplier=4):
    ...
    adx = df.ta.adx()
    obv = df.ta.obv()
    #df['slope'] = df.ta.slope(close=adx)
    df = pd.concat([df,adx,obv],axis=1)
...
def check_buy_sell_signals(df):
    ...
    #         close_order_buy_coin_usdt(self,'ATOMUSDT')  
    #         in_position = False
    #         message = f"CLOSE POSITION LONG ATOMUSDT"
    ...
    #         #order = exchange.create_market_buy_order('ETH/USD', 0.05)
    #     #order = exchange.create_order(symbol='adausdt', side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=1)
    #         close_order_sell_coin_usdt(self,'ATOMUSDT')  
    #         in_position = False
    #         message = f"CLOSE POSITION SHORT ATOMUSDT"
...
def run_bot():
    ...
    # bars = exchange.fetch_ohlcv('ATOMUSDT', timeframe='5m', limit=140)
    ...
    bars = request_client.get_candlestick_data(symbol="ATOMUSDT", interval=CandlestickInterval.MIN3,startTime=None, endTime=None, limit=100)
