    ...
    try:
        while True:
            ...
            for key in object_list_open:
                if ((key.get('positionSide') =='LONG') and (key.get('side') == "SELL")):
            ...
            for key in object_list:
                if (key.get('positionSide') =='LONG' and key.get('positionAmt') > 0.0):
                    sym = key.get('symbol')
                    ...
                    p = str(round(float(priceIn) +((0.03* float(priceIn))/20),4))
                    ploss = str(round(float(priceIn) +((-0.40*float(priceIn))/20),4))
                    ...
                    try:
                        ...
                        if (priceIn > 120):
                            ...
                            p = str(round(float(priceIn) +((0.065* float(priceIn))/20),2))
                            ploss =  str(round(float(priceIn) +((-0.20*float(priceIn))/20),2))
                            plost =  str(round(float(priceIn) +((-1.50*float(priceIn))/20),2))
                            if (mPrice >= Decimal(ploss)):
                            ## try:

                                take_profit_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.LIMIT, price=p, quantity=a,timeInForce='GTC')
                                time.sleep(1)
                            #plost =  str(round(float(priceIn) +((-1.50*float(priceIn))/20),2))
                            if (mPrice < Decimal(plost) and float(a) >= 0.01):
                                ...
                                cut_loss_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET, quantity=str(round(float(a),4)))
                            elif (mPrice < Decimal(plost) and float(a) > 0.006 and float(a) <= 0.008):
                                #cut_loss_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.LIMIT, price=mPrice, quantity=a//2,timeInForce='GTC')
                                cut_loss_long = request_client.post_order(symbol=sym, side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET, quantity=str(round(float(a)/4,4)))
                            elif (mPrice < Decimal(plost) and float(a) > 0.004 and float(a) <= 0.006):
                                #cut_loss_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.LIMIT, price=mPrice, quantity=a//2,timeInForce='GTC')
                                cut_loss_long = request_client.post_order(symbol=sym, side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET, quantity=str(round(float(a)/3,4)))
                            elif (mPrice < Decimal(plost) and float(a) >= 0.002 and float(a) <= 0.004):
                                #cut_loss_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.LIMIT, price=mPrice, quantity=a//2,timeInForce='GTC')
                                cut_loss_long = request_client.post_order(symbol=sym, side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET, quantity=str(round(float(a),4)))

                        elif ((priceIn > 2) and (priceIn <=120)):
                            p = str(round(float(priceIn) +((0.065* float(priceIn))/20),3))
                            ploss =  str(round(float(priceIn) +((-0.20*float(priceIn))/20),3))
                            plost =  str(round(float(priceIn) +((-1.50*float(priceIn))/20),3))
                            if (mPrice >= Decimal(ploss)):
            ##                    try:

                                take_profit_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.LIMIT, price=p, quantity=a,timeInForce='GTC')
                                time.sleep(1)
                            if (mPrice < Decimal(plost) and float(a)>= 5):
                                #cut_loss_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.LIMIT, price=mPrice, quantity=a//2,timeInForce='GTC')
                                cut_loss_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET, quantity=str(round(float(a),2)))
                            # elif (mPrice < Decimal(plost) and float(a) > 60 and float(a) <= 80):
                            #     #cut_loss_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.LIMIT, price=mPrice, quantity=a//2,timeInForce='GTC')
                            #     cut_loss_long = request_client.post_order(symbol=sym, side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET, quantity=str(round(float(a)//4,2)))
                            # elif (mPrice < Decimal(plost) and float(a) > 40 and float(a) <= 60):
                            #     #cut_loss_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.LIMIT, price=mPrice, quantity=a//2,timeInForce='GTC')
                            #     cut_loss_long = request_client.post_order(symbol=sym, side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET, quantity=str(round(float(a)//3,2)))
                            # elif (mPrice < Decimal(plost) and float(a) >= 20 and float(a) <= 40):
                            #     #cut_loss_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.LIMIT, price=mPrice, quantity=a//2,timeInForce='GTC')
                            #     cut_loss_long = request_client.post_order(symbol=sym, side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET, quantity=str(round(float(a),2)))
                        elif ((priceIn >= 0.2) and (priceIn <=2)):
                            p = str(round(float(priceIn) +((0.065* float(priceIn))/20),4))
                            ploss =  str(round(float(priceIn) +((-0.20*float(priceIn))/20),4))
                            plost =  str(round(float(priceIn) +((-1.50*float(priceIn))/20),4))
            ...
            ##                    try:
                            ...
                            p = str(round(float(priceIn) +((0.065* float(priceIn))/20),6))
                            ploss =  str(round(float(priceIn) +((-0.20*float(priceIn))/20),6))
                            plost =  str(round(float(priceIn) +((-1.50*float(priceIn))/20),6))
                            if (mPrice >= Decimal(ploss)):
            ##                    try:
