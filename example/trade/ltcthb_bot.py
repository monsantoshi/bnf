...
def check_buy_sell_signals(df):
    ...
    if not df['in_uptrend'][previous_row_index] and df['in_uptrend'][last_row_index]:
        ...
        if not in_position:
            #order = exchange.create_market_buy_order('LTC/USD', 0.05)
            ...
            order = take_order_buy_coin_perp_p('THB_LTC')
            print(order)
            in_position = True
        else:
    ...
    if df['in_uptrend'][previous_row_index] and not df['in_uptrend'][last_row_index]:
        if in_position:
            ...
            order = take_order_sell_coin_perp_p('THB_LTC')
            print(order)
            in_position = False
        else:
...
def run_bot():
    ...
    bars = exchange.fetch_ohlcv('LTC/USDT', timeframe='3m', limit=140)
    print("THB_LTC")
