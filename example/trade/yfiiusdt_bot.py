...
def take_order_buy_coin_adausdt(self,markett,coin):
    ...
    #         coin_data = collection_coin.find_one({"market":markett})
            ...
            if ((float(a) < 0.01) ):
                ...
                PrintBasic.print_obj(take_profit_coin)
            if ((float(a) >= 0.01 ) and float(a) < 0.004 and float(mPrice)< float(plostf) ):
...
def check_buy_sell_signals(df):
    ...
    #         close_order_buy_coin_usdt(self,'YFIIUSDT')  
    #         in_position = False
    #         message = f"CLOSE POSITION LONG YFIIUSDT"
    ...
    #         #order = exchange.create_market_buy_order('ETH/USD', 0.05)
    #     #order = exchange.create_order(symbol='adausdt', side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=1)
    #         close_order_sell_coin_usdt(self,'YFIIUSDT')  
    #         in_position = False
    #         message = f"CLOSE POSITION SHORT YFIIUSDT"
...
def run_bot():
    ...
    # bars = exchange.fetch_ohlcv('YFIIUSDT', timeframe='5m', limit=140)
    ...
    bars = request_client.get_candlestick_data(symbol="YFIIUSDT", interval=CandlestickInterval.MIN3,startTime=None, endTime=None, limit=100)
