    ...
    for key in object_list:
        if (key.get('positionSide') =='SHORT' and key.get('symbol') == markett and key.get('positionAmt') <= 0.0):
            ...
            if ((float(ass) <  40)):
                ...
                PrintBasic.print_obj(take_profit_coin)
            if ((float(ass) >=  40 )  and float(ass) <80 and float(mPrices) > float(plostsf) ):
...
def take_order_buy_coin_adausdt(self,markett,coin):
    ...
    #         coin_data = collection_coin.find_one({"market":markett})
            ...
            if ((float(a) >= 40  )  and float(a) < 80 and float(mPrice) < float(plostf)):
...
def check_buy_sell_signals(df):
    ...
    #         close_order_buy_coin_usdt(self,'DOGEUSDT')  
    #         in_position = False
    #         message = f"CLOSE POSITION LONG DOGEUSDT"
    ...
    #         #order = exchange.create_market_buy_order('ETH/USD', 0.05)
    #     #order = exchange.create_order(symbol='adausdt', side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=1)
    #         close_order_sell_coin_usdt(self,'DOGEUSDT')  
    #         in_position = False
    #         message = f"CLOSE POSITION SHORT DOGEUSDT"
    ...
    if df['in_uptrendAll'][last_row_index] == 8:
        ...
        if not in_position:
            ...
            #order = exchange.create_order(symbol='adausdt', side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=1)
                ...
                order = take_order_buy_coin_adausdt(self,markett,40)
                print(order)
                in_position = True
                message = f"LONG DOGEUSDT"
    ...
    if df['in_uptrendAll'][last_row_index] == 22:
        if in_position:
            ...
            if (df['in_uptrendAll'][previous_row_index] != 22):
                ...
                order = take_order_sell_coin_adausdt(self,markett,40)
                print(order)
                in_position = False
                message = f"SHORT DOGEUSDT"
...
def run_bot():
    ...
    # bars = exchange.fetch_ohlcv('DOGEUSDT', timeframe='30m', limit=140)
    ...
    bars = request_client.get_candlestick_data(symbol="DOGEUSDT", interval=CandlestickInterval.MIN3,startTime=None, endTime=None, limit=100)
