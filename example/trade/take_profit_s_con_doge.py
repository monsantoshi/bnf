...
if __name__ == '__main__':
    ...
    try:
        while True:
            ...
            #     if ((keys.get('positionSide') =='SHORT') and (keys.get('symbol') == "DOGEUSD_PERP") and (keys.get('side') == "BUY")):
            ...
            for keys in object_list_open:
                if ((keys.get('positionSide') =='SHORT') and (keys.get('symbol') == "DOGEUSD_PERP") and (keys.get('type')=="LIMIT") and (keys.get('status')=="NEW") and (keys.get('side') == "BUY")):
                    markets = keys.get('symbol')
                    orderIds = keys.get('orderId')
...
###########################
            ...
            for key in object_list:
                ...
                f.close()
                if (key.get('positionSide') =='SHORT' and key.get('symbol') =='DOGEUSD_PERP' and key.get('positionAmt') < 0.0):
                    syms = key.get('symbol')
                    ...
                    try:
                        ...
                        ps = str(round(float(priceIns) -((ps* float(priceIns))/25),6))

                        psf = str(round(float(priceIns) -((psf* float(priceIns))/25),6))
                        psff = str(round(float(priceIns) -((psff* float(priceIns))/25),6))
                        psfff = str(round(float(priceIns) -((psfff* float(priceIns))/25),6))
                        psffff = str(round(float(priceIns) -((psffff* float(priceIns))/25),6))
                        psloss = str(round(float(priceIns) +((pslost*float(priceIns))/25),6))
                        pslostf = str(round(float(priceIns) +((pslostf*float(priceIns))/25),6))
                        pslostl = str(round(float(priceIns) +((pslostl*float(priceIns))/25),6))
                        ...
                        if (mPrices <= Decimal(psloss)  and float(ass) > 0 ):
                            try:
                                ...
                                if float(ass) > float(conts):
                                    ...
                                    ake_profit_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=psf, quantity=str(rest),timeInForce='GTC')
                                    #time.sleep(1)
                                    conts = 0
                            except IndexError as error:
                                print("Exiting: with error in try" + "\n")
                        ...
                        #if (mPrices >= Decimal(pslostl)):
                            #cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=mPrices, quantity=ass//2,timeInForce='GTC')
                            #cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET, quantity= str(round(float(ass),6)))
                        ...
                        if (mPrices <= Decimal(psff) and mPrices > Decimal(psfff) ):
                            # ps = []
                            add_int_short_price =  round(float(priceIns)+ ((float(mPrices) - float(priceIns))/(float(priceIns)*100)*first_p),6)
                            #print("in if")
                            #time.sleep(1)
                            add_short_price = 0
                            ...
                            #while True:
                                ...
                                for keyi in object_list_in:
                                    if (keyi.get('positionSide') =='SHORT' and keyi.get('symbol') =='DOGEUSD_PERP' and keyi.get('positionAmt') < 0.0):
                                        ...
                                        if (float(ass) != float(short_amount) or  mPrices < Decimal(psfff)):
                                            stop_threads = True
                                        psin = str(round(float(priceIns) -((first_p* float(priceIns))/25),6))
                                        ...
                                        if (float(dym_cur_short_price) < psl[-1]):
                                            ...
                                            psl = []
                                            stop_threads = True
                        if (mPrices <= Decimal(psfff) and mPrices > Decimal(psffff) ):
                            # ps = []
                            add_int_short_price =  round(float(priceIns)+ ((float(mPrices) - float(priceIns))/(float(priceIns)*100)*sec_p),6)
                            #print("in if")
                            #time.sleep(1)
                            add_short_price = 0
                            psl.append(add_int_short_price)
                                ...
                                for keyi in object_list_in:
                                    if (keyi.get('positionSide') =='SHORT' and keyi.get('symbol') =='DOGEUSD_PERP' and keyi.get('positionAmt') < 0.0):
                                        ...
                                        if (float(ass) != float(short_amount) or  mPrices < Decimal(psffff)):
                                            stop_threads = True
                                        psin = str(round(float(priceIns) -((sec_p* float(priceIns))/25),6))
                                        # if (mPrices <= Decimal(psin) ):
                                        #     stop_threads = True
                                        if (float(dym_cur_short_price) < psl[-1]):
                                            if (add_short_p < psl[-1]):
                        ...
                        if (mPrices <= Decimal(psffff) and mPrices > Decimal(ps) ):
                            # ps = []
                            add_int_short_price =  round(float(priceIns)+ ((float(mPrices) - float(priceIns))/(float(priceIns)*100)*thr_p),6)
                            #print("in if")
                            #time.sleep(1)
                            add_short_price = 0
                            psl.append(add_int_short_price)
                                ...
                                for keyi in object_list_in:
                                    if (keyi.get('positionSide') =='SHORT' and keyi.get('symbol') =='DOGEUSD_PERP' and keyi.get('positionAmt') < 0.0):
                                        ...
                                        if (float(ass) != float(short_amount)):
                                            stop_threads = True
                                        psin = str(round(float(priceIns) -((thr_p* float(priceIns))/25),6))
                                        # if (mPrices <= Decimal(psin) ):
                                        #     stop_threads = True
                                        if (float(dym_cur_short_price) < psl[-1]):
                                            if (add_short_p < psl[-1]):
