...
def check_buy_sell_signals(df):
    ...
    #         close_order_buy_coin_usdt(self,'LTCUSDT')  
    #         in_position = False
    #         message = f"CLOSE POSITION LONG LTCUSDT"
    ...
    #         #order = exchange.create_market_buy_order('ETH/USD', 0.05)
    #     #order = exchange.create_order(symbol='adausdt', side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=1)
    #         close_order_sell_coin_usdt(self,'LTCUSDT')  
    #         in_position = False
    #         message = f"CLOSE POSITION SHORT LTCUSDT"
    ...
    if df['in_uptrendAll'][last_row_index] == 8:
        ...
        if not in_position:
            ...
            if (df['in_uptrendAll'][previous_row_index] != 8):# 6 or df['in_uptrendAll'][previous_row_index] == 11 or df['in_uptrendAll'][previous_row_index] == 4 or df['in_uptrendAll'][previous_row_index] == 0 or df['in_uptrendAll'][previous_row_index] == 8 or df['in_uptrendAll'][previous_row_index] == 1):
            #order = exchange.create_market_buy_order('ETH/USD', 0.05)
            #order = exchange.create_order(symbol='adausdt', side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=1)
                markett = "LTCUSDT"
    ...
    if df['in_uptrendAll'][last_row_index] == 22:
        if in_position:
            ...
            if (df['in_uptrendAll'][previous_row_index] != 22):# 1 or df['in_uptrendAll'][previous_row_index] == 8 or df['in_uptrendAll'][previous_row_index] == 3 or df['in_uptrendAll'][previous_row_index] == 0 or df['in_uptrendAll'][previous_row_index] == 6 or df['in_uptrendAll'][previous_row_index] == 4):
...
def run_bot():
    ...
    # bars = exchange.fetch_ohlcv('LTCUSDT', timeframe='5m', limit=140)
    ...
    # request_client = RequestClient(api_key=config.BINANCE_API_KEY, secret_key=config.BINANCE_SECRET_KEY)
    request_client = RequestClient(api_key=config.BINANCE_API_KEY, secret_key=config.BINANCE_SECRET_KEY)
    bars = request_client.get_candlestick_data(symbol="LTCUSDT", interval=CandlestickInterval.MIN3,startTime=None, endTime=None, limit=100)
