    ...
    def take_order_point(self):
        ...
        for key in object_list:
            ...
            if (key.get('positionSide') =='LONG' and (key.get('symbol') == "ADAUSD_PERP")):
                ...
                pl = str(round(float(priceIn) -((0.25* float(priceIn))/20),5))
                ...
                if (float(positionAmtLong) >= 0):
                    ...
                    if ((obv_ada ==2 and sweat_ada ==2) and (float(positionAmtShort) >= 0) and (float(positionAmtShort) < 30)):
                        Up = True
                        Down = False
                        print("Up",Up)
                        time.sleep(1)
                        print("take_order_long_full")
                        take_order_long = request_client.post_order(symbol="ADAUSD_PERP", side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=buy_unitada)
                        ...
                        time.sleep(1)
                    elif ((obv_ada ==2 and sweat_ada ==2) and (float(positionAmtShort) >= 30) ):
                        Up = True
                        Down = False
                        print("Up",Up)
                        time.sleep(1)
                        print("take_order_long_full")
                        take_order_long = request_client.post_order(symbol="ADAUSD_PERP", side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=buy_unitada*15)
                        PrintBasic.print_obj(take_order_long)
                       # update_obv(0,'BNBUSD_PERP')
                        update_sweat(0,'ADAUSD_PERP')
                        #update_guide(0,'BNBUSD_PERP')
                ...
                #         guide_bnb = 0
                #         time.sleep(1)
                    # elif ((obv_ada+sweat_ada) ==4 and (float(a) < float(step_two_unitada))):
                    #     Up = False
                    #     Down = True
                    #     print("TP_LONG",True)
                    #     time.sleep(1)
                    ...
                    #     #guide_bnb = 0
                    #     time.sleep(1)
                elif ((float(positionAmtLong) > 0) and (float(positionAmtLong) < float(step_two_unitada)) and  (float(mPrice) <= float(pl))):
                    ...
                    if ((obv_ada ==2 and sweat_ada ==2)):
                        ...
                        #guide_bnb = 0
                        time.sleep(1)
                elif ((float(positionAmtLong) >= float(step_two_unitada)) and (float(positionAmtLong) < float(step_three_unitada)) and  (float(mPrice) <= float(pll))):
        ...
        for keys in object_list:
            ...
            if (keys.get('positionSide') =='SHORT' and (keys.get('symbol') == "ADAUSD_PERP")):
                ...
                if (float(positionAmtShort) >= 0):
                    ...
                    if ((obv_ada ==1 and sweat_ada ==1) and (float(positionAmtLong) >= 0) and (float(positionAmtLong) < 30)):
                        Down = True
                        Up = False
                        ...
                        take_order_short = request_client.post_order(symbol="ADAUSD_PERP",  side=OrderSide.SELL, positionSide=PositionSide.SHORT,ordertype=OrderType.MARKET,quantity=sale_unitada)
                        ...
                        time.sleep(1)
                    elif ((obv_ada ==1 and sweat_ada ==1) and (float(positionAmtLong) >= 30)):
                        Down = True
                        Up = False
                        ...
                        take_order_short = request_client.post_order(symbol="ADAUSD_PERP",  side=OrderSide.SELL, positionSide=PositionSide.SHORT,ordertype=OrderType.MARKET,quantity=sale_unitada*15)
                        PrintBasic.print_obj(take_order_short)
                        #update_obv(0,'BNBUSD_PERP')
                        update_sweat(0,'ADAUSD_PERP')
                        #update_guide(0,'BNBUSD_PERP')
                ...
                #         time.sleep(1)
                    # elif ((obv_ada ==1 and sweat_ada ==1) and (float(ass) < float(step_two_unitada))):
                    #     Down = False
                    #     Up = True
                    ...
                    #     time.sleep(1)
                elif (float(positionAmtShort) > 0 and float(positionAmtShort) < float(step_two_unitada) and (float(mPrices) >= float(pss))):
                    # client = MongoClient(port=27017)
                    # db = client.binance
                    # collection_ltc = db["count"]
                    ...
                    if ((obv_ada ==1 and sweat_ada ==1)):
                        ...
                        time.sleep(1)
                elif (float(positionAmtShort) >= float(step_two_unitada) and float(positionAmtShort) < float(step_three_unitada) and (float(mPrices) >= float(psss))):
                    # client = MongoClient(port=27017)
                    # db = client.binance
                    # collection_ltc = db["count"]
        ...
        #             # sweat = ltc_data["sweat"]
        #             # if ((dark =="1" and obv =="1" and sweat =="1")):
        #             if ((obv_btc ==2 and sweat_btc ==2) and (float(positionAmtShort) >= 0) and (float(positionAmtShort) < 30)):
        #                 Up = True
        #                 Down = False
        #                 print("Up",Up)
        #                 time.sleep(1)
        ...
        #                 time.sleep(1)
        #             elif ((obv_btc ==2 and sweat_btc ==2) and (float(positionAmtShort) >= 30) ):
        #                 Up = True
        #                 Down = False
        #                 print("Up",Up)
        #                 time.sleep(1)
        ...
        #             # sweat = ltc_data["sweat"]
        #             # if ((dark =="0" and obv =="0" and sweat =="0")):
        #             if ((obv_btc ==1 and sweat_btc ==1) and (float(positionAmtLong) >= 0) and (float(positionAmtLong) < 30)):
        #                 Down = True
        #                 Up = False
        ...
        #                 time.sleep(1)
        #             elif ((obv_btc ==1 and sweat_btc ==1) and (float(positionAmtLong) >= 30)):
        #                 Down = True
        #                 Up = False
