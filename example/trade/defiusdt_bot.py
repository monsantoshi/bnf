...
def make_profit_long():
    ...
    for keys in object_list_open:
        #cont = 0
        if ((keys.get('positionSide') =='LONG') and (keys.get('symbol') == "DEFIUSDT")  and (keys.get('type')=="LIMIT") and (keys.get('status')=="NEW") and (keys.get('side') == "SELL")):
    ...
    for key in object_list:
        ...
        f.close()
        if (key.get('positionSide') =='LONG' and key.get('symbol') =='DEFIUSDT' and key.get('positionAmt') > 0.0):
            sym = key.get('symbol')
            mPrice = key.get('markPrice')
            priceIn = key.get('entryPrice')
            positionAmtLong = key.get('positionAmt')
            ...
            try:
                ...
                pf = str(round(float(priceIn) +((0.05* float(priceIn))/20),1))
                pff = str(round(float(priceIn) +((pff* float(priceIn))/20),1))
                puffs = str(round(float(priceIn) +((puffs* float(priceIn))/20),1))
                ...
                if (mPrice >= Decimal(ploss) ):
                    try:
                        ...
                        if float(a) > float(conts):
                            ...
                            take_profit_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.LIMIT, price=pf, quantity=str(round(rest,1)),timeInForce='GTC')
                            #time.sleep(1) 
                            conts = 0
                    except IndexError as error:
                        print("Exiting: with error in try" + "\n")
...
def make_profit_short():
    ...
    for keys in object_list_open:
        if ((keys.get('positionSide') =='SHORT') and (keys.get('symbol') == "DEFIUSDT") and (keys.get('type')=="LIMIT") and (keys.get('status')=="NEW") and (keys.get('side') == "BUY")):
            markets = keys.get('symbol')
            orderIds = keys.get('orderId')
            conts = keys.get('origQty')
            print("market",markets)
...
###########################
    ...
    for key in object_list:
        ...
        f.close()
        if (key.get('positionSide') =='SHORT' and key.get('symbol') =='DEFIUSDT' and key.get('positionAmt') < 0.0):
            syms = key.get('symbol')
            mPrices = key.get('markPrice')
            priceIns = key.get('entryPrice')
            positionAmtShort = key.get('positionAmt')
            ...
            try:
                ...
                ps = str(round(float(priceIns) -((ps* float(priceIns))/20),1))

                psf = str(round(float(priceIns) -((0.05* float(priceIns))/20),1))
                psff = str(round(float(priceIns) -((psff* float(priceIns))/20),1))
                psfff = str(round(float(priceIns) -((psfff* float(priceIns))/20),1))
                psffff = str(round(float(priceIns) -((psffff* float(priceIns))/20),1))
                psloss = str(round(float(priceIns) +((pslost*float(priceIns))/20),1))
                pslostf = str(round(float(priceIns) +((pslostf*float(priceIns))/20),1))
                pslostl = str(round(float(priceIns) +((pslostl*float(priceIns))/20),1))
                try:
                ...
                if (mPrices <= Decimal(psloss)  and float(ass) > 0 ):
                    try:
                        ...
                        if float(ass) > float(conts):
                            ...
                            ake_profit_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=psf, quantity=str(round(rest,1)),timeInForce='GTC')
                            #time.sleep(1)
                            conts = 0
                    except IndexError as error:
                        print("Exiting: with error in try" + "\n")
...
def check_buy_sell_signals(df):
    ...
    if not df['in_uptrend'][previous_row_index] and df['in_uptrend'][last_row_index]:
        ...
        if not in_position:
            ...
            close_order_sell_coin_usdt(self,'DEFIUSDT')
            order = take_order_buy_coin_adausdt(self,'DEFIUSDT',0.01)
            print(order)
            in_position = True
            message = f"LONG DEFI"
    ...
    if df['in_uptrend'][previous_row_index] and not df['in_uptrend'][last_row_index]:
        if in_position:
            ...
            close_order_buy_coin_usdt(self,'DEFIUSDT')
            order = take_order_sell_coin_adausdt(self,'DEFIUSDT',0.01)
            print(order)
            in_position = False
            message = f"SHORT DEFI "
...
def run_bot():
    ...
    bars = exchange.fetch_ohlcv('DEFIUSDT', timeframe='5m', limit=50)
