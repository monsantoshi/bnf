...
def take_order_buy_bnbusdt():
    ...
    my_balance = client.get_asset_balance(asset='BNB')
    unit_avl = float(my_balance['free'])   
    if (unit_avl < 0.02): 
        order = client.create_order(symbol='BNBUSDT',side=Client.SIDE_BUY,type=Client.ORDER_TYPE_MARKET,quantity=0.03)
    return
...
def take_order_sell_bnbusdt():
    ...
    client = Client(api_key, api_secret)
    my_balance = client.get_asset_balance(asset='BNB')
    unit_avl = truncate(float(my_balance['free']),3)
        ...
        if trade['isBuyer']==True:
            ...
            if (float(sell_price) > pl[-1]):
                ...
                print("add_last_price" , sell_price)
                    #time.sleep(1)
                print("pl in if",pl)
    pricef = str(round(float(pl[-1]),1))
    print("last_TP_price",pricef)

    if (float(unit_avl) > 0):
        #print("unit_avl", unit_avl)
        order = client.create_order(symbol='BNBUSDT',side=Client.SIDE_SELL,type=Client.ORDER_TYPE_LIMIT,price=pricef,quantity=unit_avl,timeInForce=Client.TIME_IN_FORCE_GTC)
...
def run_bot():
    ...
    bars = exchange.fetch_ohlcv('BNB/USDT', timeframe='m', limit=140)
    print("SPOT BNB/USDT")
