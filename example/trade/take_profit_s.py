                ...
                if (key.get('positionSide') =='SHORT' and key.get('positionAmt') < 0.0):
                    ...
                    try:
                        if (priceIns > 10000):
                            ...
                            ps = str(round(float(priceIns) -((0.06* float(priceIns))/20),1))
                            psloss = str(round(float(priceIns) +((0.20*float(priceIns))/20),1))
                            pslostf = str(round(float(priceIns) +((0.45*float(priceIns))/20),1))
                            pslostl = str(round(float(priceIns) +((1.00*float(priceIns))/20),1))
                            ...
                            #             re_unit = float(ass) - ((float(ass)-float(al))/2)
                            #             #cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=mPrices, quantity=ass//2,timeInForce='GTC')
                        ...
                        elif ((priceIns > 100) and (priceIns <=10000)):
                            ps = str(round(float(priceIns) -((0.06* float(priceIns))/20),2))
                            psloss = str(round(float(priceIns) +((0.20*float(priceIns))/20),2))
                            pslostf = str(round(float(priceIns) +((0.45*float(priceIns))/20),2))
                            pslostl = str(round(float(priceIns) +((1.00*float(priceIns))/20),2))
                            if (mPrices <= Decimal(psloss)):
                                ake_profit_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=ps, quantity=ass,timeInForce='GTC')
                                time.sleep(1)
                            if (mPrices >= Decimal(pslostl)):
                            ...
                            #             cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET, quantity= str(round(float(re_unit),2))) 
                        elif ((priceIns > 2) and (priceIns <=100)):
                            ps = str(round(float(priceIns) -((0.06* float(priceIns))/20),3))
                            psloss = str(round(float(priceIns) +((0.20*float(priceIns))/20),3))
                            pslostf = str(round(float(priceIns) +((0.45*float(priceIns))/20),3))
                            pslostl = str(round(float(priceIns) +((1.00*float(priceIns))/20),3))
                            if (mPrices <= Decimal(psloss)):
                                ake_profit_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=ps, quantity=ass,timeInForce='GTC')
                                time.sleep(1)
                            if (mPrices >= Decimal(pslostl)):
                            ...
                            ps = str(round(float(priceIns) -((0.06* float(priceIns))/20),4))
                            psloss = str(round(float(priceIns) +((0.20*float(priceIns))/20),4))
                            pslostf = str(round(float(priceIns) +((0.45*float(priceIns))/20),4))
                            pslostl = str(round(float(priceIns) +((1.00*float(priceIns))/20),4))
                            if (mPrices <= Decimal(psloss)):
                                ake_profit_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=ps, quantity=ass,timeInForce='GTC')
                                time.sleep(1)
                            if (mPrices >= Decimal(pslostl)):
                            ...
                            ps = str(round(float(priceIns) -((0.06* float(priceIns))/20),6))
                            psloss = str(round(float(priceIns) +((0.20*float(priceIns))/20),6))
                            pslostf = str(round(float(priceIns) +((0.45*float(priceIns))/20),6))
                            pslostl = str(round(float(priceIns) +((1.00*float(priceIns))/20),6))
                            if (mPrices <= Decimal(psloss)):
                                ake_profit_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=ps, quantity=ass,timeInForce='GTC')
                                time.sleep(1)
                            if (mPrices >= Decimal(pslostl)):
