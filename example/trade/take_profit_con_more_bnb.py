            ...
            for key in object_list:
                ...
                if (key.get('positionSide') =='LONG' and key.get('symbol') =='BNBUSD_PERP' and key.get('positionAmt') > 0.0):
                    ...
                    try:
                        ...
                        p = str(round(float(priceIn) +((0.50* float(priceIn))/20),2))
                        pf = str(round(float(priceIn) +((0.02* float(priceIn))/20),2))
                        pff = str(round(float(priceIn) +((0.025* float(priceIn))/20),2))
                        pfff = str(round(float(priceIn) +((0.05* float(priceIn))/20),2))
                        pffff = str(round(float(priceIn) +((0.40* float(priceIn))/20),2))
                        ploss =  str(round(float(priceIn) +((-0.10*float(priceIn))/20),2))
                        plostf =  str(round(float(priceIn) +((-0.15*float(priceIn))/20),2))
                        plostl =  str(round(float(priceIn) +((-0.20*float(priceIn))/20),2))
                        if (mPrice >= Decimal(pffff) ):
                                request_client = RequestClient(api_key=g_api_key, secret_key=g_secret_key)
                                ...
                                for key in object_list:
                                    if (key.get('positionSide') =='LONG' and key.get('positionAmt') > 0.0  and key.get('symbol') == sym):
                                        syms = key.get('symbol')
                                        mPrices = key.get('markPrice')
                                        priceIns = key.get('entryPrice')
                                        positionAmtSLong = key.get('positionAmt')
                                        al = str(round(float(positionAmtLong),2))

                                       # re_unit = float(al)//2

                                        cut_loss_long = request_client.post_order(symbol=sym, side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET, quantity= str(float(al)//5))
