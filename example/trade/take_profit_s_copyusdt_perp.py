    ...
    try:
        while True:
            ...
            for keys in object_list_open:
                if ((keys.get('positionSide') =='SHORT') and (keys.get('symbol') == "BNBUSDT") and (keys.get('type')=="LIMIT") and (keys.get('status')=="NEW") and (keys.get('side') == "BUY")):
...
###########################
            ...
            for key in object_list:
                ...
                if (key.get('positionSide') =='SHORT' and key.get('symbol') =='BNBUSDT' and key.get('positionAmt') < 0.0):
                    syms = key.get('symbol')
                    ...
                    try:
                        ...
                        psf = str(round(float(priceIns) -((0.05* float(priceIns))/20),2))
                        psff = str(round(float(priceIns) -((0.08* float(priceIns))/20),2))
                        psfff = str(round(float(priceIns) -((0.15* float(priceIns))/20),2))
                        psffff = str(round(float(priceIns) -((0.50* float(priceIns))/20),2))
                        psloss = str(round(float(priceIns) +((0.20*float(priceIns))/20),2))
                        pslostf = str(round(float(priceIns) +((0.25*float(priceIns))/20),2))
                        pslostl = str(round(float(priceIns) +((0.40*float(priceIns))/20),2))
                        ...
                        #cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT,stopPrice=pslostf,  ordertype=OrderType.STOP_MARKET, closePosition='True')
                        ...
                        elif (mPrices <= Decimal(psloss)  and float(ass) >= 1):
                            ...
                            #time.sleep(1)
                        elif (mPrices <= Decimal(psloss)  and float(ass) >= 0.6 and  float(ass) < 1):
                            if float(conts) == 0:
                                ake_profit_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=psf, quantity=ass,timeInForce='GTC')
                            ...
                            if float(ass) > float(conts):
                                ...
                                #time.sleep(1)
                                conts = 0
                        elif (mPrices <= Decimal(psff) and mPrices > Decimal(psfff) and float(ass) < 0.6):
                            # ps = []
                            ...
                            #while True:
                                ...
                                for keyi in object_list_in:
                                    if (keyi.get('positionSide') =='SHORT' and keyi.get('symbol') =='BNBUSDT' and keyi.get('positionAmt') < 0.0):
                                        ...
                                        if (float(dym_cur_short_price) < psl[-1]):
                                            ...
                                            psl = []
                                            stop_threads = True
                        elif (mPrices <= Decimal(psfff) and mPrices > Decimal(ps) and float(ass) < 0.6):
                            # ps = []
