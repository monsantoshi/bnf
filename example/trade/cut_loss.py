...
def insert_open_order(x):
    ...
    client = MongoClient(port=27017)
    db = client.binance
    collection_open = db["close_mon70"]
    #count_ss = count_s + count
...
if __name__ == '__main__':
    ...
    try:
        while True:
            ...
            collection_open = db["close_mon70"]
            #count_ss = count_s + count
            collection_open.delete_many({})
            print(" Beging Insert open orders ")

            for o in object_list_open:
               insert_open_order(o)
            print(" End Insert open orders")
            ...
            index_data_long = collection_test.find({ "positionAmt": { "$gt": 0 } })

            for doclloss in index_data_long:
                #print("cut loss long order in this coins")
                print("Long", doclloss["symbol"])
                sym = doclloss["symbol"]
                priceIn = doclloss["entryPrice"]
                mPrice = doclloss["markPrice"]
                a = doclloss["positionAmt"]

                client = MongoClient(port=27017)
                db = client.binance
                collection_cut_loss = db["cut_loss"]

                cut_data = collection_cut_loss.find_one({"market": sym})
                ac = cut_data["step_two_unit"]
                
                #if ((sym == 'DOGEUSDT') or (sym == 'XRPUSDT') or (sym == 'ALPHAUSDT') or (sym == 'ALGOUSDT') or (sym == 'SXPUSDT')  or (sym == 'ETHUSDT')):

                if ((priceIn > 10) ):
                    
                    #p = str(round(float(docl["entryPrice"]+((0.03*docl["entryPrice"])/20)),2))
                    ploss = str(round(float(doclloss["entryPrice"]+((-0.30*doclloss["entryPrice"])/20)),2))
                ...
                elif ((priceIn > 2) and (priceIn <10) ):
                    #p = str(round(float(docl["entryPrice"]+((0.03*docl["entryPrice"])/20)),3))
                    ploss = str(round(float(doclloss["entryPrice"]+((-0.30*doclloss["entryPrice"])/20)),3))
                    if (mPrice <= Decimal(ploss)):
                        cut_loss_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=str(round(float(a),2)))

                elif ((priceIn > 0.2) and (priceIn <= 2 ) ):
                    #p = str(round(float(docl["entryPrice"]+((0.03*docl["entryPrice"])/20)),4))

                    ploss = str(round(float(doclloss["entryPrice"]+((-0.30*doclloss["entryPrice"])/20)),4))
                    if (mPrice <= Decimal(ploss)):
                        cut_loss_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=str(round(float(a),2)))
                    ...
                    ploss = str(round(float(doclloss["entryPrice"]+((-0.30*doclloss["entryPrice"])/20)),6))
                    if (mPrice <= Decimal(ploss)):
                        cut_loss_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=str(round(float(a),2)))

                time.sleep(1)

            index_data_short = collection_test.find({ "positionAmt": { "$lt": 0 } })
#            index_cut_loss_short = collection_cut_loss.find({ "positionAmtShort": { "$gt": 0 } })
            for docsloss in index_data_short:
                #print("short order in this coins")
                print("Short", docsloss["symbol"])
                #=8.189-((-0.05*8.189)/20)*-1
                syms = docsloss["symbol"]
                priceIns = docsloss["entryPrice"]
                mPrices = docsloss["markPrice"]
                ass = docsloss["positionAmt"]*-1

                client = MongoClient(port=27017)
                db = client.binance
                collection_cut_loss = db["cut_loss"]

                cut_data = collection_cut_loss.find_one({"market": syms})
                acs = cut_data["step_two_unit"]
                
                #if ((syms == 'DOGEUSDT') or (syms == 'XRPUSDT')  or (syms == 'SXPUSDT') or (syms == 'SXPUSDT') or (syms == 'RLCUSDT') or (syms == 'ALPHAUSDT') or (syms == 'ALGOUSDT') or (syms == 'ETHUSDT') or (syms == 'BTCUSDT') or (syms == 'BCHUSDT')):
                if ((priceIns > 10) ):
                    print("a")
                    
                    #ps = str(round(float(docs["entryPrice"]-((-0.03*docs["entryPrice"])/20)*-1),2))
                    psloss = str(round(float(docsloss["entryPrice"]-((0.30*docsloss["entryPrice"])/20)*-1),2))
                    if (mPrices >= Decimal(psloss)):
                        print("mPrice", mPrices)
                        print("psloss", psloss)
                ...
                elif ((priceIns > 2) and (priceIns <10) ):
                    print("b")
                    #ps = str(round(float(docs["entryPrice"]-((-0.03*docs["entryPrice"])/20)*-1),3))
                    psloss = str(round(float(docsloss["entryPrice"]-((0.30*docsloss["entryPrice"])/20)*-1),3))
                    print("psloss b", psloss)
                    if (mPrices >= Decimal(psloss)):
                        print("mPrice", mPrices)
                        print("psloss", psloss)
                        cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET, quantity= str(round(float(ass),2)))

                elif ((priceIns > 0.2) and (priceIns <= 2) ):
                    print("c")
                    #ps = str(round(float(docs["entryPrice"]-((-0.03*docs["entryPrice"])/20)*-1),4))
                    psloss = str(round(float(docsloss["entryPrice"]-((0.30*docsloss["entryPrice"])/20)*-1),4))
                    if (mPrices >= Decimal(psloss)):
                        print("mPrice", mPrices)
                        print("psloss", psloss)
                        cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET, quantity= str(round(float(ass),2)))
                    ...
                    print("d")
                    #ps = str(round(float(docs["entryPrice"]-((-0.03*docs["entryPrice"])/20)*-1),4))
                    psloss = str(round(float(docsloss["entryPrice"]-((0.30*docsloss["entryPrice"])/20)*-1),6))
                    if (mPrices >= Decimal(psloss)):
                        print("mPrice", mPrices)
                        print("psloss", psloss)
                        cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET, quantity= str(round(float(ass),2)))
