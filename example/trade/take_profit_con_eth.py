    ...
    try:
        while True:
            ...
            for keys in object_list_open:
                if ((keys.get('positionSide') =='LONG') and (keys.get('symbol') == "ETHUSD_PERP")  and (keys.get('type')=="LIMIT") and (keys.get('status')=="NEW") and (keys.get('side') == "SELL")):
...
###########################
            ...
            for key in object_list:
                ...
                if (key.get('positionSide') =='LONG' and key.get('symbol') =='ETHUSD_PERP'  and  key.get('positionAmt') > 0.0):
                    sym = key.get('symbol')
                   ...
                   # pres = (d.as_tuple().exponent)*-1
                    ...
                    try:
                        ...
                        p = str(round(float(priceIn) +((3.00* float(priceIn))/20),2))
                        pf = str(round(float(priceIn) +((0.10* float(priceIn))/20),2))
                        pff = str(round(float(priceIn) +((0.15* float(priceIn))/20),2))
                        pfff = str(round(float(priceIn) +((0.20* float(priceIn))/20),2))
                        pffff = str(round(float(priceIn) +((1.00* float(priceIn))/20),2))
                        ploss =  str(round(float(priceIn) +((-0.20*float(priceIn))/20),2))
                        plostf =  str(round(float(priceIn) +((-0.20*float(priceIn))/20),2))
                        plostl =  str(round(float(priceIn) +((-0.50*float(priceIn))/20),2))
                        ...
                        #                     cut_loss_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET, quantity= str(round(float(re_unit),2)))
                        if (mPrice >= Decimal(ploss) and float(a) >=90 ):
                            ...
                            time.sleep(5)
                        elif (mPrice >= Decimal(ploss) and float(a) >=2 and float(a) < 90 ):
                            if float(conts) == 0:
                            ...
                            if float(a) > float(conts):
                                ...
                                #time.sleep(5) 
                                conts = 0
                        elif (mPrice >= Decimal(pfff) and mPrice < Decimal(pffff) and float(a) < 2 ):
                            #pl = []
                            add_int_long_price =   round(float(priceIn)+ ((float(mPrice) - float(priceIn))/(float(priceIn)*100)*0.50),2)
                            print("in if")
                            ...
                            while ((stop_threads == False) and  mPrice < Decimal(p)):
                                ...
                                for key in object_list_in:
                                    if (key.get('positionSide') =='LONG' and key.get('symbol') =='ETHUSD_PERP' and key.get('positionAmt') > 0.0):
                                        ...
                                        #print("dc_price",dym_cur_long_price)
                                        add_long_price =  float(priceIn)+ float(priceIn)*((float(dym_cur_long_price) - float(priceIn))/(float(priceIn))*0.50)
                                        ...
                                        if (float(a) != float(long_amount)):
                                            stop_threads = True
                                        pin = str(round(float(priceIn) +((0.50* float(priceIn))/20),2))
                                        ...
                                        if (float(dym_cur_long_price) > pl[-1]):
                                            ...
                                            pl = []       
                                            stop_threads = True    
                        elif (mPrice >= Decimal(pffff) and mPrice < Decimal(p) and float(a) <=20 ):
                            #pl = []
