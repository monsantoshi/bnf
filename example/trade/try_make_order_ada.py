...
def take_order_sell_coin(self,market):
    ...
    for key in object_list:
        if (key.get('positionSide') =='SHORT' and key.get('symbol') == 'adausd_perp' and key.get('positionAmt') <= 0.0):
           ...
           # time.sleep(5)
            ...
            coin_data = collection_cutloss.find_one({"market": 'adausd_perp'})
            ...
            if ((float(ass) >=  0) and (float(ass) <= float(step_two_unit))):
                take_profit_long = request_client.post_order(symbol='adausd_perp', side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET, quantity=1)
    ...
    return
def take_order_buy_coin(self,market):
    ...
    for key in object_list:
        if (key.get('positionSide') =='LONG' and key.get('positionAmt') >= 0.0  and key.get('symbol') == 'adausd_perp'):

            positionAmtShort = key.get('positionAmt')
            #print(market,'',markPrice,'',entryPriceLong,'',positionAmtLong)
            a = str(round(float(positionAmtShort),3))
            ...
            coin_data = collection_coin.find_one({"market":'adausd_perp'})
            ...
            if ((float(a) >= 0) and (float(a) <= float(step_two_unit))):
                ...
                take_profit_long = request_client.post_order(symbol='adausd_perp', side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET, quantity=1)
                PrintBasic.print_obj(take_profit_long)
                time.sleep(1)
...
def update_guide(d,market):
    ...
    collection_count.update_one({"market": market},{"$set":{"guide":d} })      
class TryToTakeOrder:
    ...
    def take_order_point(self):
        ...
        result = request_client.get_order_book(symbol = "adausd_perp", limit = 20)
        init_bq = []
        init_bp = []
        init_aq = []
        init_ap = []
        ...
        for i in range(20):
            result_xtz = request_client.get_order_book(symbol = "adausd_perp", limit = 20)
            #time.sleep(1)
        ...
        print("price_asks_last ",price_asks[19])
        print("price_bids_first ",price_bids[0])
        print("price_bids_last ",price_bids[19])
        time.sleep(2)
        s = 0
        l = 0
        ...
        #time.sleep(30)
        #old matomod
        kline2 = request_client.get_candlestick_data(symbol="adausd_perp", interval=CandlestickInterval.MIN3,startTime=None, endTime=None, limit=20)
        ...
        Down =  (float(haopen6) > float(haclose6)) and (s == 1)
        #atom_TrendDown = Bearish_Engulfing
        print("Down",Down)
        Up =  (float(haopen6) < float(haclose6)) and (l == 1)
        ...
        if ((Down)):
            ...
            #take_order_sell_coin(self,market)
            update_guide(1,'ADAUSD_PERP')
            time.sleep(1) 
        ...
        if ((Up)):
            ...
            #take_order_buy_coin(self,market)
            update_guide(2,'ADAUSD_PERP')
...
if __name__ == '__main__':
    ...
    try:
        while True:
            request_client = RequestClient(api_key=g_api_key, secret_key=g_secret_key)
            time.sleep(1)
            thread1 = Bin3(1,"Check Loss Short More")
            #thread8.start()
            #thread5.start()
            thread1.start()
            time.sleep(1)
