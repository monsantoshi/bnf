...
def take_order_buy_coin_perp_p(markett):
    ...
    unit_avl = data[markett]['available']
    #unit_res = data[markett]['reserved']
...
def check_buy_sell_signals(df):
    ...
    if not df['in_uptrend'][previous_row_index] and df['in_uptrend'][last_row_index]:
        ...
        if not in_position:
            ...
            order = take_order_buy_coin_perp_p('THB_DOGE')
            print(order)
            message = f"BUY THB_DOGE"
    ...
    if df['in_uptrend'][previous_row_index] and not df['in_uptrend'][last_row_index]:
        if in_position:
            ...
            order = take_order_sell_coin_perp_p('THB_DOGE')
            print(order)
            message = f"SELL THB_DOGE"
...
def run_bot():
    ...
    bars = exchange.fetch_ohlcv('DOGE/USDT', timeframe='5m', limit=140)
    print("THB_DOGE")
    ...
    supertrend_data = cal.calculate_signal.upertrend(df)
