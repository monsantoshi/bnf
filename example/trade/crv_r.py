...
def take_order_sell(self):
    ...
    count_s = crv_data["count_ss"]
    
    limit_s = crv_data["limit_ss"]
    count_l = crv_data["count_ll"]
    
    limit_l = crv_data["limit_ll"]
    reentryunit = str(round(float(crv_data["reentry_units"]),4))
...
def inc_count_s(count):
    ...
    collection_crv.update_one({"market": 'CRVUSDT'},{"$set":{"count_ss":count_ss} })
...
def cls_count_s(count):
    ...
    collection_crv = db["cut_loss"]
    crv_data = collection_crv.find_one({"market":"CRVUSDT"})
    count_ll = crv_data["count_ll"]
    if (count_ll > 5):
        collection_crv.update_one({"market": 'CRVUSDT'},{"$set":{"count_ss":count} })
...
def inc_count_l(count):
    ...
    collection_crv.update_one({"market": 'CRVUSDT'},{"$set":{"count_ll":count_ll} })
    #if (count_l > limit_l+1):
        #count_ss = count_s + count
        #collection_crv.update_one({"market": 'CRVUSDT'},{"$set":{"count_ll":0} })
...
def cls_count_l(count):
    ...
    collection_crv = db["cut_loss"]
    crv_data = collection_crv.find_one({"market":"CRVUSDT"})
    count_ss = crv_data["count_ss"]
    if (count_ss > 5):
        collection_crv.update_one({"market": 'CRVUSDT'},{"$set":{"count_ll":count} })
    return
...
##    step_seven_unit = crv_data["step_seven_unit"]
    ...
    reentry_unit = crv_data["reentry_units"]
