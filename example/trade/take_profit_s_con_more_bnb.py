            ...
            for key in object_list:
                ...
                if (key.get('positionSide') =='SHORT' and key.get('symbol') =='BNBUSD_PERP' and key.get('positionAmt') < 0.0):
                    ...
                    try:
                        ...
                        ps = str(round(float(priceIns) -((0.50* float(priceIns))/20),2))

                        psf = str(round(float(priceIns) -((0.02* float(priceIns))/20),2))
                        psff = str(round(float(priceIns) -((0.025* float(priceIns))/20),2))
                        psfff = str(round(float(priceIns) -((0.05* float(priceIns))/20),2))
                        psffff = str(round(float(priceIns) -((0.40* float(priceIns))/20),2))
                        psloss = str(round(float(priceIns) +((0.10*float(priceIns))/20),2))
                        pslostf = str(round(float(priceIns) +((0.15*float(priceIns))/20),2))
                        pslostl = str(round(float(priceIns) +((0.20*float(priceIns))/20),2))
                           
                        if (mPrices <= Decimal(psffff)):
                            request_client = RequestClient(api_key=g_api_key, secret_key=g_secret_key)
                            ...
                            for key in object_list:
                                ...
                                if (key.get('positionSide') =='SHORT' and key.get('positionAmt') < 0.0 and key.get('symbol') == syms):
                                    sym = key.get('symbol')
                                    mPrice = key.get('markPrice')
                                    priceIn = key.get('entryPrice')
                                    positionAmtShort = key.get('positionAmt')
                                    #print(market,'',markPrice,'',entryPriceLong,'',positionAmtLong)
                                    al = str(float(positionAmtShort)*-1)

                                   # re_unit = ((float(al))//2)
                                    cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET, quantity= str(float(al)//5) )
