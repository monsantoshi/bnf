...
class TryToTakeOrder:
    ...
    def take_order_point(self):
        ...
        for resb in result.bids:
            init_bq.append(round(float(resb.qty),3))
            init_bp.append(round(float(resb.price),3))
        for resa in result.asks:
            init_aq.append(round(float(resa.qty),3))
            init_ap.append(round(float(resa.price),3))
        ...
        for i in range(20):
            ...
            for resb in result_xtz.bids:
                #print(resb.qty)
                bid_q.append(round(float(resb.qty),3))
                bid_p.append(round(float(resb.price),3))

               
            for resa in result_xtz.asks:
                #print(resb.qty)
                ask_q.append(round(float(resa.qty),3))
                ask_p.append(round(float(resa.price),3))
    ...
    def take_order_point_long(self):
        ...
        for res in result.bids:
            init_bq.append(round(float(res.qty),3))
            init_bp.append(round(float(res.price),3))
        for res in result.asks:
            init_aq.append(round(float(res.qty),3))
            init_ap.append(round(float(res.price),3))
        index_init_bq = init_bq.index(max(init_bq))
        init_price_bid = init_bp[index_init_bq]
        #print("init_price_bid" ,init_price_bid)
        index_init_aq = init_aq.index(max(init_aq))
    ...
    def check_profit_long_profit(self):
        ...
        if stop_threads:
            ...
            #print("long_price:  ",self.long_price +"   "+"int_long_price: ",self.int_cur_long_price +"   "+
             #     "int_long_rate: ",round((self.int_long_rate*100),3))
            print("int_long_rate : ",round((int_long_rate*100),3))
            ...
            if (((int_long_rate*100) >= stop_profit_rate_long) and ((long_amount > step_two_unit) and (long_amount < step_three_unit))):
                add_int_long_price =  round(float(long_price)+ ((float(int_cur_long_price) - float(long_price))/(float(long_price)*100)*protect_profit),3)
                #print("in if")
                #time.sleep(1)
                add_long_price = 0
...
class CheckProfitShort:
    ...
    def check_profit_short_profit(self):
        ...
        if stop_threads:
            ...
            #print("long_price:  ",self.long_price +"   "+"int_long_price: ",self.int_cur_long_price +"   "+
             #     "int_long_rate: ",round((self.int_long_rate*100),3))
            print("int_short_rate : ",round((int_short_rate*100),3))
            ...
            if (((int_short_rate*100) >= stop_profit_rate_short) and ((short_amount > step_two_unit) and  (short_amount < step_three_unit))):
                add_int_short_price =  round(float(short_price)+ ((float(int_cur_short_price) - float(short_price))/(float(short_price)*100)*protect_profit_s),3)
                #print("in if")
                #time.sleep(1)
                add_short_price = 0
                ...
                while True:
                    ...
                    add_short_p = round(add_short_price,3)
                    #pl.append(add_long_p)
...
class CheckProfitLongFromOut:
    ...
    def check_profit_long_profit(self):
        ...
        if (in_short_out):
            ...
            #print("long_price:  ",self.long_price +"   "+"int_long_price: ",self.int_cur_long_price +"   "+
             #     "int_long_rate: ",round((self.int_long_rate*100),3))
            print("int_short_rate_out : ",round((int_short_rate_out*100),3))
...
class CheckProfitShortFromOut:
    ...
    def check_profit_short_profit(self):
        ...
        if (in_long_out):
            ...
            #print("long_price:  ",self.long_price +"   "+"int_long_price: ",self.int_cur_long_price +"   "+
             #     "int_long_rate: ",round((self.int_long_rate*100),3))
            print("int_long_rate_out : ",round((int_long_rate_out*100),3))
...
class CheckLossLongFromIn:
    ...
    def check_profit_long_profit(self):
        ...
        if (in_long_in):
            ...
            #print("long_price:  ",self.long_price +"   "+"int_long_price: ",self.int_cur_long_price +"   "+
             #     "int_long_rate: ",round((self.int_long_rate*100),3))
            print("int_long_rate_in : ",round((int_long_rate_in*100),3))
...
class CheckLossShortFromIn:
    ...
    def check_profit_short_profit(self):
        ...
        if (in_short_in):
            ...
            #print("long_price:  ",self.long_price +"   "+"int_long_price: ",self.int_cur_long_price +"   "+
             #     "int_long_rate: ",round((self.int_long_rate*100),3))
            print("int_short_rate_in : ",round((int_short_rate_in*100),3))
    ...
    try:
        while True:
            ...
            for keys in object_list_open:
                if ((keys.get('positionSide') =='SHORT') and (keys.get('symbol') == "ATOMUSDT") and (keys.get('type')=="LIMIT") and (keys.get('status')=="NEW") and (keys.get('side') == "BUY")):
...
###########################
            ...
            for key in object_list:
                ...
                f.close()
                if (key.get('positionSide') =='SHORT' and key.get('symbol') =='ATOMUSDT' and key.get('positionAmt') < 0.0):
                    syms = key.get('symbol')
                    ...
                    try:
                        ...
                        ps = str(round(float(priceIns) -((ps* float(priceIns))/20),3))

                        psf = str(round(float(priceIns) -((psf* float(priceIns))/20),3))
                        psff = str(round(float(priceIns) -((psff* float(priceIns))/20),3))
                        psfff = str(round(float(priceIns) -((psfff* float(priceIns))/20),3))
                        psffff = str(round(float(priceIns) -((psffff* float(priceIns))/20),3))
                        psloss = str(round(float(priceIns) +((pslost*float(priceIns))/20),3))
                        pslostf = str(round(float(priceIns) +((pslostf*float(priceIns))/20),3))
                        pslostl = str(round(float(priceIns) +((pslostl*float(priceIns))/20),3))
                        ...
                        if (mPrices <= Decimal(psloss)  and float(ass) > 0 ):
                            try:
                                ...
                                if float(ass) > float(conts):
                                    ...
                                    ake_profit_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=psf, quantity=str(round(rest,3)),timeInForce='GTC')
                                    #time.sleep(1)
                                    conts = 0
                            except IndexError as error:
                                print("Exiting: with error in try" + "\n")
                        ...
                        #if (mPrices >= Decimal(pslostl)):
                            #cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.LIMIT, price=mPrices, quantity=ass//2,timeInForce='GTC')
                            #cut_loss_short = request_client.post_order(symbol=syms, side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET, quantity= str(round(float(ass),3)))
                        ...
                        if (mPrices <= Decimal(psff) and mPrices > Decimal(psfff) ):
                            # ps = []
                            add_int_short_price =  round(float(priceIns)+ ((float(mPrices) - float(priceIns))/(float(priceIns)*100)*first_p),3)
                            #print("in if")
                            #time.sleep(1)
                            add_short_price = 0
                            ...
                            #while True:
                                ...
                                for keyi in object_list_in:
                                    if (keyi.get('positionSide') =='SHORT' and keyi.get('symbol') =='ATOMUSDT' and keyi.get('positionAmt') < 0.0):
                                        ...
                                        if (float(ass) != float(short_amount) or  mPrices < Decimal(psfff)):
                                            stop_threads = True
                                        psin = str(round(float(priceIns) -((first_p* float(priceIns))/20),3))
                            ...
                            # ps = []
                            add_int_short_price =  round(float(priceIns)+ ((float(mPrices) - float(priceIns))/(float(priceIns)*100)*sec_p),3)
                            #print("in if")
                            #time.sleep(1)
                            add_short_price = 0
                            psl.append(add_int_short_price)
                                ...
                                for keyi in object_list_in:
                                    if (keyi.get('positionSide') =='SHORT' and keyi.get('symbol') =='ATOMUSDT' and keyi.get('positionAmt') < 0.0):
                                        ...
                                        if (float(ass) != float(short_amount) or  mPrices < Decimal(psffff)):
                                            stop_threads = True
                                        psin = str(round(float(priceIns) -((sec_p* float(priceIns))/20),3))
                                        # if (mPrices <= Decimal(psin) ):
                                        #     stop_threads = True
                                        if (float(dym_cur_short_price) < psl[-1]):
                                            if (add_short_p < psl[-1]):
                            ...
                            # ps = []
                            add_int_short_price =  round(float(priceIns)+ ((float(mPrices) - float(priceIns))/(float(priceIns)*100)*thr_p),3)
                            #print("in if")
                            #time.sleep(1)
                            add_short_price = 0
                            psl.append(add_int_short_price)
                                ...
                                for keyi in object_list_in:
                                    if (keyi.get('positionSide') =='SHORT' and keyi.get('symbol') =='ATOMUSDT' and keyi.get('positionAmt') < 0.0):
                                        ...
                                        if (float(ass) != float(short_amount)):
                                            stop_threads = True
                                        psin = str(round(float(priceIns) -((thr_p* float(priceIns))/20),3))
                                        # if (mPrices <= Decimal(psin) ):
                                        #     stop_threads = True
                                        if (float(dym_cur_short_price) < psl[-1]):
                                            if (add_short_p < psl[-1]):
