    ...
    try:
        while True:
            ...
            for keys in object_list_open:
                if ((keys.get('positionSide') =='LONG') and (keys.get('symbol') == "BTCUSD_PERP") and (keys.get('type')=="LIMIT") and (keys.get('status')=="NEW") and (keys.get('side') == "SELL")):
...
###########################
            ...
            for key in object_list:
                ...
                if (key.get('positionSide') =='LONG' and key.get('symbol') =='BTCUSD_PERP' and key.get('positionAmt') > 0.0):
                    sym = key.get('symbol')
                    ...
                    try:
                        ...
                        p = str(round(float(priceIn) +((0.50* float(priceIn))/20),1))
                        pf = str(round(float(priceIn) +((0.02* float(priceIn))/20),1))
                        pff = str(round(float(priceIn) +((0.025* float(priceIn))/20),1))
                        pfff = str(round(float(priceIn) +((0.03* float(priceIn))/20),1))
                        pffff = str(round(float(priceIn) +((0.15* float(priceIn))/20),1))
                        ploss =  str(round(float(priceIn) +((-0.10*float(priceIn))/20),1))
                        plostf =  str(round(float(priceIn) +((-0.15*float(priceIn))/20),1))
                        plostl =  str(round(float(priceIn) +((-0.20*float(priceIn))/20),1))
                        ...
                        #             #print(market,'',markPrice,'',entryPriceLong,'',positionAmtLong)
                        #             ass = str(round(float(positionAmtShort)*-1,1))
                        #             if (mPrice < Decimal(plostf) and float(a) > float(ass)*2):
                        ...
                        #                     cut_loss_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET, quantity= str(round(float(re_unit),1)))
                        if (mPrice >= Decimal(ploss) and float(a) >=6 ):
                            ...
                            time.sleep(5)
                        elif (mPrice >= Decimal(ploss) and float(a) >= 1 and float(a) < 6 ):
                            if float(conts) == 0:
                                take_profit_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.LIMIT, price=pfff, quantity=a,timeInForce='GTC')
                                #time.sleep(5) 
                                conts= 0
                            if float(a) > float(conts):
                                rest = float(a) - float(conts)
                                take_profit_long = request_client.post_order(symbol=sym, side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.LIMIT, price=pfff, quantity=str(rest),timeInForce='GTC')
                                #time.sleep(5) 
                                conts = 0
                        elif (mPrice >= Decimal(pfff) and mPrice < Decimal(pffff)  and float(a) > 0 and float(a) < 1  ):
                            #pl = []
                                ...
                                for key in object_list_in:
                                    if (key.get('positionSide') =='LONG' and key.get('symbol') =='BTCUSD_PERP' and key.get('positionAmt') > 0.0):
                                        ...
                                        if (float(dym_cur_long_price) > pl[-1]):
                                            ...
                                            pl = []       
                                            stop_threads = True    
                        elif (mPrice >= Decimal(pffff) and mPrice < Decimal(p) and float(a) <= 2 ):
                            #pl = []
