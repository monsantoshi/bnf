...
class TryToTakeOrder:
    ...
    def take_order_point(self):
        ...
        result = request_client.get_order_book(symbol = "DOGEUSDT", limit = 20)
        init_bq = []
        init_bp = []
        init_aq = []
        init_ap = []
        ...
        for i in range(20):
            result_xtz = request_client.get_order_book(symbol = "DOGEUSDT", limit = 20)
            #time.sleep(1)
        ...
        if (price_bids[0]< price_bids[19]):
            ...
            l = 0
        #old mdogeod
        kline2 = request_client.get_candlestick_data(symbol="DOGEUSDT", interval=CandlestickInterval.MIN3,startTime=None, endTime=None, limit=20)
        ...
        #doge_TrendDown = Bearish_Engulfing
        print("Down",Down)
        Up =  (float(haopen) < float(haclose)) and (l == 1)
        #doge_TrendUp =  Bullish_Engulfing
        print("Up",Up)
        result_p = request_client.get_position()
        #doge_account_trade = request_client.get_account_trades(symbol="DOGEUSDT",limit= 1)
        #time.sleep(1)
        order_list = []
        ...
        if ((Down) and ((doge_active_plan =='C'))):
       
            print("take_order_short_full")
            take_order_sell(self)
            #take_order_short = request_client.post_order(symbol="DOGEUSDT",  side=OrderSide.SELL, positionSide=PositionSide.SHORT,ordertype=OrderType.MARKET,quantity=sale_unit)
            #PrintBasic.print_obj(take_order_short)
            #time.sleep(1)

            return
       
        if ((Up) and ((doge_active_plan =='B'))):
            print("take_order_long_full")
            take_order_buy(self)
            #take_order_long = request_client.post_order(symbol="DOGEUSDT", side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=buy_unit)
...
class CountCandle:
    ...
    def take_order_point(self):
        ...
        for i in range(20):
            result_doge = request_client.get_order_book(symbol = "DOGEUSDT", limit = 20)
            #time.sleep(1)
            for resb in result_doge.bids:
                #print(resb.qty)
                bid_q.append(round(float(resb.qty),4))
                bid_p.append(round(float(resb.price),4))

               
            for resa in result_doge.asks:
                #print(resb.qty)
                ask_q.append(round(float(resa.qty),4))
                ask_p.append(round(float(resa.price),4))
        ...
        if (price_bids[0]< price_bids[19]):
            ...
            s = 1
        #old mdogeod
        kline2 = request_client.get_candlestick_data(symbol="DOGEUSDT", interval=CandlestickInterval.MIN35,startTime=None, endTime=None, limit=10)
        #print("======= Kline/Candlestick Data =======")
        #PrintMix.print_data(result[0].volume)
        #print("======================================")
        open_list = []
        ...
        result_m = request_client.get_mark_price("DOGEUSDT")
...
class CheckProfitLong:
    ...
    def check_profit_long_doge(self):
            #price_exit = 0
        global stop_threads
        if stop_threads:
            return
            ...
            if (((int_long_rate*100) >= min_profit_rate_long) and (long_amount >=  step_three_unit)):
                take_profit_doge = request_client.post_order(symbol="DOGEUSDT", side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=long_amount)
                PrintBasic.print_obj(take_profit_doge)
                time.sleep(1)
            ...
            if (((int_long_rate*100) >= stop_profit_rate_long) and ((long_amount > step_two_unit) and (long_amount < step_three_unit))):
                ...
                while True:
                    ...
                    #doge_account_trade_in = request_client.get_account_trades(symbol="DOGEUSDT",limit= 1)
                    #time.sleep(1)
                    object_list_in = []
                    ...
                    if (float(dym_cur_long_price) > pl[-1]):
                        ...
                        #cancel_order_all()
                        take_profit_doge = request_client_in.post_order(symbol="DOGEUSDT", side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=long_amount)
                        PrintBasic.print_obj(take_profit_doge)
                        time.sleep(1)
...
class CheckProfitShort:
    ...
    def check_profit_short_doge(self):
            #price_exit = 0
        global stop_threads
        if stop_threads:
            return
            ...
            if (((int_short_rate*100) >= min_profit_rate_short) and (short_amount >= step_three_unit)):
                take_profit_doge = request_client.post_order(symbol="DOGEUSDT", side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=short_amount)
                PrintBasic.print_obj(take_profit_doge)
                time.sleep(1)
            ...
            if (((int_short_rate*100) >= stop_profit_rate_short) and ((short_amount > step_two_unit) and  (short_amount < step_three_unit))):
                ...
                while True:
                    ...
                    if (float(dym_cur_short_price) < ps[-1]):
                        ...
                        #cancel_order_all()
                        take_profit_doge = request_client_in.post_order(symbol="DOGEUSDT", side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=short_amount)
                        PrintBasic.print_obj(take_profit_doge)
                        time.sleep(1)
        ...
        if (in_short_out):
            ...
            if ((int_short_rate_out *100) >= stop_profit_rate_short):
                ...
                if ((short_amount_in == 0) and (short_amount_in < short_amount_out)):
                    take_profit_doge = request_client_in.post_order(symbol="DOGEUSDT", side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=sale_unit)
                    PrintBasic.print_obj(take_profit_doge)
                    time.sleep(1)
...
##                    l = 0
##                #old mdogeod
##                kline2 = request_client.get_candlestick_data(symbol="DOGEUSDT", interval=interval,startTime=None, endTime=None, limit=20)
##                #print("======= Kline/Candlestick Data =======")
##                #PrintMix.print_data(result[0].volume)
##                #print("======================================")
##                open_list = []
...
##                    take_profit_doge = request_client_in.post_order(symbol="DOGEUSDT", side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=buy_unit*2)
##                    PrintBasic.print_obj(take_profit_doge)
##                    time.sleep(1)
##
##                if (in_long_in):
##                    if ((Up) and (long_amount_in < step_three_unit) and (long_amount_in < short_amount_out) and (short_amount_out < step_three_unit)):
##                        take_profit_doge = request_client_in.post_order(symbol="DOGEUSDT", side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=buy_unit)
##                        PrintBasic.print_obj(take_profit_doge)
##                        time.sleep(1)
##                        return
##                    if ((Up) and (long_amount_in < step_three_unit) and (long_amount_in < short_amount_out) and (short_amount_out >= step_three_unit)):
...
##                    take_profit_doge = request_client_in.post_order(symbol="DOGEUSDT", side=OrderSide.SELL, positionSide=PositionSide.SHORT,ordertype=OrderType.MARKET,  quantity=sale_unit*2)
##                    PrintBasic.print_obj(take_profit_doge)
##                    time.sleep(1)
##
##                if (in_short_in):
...
class CheckLossShort:
    ...
    def check_loss_short_doge(self):
...
class CheckProfitLongMore:
    ...
    def check_profit_long_doge(self):
        ...
        if (((int_long_rate*100) >= stop_profit_rate_long_more) and ((long_amount >=  step_three_unit) and (long_amount < step_four_unit))):
            take_profit_doge = request_client.post_order(symbol="DOGEUSDT", side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=buy_unit//2)
            PrintBasic.print_obj(take_profit_doge)
            time.sleep(1)
            stop_threads = True
            #subprocess.call([r'C:\Users\mon\Desktop\Batch_Re\doge_m2End.bat'])
...
class CheckProfitShortMore:
    ...
    def check_profit_short_doge(self):
        ...
        if (((int_short_rate*100) >= stop_profit_rate_short_more) and ((short_amount >= step_three_unit) and (short_amount < step_four_unit))):
            take_profit_doge = request_client.post_order(symbol="DOGEUSDT", side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=sale_unit//2)
            PrintBasic.print_obj(take_profit_doge)
            time.sleep(1)
            stop_threads = True
            #subprocess.call([r'C:\Users\mon\Desktop\Batch_Re\doge_m2End.bat'])
...
class CheckLossLongFromIn:
    ...
    def check_profit_long_doge(self):
        ...
        if long_amount_in > 0:
            ...
            plossfirst = str(round(float(object_list_in[long_idx]["entryPrice"])+((-0.50*(object_list_in[long_idx]["entryPrice"])/20)),4))
        ...
        #print("long_price:  ",self.long_price +"   "+"int_long_price: ",self.int_cur_long_price +"   "+
         #     "int_long_rate: ",round((self.int_long_rate*100),4))
            ...
            if ((float(int_cur_long_price_in) <= float(plossfirst)) and (short_amount_in == 0)):
                ...
                take_profit_doge = request_client_in.post_order(symbol="DOGEUSDT", side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=str(round(float(reentry_unit),4)))
                PrintBasic.print_obj(take_profit_doge)
                time.sleep(1)
...
class CheckLossShortFromIn:
    ...
    def check_profit_short_doge(self):
        ...
        if short_amount_in > 0:
            plossfirst = str(round(float(object_list_in[short_idx]["entryPrice"])-((1.00*(object_list_in[long_idx]["entryPrice"])/20)*-1),4))

        #print("long_price:  ",self.long_price +"   "+"int_long_price: ",self.int_cur_long_price +"   "+
         #     "int_long_rate: ",round((self.int_long_rate*100),4))
            #print("int_long_rate_in : ",round((int_long_rate_in*100),2))
            #print("Mark Price : ",int_cur_long_price_in)
            if ((float(int_cur_short_price_in) >= float(plossfirst)) and (long_amount_in == 0)):

                take_profit_doge = request_client_in.post_order(symbol="DOGEUSDT", side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=str(round(float(reentry_unit),4)))
                PrintBasic.print_obj(take_profit_doge)
                time.sleep(1)
        ...
        try:
            ...
            if email_message.is_multipart():
                ...
                # print('To:\t\t', email_message['To'])
                    ...
                    #rs1 = s1.find('doge3MSH')
                    #rl1 = s1.find('doge3MLO')

                    # How to use find()
                    if (s1.find('BUY doge') != -1):
...
def take_order_sell(self):
    ...
    db = client.binance
    collection_doge = db["cut_loss"]
 
    doge_data = collection_doge.find_one({"market": 'DOGEUSDT'})
    sale_unit = doge_data["sale"]

    step_two_unit = doge_data["step_two_unit"]


    
    count_s = doge_data["count_s"]
    
    limit_s = doge_data["limit_s"]
    reentryunit = str(round(float(doge_data["reentry_unit"]),4))

    if ((short_amount_in == 0) and (short_amount_in <= step_two_unit) and (count_s > limit_s)):
        inc_inc_s(1)
        if ((inc_s % 7)==0):
            take_profit_doge = request_client.post_order(symbol="DOGEUSDT", side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=str(round(float(reentryunit)//4,4)))
            PrintBasic.print_obj(take_profit_doge)
            time.sleep(1)
        else:
            take_profit_doge = request_client.post_order(symbol="DOGEUSDT", side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=reentryunit)
            PrintBasic.print_obj(take_profit_doge)
            time.sleep(1)
        #inc_count_s(1)
        cls_count_s(0)
        ...
        inc_inc_s(1)
        take_profit_doge = request_client.post_order(symbol="DOGEUSDT", side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=sale_unit)
        PrintBasic.print_obj(take_profit_doge)
        time.sleep(1)
        #inc_count_s(1)
        cls_count_s(0)
    ...
    #     take_profit_doge = request_client.post_order(symbol="DOGEUSDT", side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=long_amount_in)
    #     PrintBasic.print_obj(take_profit_doge)
    #     time.sleep(1)
...
def take_order_buy(self):
    ...
    count_l = doge_data["count_l"]
    
    limit_l = doge_data["limit_l"]

    if ((long_amount_in == 0) and (long_amount_in <= step_two_unit) and (count_l > limit_l)):
        inc_inc_l(1)
        if ((inc_l % 7) == 0):
            take_profit_doge = request_client.post_order(symbol="DOGEUSDT", side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=str(round(float(reentryunit)//4,4)))
            PrintBasic.print_obj(take_profit_doge)
            time.sleep(1)
        else:
                    
            take_profit_doge = request_client.post_order(symbol="DOGEUSDT", side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=reentryunit)
            PrintBasic.print_obj(take_profit_doge)
            time.sleep(1)
        #inc_count_s(1)
        cls_count_l(0)
    ...
    #     cut_loss_doge = request_client.post_order(symbol="DOGEUSDT", side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=short_amount_in)
    #     time.sleep(1)
    elif ((long_amount_in > 0) and (long_amount_in <= step_two_unit) and (count_l > limit_l)):
        inc_inc_l(1)
        take_profit_doge = request_client.post_order(symbol="DOGEUSDT", side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=buy_unit)
        PrintBasic.print_obj(take_profit_doge)
        time.sleep(1)
        #inc_count_s(1)
        cls_count_l(0)
...
def inc_inc_l(count):
    ...
    #if (count_s <= limit_s):
    inc_ll = inc_l + count
    collection_doge.update_one({"market": 'DOGEUSDT'},{"$set":{"inc_l":inc_ll} })
    #f (count_s > limit_s+1):
     #   #count_ss = count_s + count
     #   collection_doge.update_one({"market": 'DOGEUSDT'},{"$set":{"count_s":0} })
...
def inc_inc_s(count):
    ...
    #if (count_s <= limit_s):
    inc_ss = inc_s + count
    collection_doge.update_one({"market": 'DOGEUSDT'},{"$set":{"inc_s":inc_ss} })
    #f (count_s > limit_s+1):
     #   #count_ss = count_s + count
     #   collection_doge.update_one({"market": 'DOGEUSDT'},{"$set":{"count_s":0} })
...
def inc_count_s(count):
    ...
    if (count_s <= limit_s):
        count_ss = count_s + count
        collection_doge.update_one({"market": 'DOGEUSDT'},{"$set":{"count_s":count_ss} })
...
def cls_count_s(count):
    ...
    db = client.binance
    collection_doge = db["cut_loss"]
    collection_doge.update_one({"market": 'DOGEUSDT'},{"$set":{"count_s":count} })
...
def inc_count_l(count):
    ...
    if (count_l <= limit_l):
        ...
        collection_doge.update_one({"market": 'DOGEUSDT'},{"$set":{"count_l":count_ll} })
    if (count_l > limit_l+1):
        #count_ss = count_s + count
        collection_doge.update_one({"market": 'DOGEUSDT'},{"$set":{"count_l":0} })
...
def cls_count_l(count):
    ...
    db = client.binance
    collection_doge = db["cut_loss"]
    collection_doge.update_one({"market": 'DOGEUSDT'},{"$set":{"count_l":count} })
    return
...
##            take_profit_doge = request_client.post_order(symbol="DOGEUSDT",newClientOrderId="long10000" , side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=reentry)
##            PrintBasic.print_obj(take_profit_doge)
##            time.sleep(5)
##            stop_threads = True
##            #subprocess.call([r'C:\Users\mon\Desktop\Batch_Re\doge_m2End.bat'])
...
##            take_profit_doge = request_client.post_order(symbol="DOGEUSDT", side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=reentrys)
##            PrintBasic.print_obj(take_profit_doge)
##            time.sleep(1)
##            #stop_threads = True
##            #subprocess.call([r'C:\Users\mon\Desktop\Batch_Re\doge_m2End.bat'])
...
class TakeProfitDoge:
    ...
    def clean_open_order_doge(self):
        ...
        for key in object_list_open:
            if ((key.get('positionSide') =='LONG') and (key.get('side') == "SELL") and (key.get('symbol') == "DOGEUSDT")):
        ...
        for keys in object_list_open:
            if ((keys.get('positionSide') =='SHORT') and (keys.get('side') == "BUY") and (keys.get('symbol') == "DOGEUSDT")):
    ...
    def take_profit_long_doge(self):
        ...
        for key in object_list:
            if (key.get('positionSide') =='LONG' and (key.get('symbol') == "DOGEUSDT") and key.get('positionAmt') >= 0.0):
        ...
        for keys in object_list:
            if (keys.get('positionSide') =='SHORT' and (keys.get('symbol') == "DOGEUSDT") and keys.get('positionAmt') <= 0.0):
...
if __name__ == '__main__':
    ...
    index_data = []
    index_data = collection_test.find({"symbol": 'DOGEUSDT'})
    ...
    stop_profit_rate_short = doge_data["mgain_profit_percent_s"]
    stop_profit_rate_long = doge_data["mgain_profit_percent"]
    stop_profit_rate_short_more = doge_data["mgain_profit_percent_s"]
    stop_profit_rate_long_more = doge_data["mgain_profit_percent"]
    min_profit_rate_short = doge_data["fgain_profit_percent_s"]
    min_profit_rate_long = doge_data["fgain_profit_percent"]
    step_two_profit_short = doge_data["lgain_profit_percent_s"]
    step_two_profit_long = doge_data["lgain_profit_percent"]
    stop_loss_rate_long = doge_data["loss_percent"]
    stop_loss_rate_short = doge_data["loss_percent_s"]
    buy_unit = doge_data["buy"]
    sale_unit = doge_data["sale"]
    step_one_unit = doge_data["step_one_unit"]
    step_two_unit = doge_data["step_two_unit"]
    step_three_unit = doge_data["step_three_unit"]
    step_four_unit = doge_data["step_four_unit"]
##    step_five_unit = doge_data["step_five_unit"]
##    step_six_unit = doge_data["step_six_unit"]
##    step_seven_unit = doge_data["step_seven_unit"]
    guided = doge_data["guided"]
    macd = doge_data["macd"]
    count_l = doge_data["count_l"]
    count_s = doge_data["count_s"]
    limit_l = doge_data["limit_l"]
    limit_s = doge_data["limit_s"]
    inc_l = doge_data["inc_l"]
    inc_s = doge_data["inc_s"]
    #interval = doge_data["interval"]
    reentry_unit = doge_data["reentry_unit"]
    ...
    try:
        while True:
            ...
            if (short_amount_in > 0):
                ...
                int_cur_short_price = str(round(float(object_list[short_idx]["markPrice"]),4))
                
                #int_long_rate = (float(int_cur_long_price) - float(long_price)) / float(long_price) * 20

                #plosslast = str(round(float(object_list[short_idx]["entryPrice"])-((3.00*(object_list[short_idx]["entryPrice"])/20)*-1),4))

                plossfirst = str(round(float(object_list[short_idx]["entryPrice"])-((1.00*(object_list[short_idx]["entryPrice"])/20)*-1),4))

                #ploss = str(round(float(object_list[long_idx]["entryPrice"]+((-3.00*object_list[long_idx]["entryPrice"])/20)),2))
                print("entryPrice",float(object_list[short_idx]["entryPrice"]))
                ...
                if ((float(int_cur_short_price) >= float(plossfirst))):
                    thread14 = Bin14(14,"Check Loss Short More")
                    #thread8.start()
                    #thread5.start()
                    thread14.start()
                    time.sleep(1)
                ...
                #     #thread8 = Bin8(8, "Check Loss Long doge")
