...
def supertrend(df, period=7, atr_multiplier=4):
    ...
    adx = df.ta.adx()
    kdj = df.ta.kdj()
    df = pd.concat([df,adx,kdj],axis=1)
...
def check_buy_sell_signals(df):
    ...
    #         close_order_buy_coin_usdt(self,'BTCUSDT')  
    #         in_position = False
    #         message = f"CLOSE POSITION LONG BTCUSDT"
    ...
    #         #order = exchange.create_market_buy_order('ETH/USD', 0.05)
    #     #order = exchange.create_order(symbol='adausdt', side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=1)
    #         close_order_sell_coin_usdt(self,'BTCUSDT')  
    #         in_position = False
    #         message = f"CLOSE POSITION SHORT BTCUSDT"
...
def run_bot():
    ...
    # bars = exchange.fetch_ohlcv('BTCUSDT', timeframe='5m', limit=140)
    ...
    bars = request_client.get_candlestick_data(symbol="BTCUSDT", interval=CandlestickInterval.MIN3,startTime=None, endTime=None, limit=100)
