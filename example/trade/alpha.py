...
class TryToTakeOrder:
    ...
    def take_order_point(self):
        ...
        result = request_client.get_order_book(symbol = "ALPHAUSDT", limit = 20)
        init_bq = []
        init_bp = []
        init_aq = []
        init_ap = []
        ...
        for i in range(20):
            result_xtz = request_client.get_order_book(symbol = "ALPHAUSDT", limit = 20)
            #time.sleep(1)
        ...
        if (price_bids[0]< price_bids[19]):
            ...
            l = 0
        #old malphaod
        kline2 = request_client.get_candlestick_data(symbol="ALPHAUSDT", interval=CandlestickInterval.MIN1,startTime=None, endTime=None, limit=20)
        ...
        #alpha_TrendDown = Bearish_Engulfing
        print("Down",Down)
        Up =  (float(haopen) < float(haclose)) and (l == 1)
        #alpha_TrendUp =  Bullish_Engulfing
        print("Up",Up)
        ...
        if ((Down) and ((alpha_active_plan =='C'))):
...
class CountCandle:
    ...
    def take_order_point(self):
        ...
        for i in range(20):
            result_alpha = request_client.get_order_book(symbol = "ALPHAUSDT", limit = 20)
            #time.sleep(1)
            for resb in result_alpha.bids:
                #print(resb.qty)
                bid_q.append(round(float(resb.qty),4))
                bid_p.append(round(float(resb.price),4))

               
            for resa in result_alpha.asks:
                #print(resb.qty)
                ask_q.append(round(float(resa.qty),4))
                ask_p.append(round(float(resa.price),4))
        ...
        if (price_bids[0]< price_bids[19]):
            ...
            s = 1
        #old malphaod
        kline2 = request_client.get_candlestick_data(symbol="ALPHAUSDT", interval=CandlestickInterval.MIN15,startTime=None, endTime=None, limit=10)
        #print("======= Kline/Candlestick Data =======")
        #PrintMix.print_data(result[0].volume)
        #print("======================================")
        open_list = []
        ...
        result_m = request_client.get_mark_price("ALPHAUSDT")
...
class CheckProfitLong:
    ...
    def check_profit_long_alpha(self):
            #price_exit = 0
        global stop_threads
        if stop_threads:
            return
            ...
            if (((int_long_rate*100) >= min_profit_rate_long) and (long_amount >=  step_three_unit)):
                take_profit_alpha = request_client.post_order(symbol="ALPHAUSDT", side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=long_amount)
            ...
            if (((int_long_rate*100) >= stop_profit_rate_long) and ((long_amount > step_two_unit) and (long_amount < step_three_unit))):
                ...
                while True:
                    ...
                    #alpha_account_trade_in = request_client.get_account_trades(symbol="ALPHAUSDT",limit= 1)
                    #time.sleep(1)
                    object_list_in = []
                    ...
                    if (float(dym_cur_long_price) > pl[-1]):
                        ...
                        #cancel_order_all()
                        take_profit_alpha = request_client_in.post_order(symbol="ALPHAUSDT", side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=long_amount)
                        PrintBasic.print_obj(take_profit_alpha)
                        time.sleep(1)
...
class CheckProfitShort:
    ...
    def check_profit_short_alpha(self):
            #price_exit = 0
        global stop_threads
        if stop_threads:
            return
            ...
            if (((int_short_rate*100) >= min_profit_rate_short) and (short_amount >= step_three_unit)):
                take_profit_alpha = request_client.post_order(symbol="ALPHAUSDT", side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=short_amount)
                PrintBasic.print_obj(take_profit_alpha)
                time.sleep(1)
            ...
            if (((int_short_rate*100) >= stop_profit_rate_short) and ((short_amount > step_two_unit) and  (short_amount < step_three_unit))):
                ...
                while True:
                    ...
                    if (float(dym_cur_short_price) < ps[-1]):
                        ...
                        #cancel_order_all()
                        take_profit_alpha = request_client_in.post_order(symbol="ALPHAUSDT", side=OrderSide.BUY,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=short_amount)
                        PrintBasic.print_obj(take_profit_alpha)
                        time.sleep(1)
        ...
        if (in_short_out):
            ...
            if ((int_short_rate_out *100) >= stop_profit_rate_short):
                ...
                if ((short_amount_in == 0) and (short_amount_in < short_amount_out)):
                    take_profit_alpha = request_client_in.post_order(symbol="ALPHAUSDT", side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=sale_unit)
                    PrintBasic.print_obj(take_profit_alpha)
                    time.sleep(1)
...
##                    l = 0
##                #old malphaod
##                kline2 = request_client.get_candlestick_data(symbol="ALPHAUSDT", interval=interval,startTime=None, endTime=None, limit=20)
##                #print("======= Kline/Candlestick Data =======")
##                #PrintMix.print_data(result[0].volume)
##                #print("======================================")
##                open_list = []
...
##                    take_profit_alpha = request_client_in.post_order(symbol="ALPHAUSDT", side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=buy_unit*2)
##                    PrintBasic.print_obj(take_profit_alpha)
##                    time.sleep(1)
##
##                if (in_long_in):
##                    if ((Up) and (long_amount_in < step_three_unit) and (long_amount_in < short_amount_out) and (short_amount_out < step_three_unit)):
##                        take_profit_alpha = request_client_in.post_order(symbol="ALPHAUSDT", side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=buy_unit)
##                        PrintBasic.print_obj(take_profit_alpha)
##                        time.sleep(1)
##                        return
##                    if ((Up) and (long_amount_in < step_three_unit) and (long_amount_in < short_amount_out) and (short_amount_out >= step_three_unit)):
...
##                    take_profit_alpha = request_client_in.post_order(symbol="ALPHAUSDT", side=OrderSide.SELL, positionSide=PositionSide.SHORT,ordertype=OrderType.MARKET,  quantity=sale_unit*2)
##                    PrintBasic.print_obj(take_profit_alpha)
##                    time.sleep(1)
##
##                if (in_short_in):
...
class CheckLossLong:
    ...
    def check_loss_long_alpha(self):
            #price_exit = 0
...
class CheckLossShort:
    ...
    def check_loss_short_alpha(self):
...
class CheckProfitLongMore:
    ...
    def check_profit_long_alpha(self):
        ...
        if (((int_long_rate*100) >= stop_profit_rate_long_more) and ((long_amount >=  step_three_unit) and (long_amount < step_four_unit))):
            take_profit_alpha = request_client.post_order(symbol="ALPHAUSDT", side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=buy_unit//2)
            PrintBasic.print_obj(take_profit_alpha)
            time.sleep(1)
            stop_threads = True
            #subprocess.call([r'C:\Users\mon\Desktop\Batch_Re\alpha_m2End.bat'])
...
class CheckProfitShortMore:
    ...
    def check_profit_short_alpha(self):
        ...
        if (((int_short_rate*100) >= stop_profit_rate_short_more) and ((short_amount >= step_three_unit) and (short_amount < step_four_unit))):
            take_profit_alpha = request_client.post_order(symbol="ALPHAUSDT", side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=sale_unit//2)
            PrintBasic.print_obj(take_profit_alpha)
            time.sleep(1)
            stop_threads = True
            #subprocess.call([r'C:\Users\mon\Desktop\Batch_Re\alpha_m2End.bat'])
        ...
        #print("long_price:  ",self.long_price +"   "+"int_long_price: ",self.int_cur_long_price +"   "+
         #     "int_long_rate: ",round((self.int_long_rate*100),4))
            ...
            if ((float(int_cur_long_price_in) <= float(plossfirst)) and (short_amount_in == 0)):
                ...
                take_profit_alpha = request_client_in.post_order(symbol="ALPHAUSDT", side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=str(round(float(reentry_unit),4)))
                PrintBasic.print_obj(take_profit_alpha)
                time.sleep(1)
        ...
        #print("long_price:  ",self.long_price +"   "+"int_long_price: ",self.int_cur_long_price +"   "+
         #     "int_long_rate: ",round((self.int_long_rate*100),4))
            ...
            if ((float(int_cur_short_price_in) >= float(plossfirst)) and (long_amount_in == 0)):
                ...
                take_profit_alpha = request_client_in.post_order(symbol="ALPHAUSDT", side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=str(round(float(reentry_unit),4)))
                PrintBasic.print_obj(take_profit_alpha)
                time.sleep(1)
...
def take_order_sell(self):
    ...
    reentryunit = str(round(float(alpha_data["reentry_unit"]),4))

    if ((short_amount_in == 0) and (short_amount_in <= step_two_unit) and (count_s > limit_s)):
        inc_inc_s(1)
        if ((inc_s % 7)==0):
            take_profit_alpha = request_client.post_order(symbol="ALPHAUSDT", side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=str(round(float(reentryunit)//4,4)))
            PrintBasic.print_obj(take_profit_alpha)
            time.sleep(1)
        else:
    ...
    #     take_profit_alpha = request_client.post_order(symbol="ALPHAUSDT", side=OrderSide.SELL,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=long_amount_in)
    #     PrintBasic.print_obj(take_profit_alpha)
    #     time.sleep(1)
...
def take_order_buy(self):
    ...
    if ((long_amount_in == 0) and (long_amount_in <= step_two_unit) and (count_l > limit_l)):
        inc_inc_l(1)
        if ((inc_l % 7) == 0):
            take_profit_alpha = request_client.post_order(symbol="ALPHAUSDT", side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=str(round(float(reentryunit)//4,4)))
            PrintBasic.print_obj(take_profit_alpha)
            time.sleep(1)
        else:
...
def inc_inc_l(count):
    ...
    #if (count_s <= limit_s):
    inc_ll = inc_l + count
    collection_alpha.update_one({"market": 'ALPHAUSDT'},{"$set":{"inc_l":inc_ll} })
    #f (count_s > limit_s+1):
     #   #count_ss = count_s + count
     #   collection_alpha.update_one({"market": 'ALPHAUSDT'},{"$set":{"count_s":0} })
...
def inc_inc_s(count):
    ...
    #if (count_s <= limit_s):
    inc_ss = inc_s + count
    collection_alpha.update_one({"market": 'ALPHAUSDT'},{"$set":{"inc_s":inc_ss} })
    #f (count_s > limit_s+1):
     #   #count_ss = count_s + count
     #   collection_alpha.update_one({"market": 'ALPHAUSDT'},{"$set":{"count_s":0} })
...
def inc_count_s(count):
    ...
    if (count_s <= limit_s):
        count_ss = count_s + count
        collection_alpha.update_one({"market": 'ALPHAUSDT'},{"$set":{"count_s":count_ss} })
...
def cls_count_s(count):
    ...
    db = client.binance
    collection_alpha = db["cut_loss"]
    collection_alpha.update_one({"market": 'ALPHAUSDT'},{"$set":{"count_s":count} })
...
def inc_count_l(count):
    ...
    if (count_l <= limit_l):
        ...
        collection_alpha.update_one({"market": 'ALPHAUSDT'},{"$set":{"count_l":count_ll} })
    if (count_l > limit_l+1):
        #count_ss = count_s + count
        collection_alpha.update_one({"market": 'ALPHAUSDT'},{"$set":{"count_l":0} })
...
def cls_count_l(count):
    ...
    db = client.binance
    collection_alpha = db["cut_loss"]
    collection_alpha.update_one({"market": 'ALPHAUSDT'},{"$set":{"count_l":count} })
    return
...
##            take_profit_alpha = request_client.post_order(symbol="ALPHAUSDT",newClientOrderId="long10000" , side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=reentry)
##            PrintBasic.print_obj(take_profit_alpha)
##            time.sleep(5)
##            stop_threads = True
##            #subprocess.call([r'C:\Users\mon\Desktop\Batch_Re\alpha_m2End.bat'])
...
##            take_profit_alpha = request_client.post_order(symbol="ALPHAUSDT", side=OrderSide.SELL,positionSide=PositionSide.SHORT, ordertype=OrderType.MARKET,  quantity=reentrys)
##            PrintBasic.print_obj(take_profit_alpha)
##            time.sleep(1)
##            #stop_threads = True
##            #subprocess.call([r'C:\Users\mon\Desktop\Batch_Re\alpha_m2End.bat'])
...
class TakeProfitDoge:
    ...
    def clean_open_order_alpha(self):
        ...
        for key in object_list_open:
            if ((key.get('positionSide') =='LONG') and (key.get('side') == "SELL") and (key.get('symbol') == "ALPHAUSDT")):
        ...
        for keys in object_list_open:
            if ((keys.get('positionSide') =='SHORT') and (keys.get('side') == "BUY") and (keys.get('symbol') == "ALPHAUSDT")):
    ...
    def take_profit_long_alpha(self):
        ...
        for key in object_list:
            if (key.get('positionSide') =='LONG' and (key.get('symbol') == "ALPHAUSDT") and key.get('positionAmt') >= 0.0):
        ...
        for keys in object_list:
            if (keys.get('positionSide') =='SHORT' and (keys.get('symbol') == "ALPHAUSDT") and keys.get('positionAmt') <= 0.0):
...
if __name__ == '__main__':
    ...
    index_data = []
    index_data = collection_test.find({"symbol": 'ALPHAUSDT'})
...
##    step_seven_unit = alpha_data["step_seven_unit"]
    ...
    limit_l = alpha_data["limit_l"]
    limit_s = alpha_data["limit_s"]
    inc_l = alpha_data["inc_l"]
    inc_s = alpha_data["inc_s"]
    ...
    try:
        while True:
            ...
            if (short_amount_in > 0):
                ...
                #     #thread8 = Bin8(8, "Check Loss Long alpha")
