...
def take_order_buy_btcusdt():
       
    api_key = config.BINANCE_API_KEY 
    api_secret = config.BINANCE_SECRET_KEY
    client = Client(api_key, api_secret)
    my_balance = client.get_asset_balance(asset='BTC')

    unit_avl = float(my_balance['free'])   

    if (unit_avl < 0.0002): 
        order = client.create_order(symbol='BTCUSDT',side=Client.SIDE_BUY,type=Client.ORDER_TYPE_MARKET,quantity=0.0002)
    return
def truncate(number, digits) -> float:
    stepper = 10.0 ** digits
    return math.trunc(stepper * number) / stepper
def take_order_sell_btcusdt():
    ...
    my_trades = client.get_my_trades(symbol='BTCUSDT',limit=2)

    for trade in my_trades:
        if trade['isBuyer']==True:
            print(trade['price'])
            pl = []
            sell_price = str(round(float(trade['price'])*1.05,1))
            first_price = 0
    ...
    if (float(unit_avl) > 0):
        #print("unit_avl", unit_avl)
        order = client.create_order(symbol='BTCUSDT',side=Client.SIDE_SELL,type=Client.ORDER_TYPE_LIMIT,price=pricef,quantity=unit_avl,timeInForce=Client.TIME_IN_FORCE_GTC)
    ...
    if df['in_uptrendAll'][last_row_index] == 3:
        ...
        if not in_position:
            ...
            #order = exchange.create_order(symbol='adausdt', side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=1)
                ...
                order = take_order_buy_btcusdt()

                print(order)
                in_position = True
                message = f"BUY BTC/USDT"
        ...
        if in_position:
            ...
            if (df['in_uptrendAll'][previous_row_index] == 1 or df['in_uptrendAll'][previous_row_index] == 8 or df['in_uptrendAll'][previous_row_index] == 3 or df['in_uptrendAll'][previous_row_index] == 0 or df['in_uptrendAll'][previous_row_index] == 6 or df['in_uptrendAll'][previous_row_index] == 4):
                ...
                order = take_order_sell_btcusdt()

                print(order)
                in_position = False
                message = f"SELL BTC/USDT"
...
def run_bot():
    ...
    bars = exchange.fetch_ohlcv('BTC/USDT', timeframe='5m', limit=140)
    print("SPOT BTC/USDT")
    #bars = sub_client.subscribe_candlestick_event("btcusdt", CandlestickInterval.MIN30, callback, error)
