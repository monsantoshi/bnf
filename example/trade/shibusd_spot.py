...
def take_order_buy_shibusdt():
       
    api_key = config.BINANCE_API_KEY 
    api_secret = config.BINANCE_SECRET_KEY
    client = Client(api_key, api_secret)
    my_balance = client.get_asset_balance(asset='SHIB')
    unit_avl = float(my_balance['free'])   
    if (unit_avl < 1000): 
        order = client.create_order(symbol='SHIBUSDT',side=Client.SIDE_BUY,type=Client.ORDER_TYPE_MARKET,quantity=150000)
    return
def truncate(number, digits) -> float:
    stepper = 10.0 ** digits
    return math.trunc(stepper * number) / stepper

def take_order_sell_shibusdt():

    api_key = config.BINANCE_API_KEY 
    api_secret = config.BINANCE_SECRET_KEY
    client = Client(api_key, api_secret)
    my_balance = client.get_asset_balance(asset='SHIB')
    unit_avl = truncate(float(my_balance['free']),0)
    #unit_tp = '%.3f'%(round(float(my_balance['free']),5))
    print("unit_avl",unit_avl)
    my_trades = client.get_my_trades(symbol='SHIBUSDT',limit=2)

    for trade in my_trades:
        if trade['isBuyer']==True:
            print(trade['price'])
            pl = []
            sell_price = str(round(float(trade['price'])*1.05,8))
            first_price = 0
            ...
            if (float(sell_price) > pl[-1]):
                ...
                print("add_last_price" , sell_price)
                    #time.sleep(1)
                print("pl in if",pl)
    pricef = str(round(float(pl[-1]),8))
    print("last_TP_price",pricef)

    if (float(unit_avl) > 0):
        #print("unit_avl", unit_avl)
        order = client.create_order(symbol='SHIBUSDT',side=Client.SIDE_SELL,type=Client.ORDER_TYPE_LIMIT,price=pricef,quantity=unit_avl,timeInForce=Client.TIME_IN_FORCE_GTC)
    ...
    if df['in_uptrendAll'][last_row_index] == 3:
        ...
        if not in_position:
            ...
            #order = exchange.create_order(symbol='adausdt', side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=1)
                ...
                order = take_order_buy_shibusdt()

                print(order)
                in_position = True
                message = f"BUY SHIB/USDT"
        ...
        if in_position:
            ...
            if (df['in_uptrendAll'][previous_row_index] == 1 or df['in_uptrendAll'][previous_row_index] == 8 or df['in_uptrendAll'][previous_row_index] == 3 or df['in_uptrendAll'][previous_row_index] == 0 or df['in_uptrendAll'][previous_row_index] == 6 or df['in_uptrendAll'][previous_row_index] == 4):
                ...
                order = take_order_sell_shibusdt()

                print(order)
                in_position = False
                message = f"SELL SHIB/USDT"
...
#         print("changed to uptrend, buy")
#         if not in_position:
#             #order = exchange.create_market_buy_order('SHIB/USD', 0.05)
#             #order = exchange.create_order(symbol='adausdt', side=OrderSide.BUY,positionSide=PositionSide.LONG, ordertype=OrderType.MARKET,  quantity=1)
...
def run_bot():
    ...
    bars = exchange.fetch_ohlcv('SHIB/USDT', timeframe='30m', limit=140)
    print("SPOT SHIB/USDT")
    #bars = sub_client.subscribe_candlestick_event("shibusdt", CandlestickInterval.MIN30, callback, error)
